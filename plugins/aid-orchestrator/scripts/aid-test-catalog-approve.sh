...
proposed_path="" project_root="" approved_path=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --approved-path) [[ $# -ge 2 ]] || _die 2 "--approved-path requires a value"; approved_path="$2"; shift 2 ;;
...
elif [[ "$approved_path" != /* ]]; then
  # Codex review: a relative --approved-path was previously written relative
  # to the CALLER's cwd, but `git -C "$project_root" add` resolves the same
  # relative string against project_root — a mismatch could publish one file
  # and stage a different (or nonexistent) path. Resolve explicitly against
  # project_root instead, matching every other path in this script.
  approved_path="${project_root}/${approved_path}"
fi

case "$approved_path" in
  "${project_root}"/*) ;;
  *) _die 2 "--approved-path '$approved_path' must resolve inside --project-root ($project_root)" ;;
esac
