...
AID_PR_EVIDENCE_FORMS='`path:line` or `path:first-last` inside the repository, `plan.md:line` for the plan itself (the line numbers of the plan in the packet below), or `absent:path` for a file the plan presumes and the repository lacks; one citation that resolves is enough'
