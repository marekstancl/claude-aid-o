...
**Acceptance Criteria:**
- [ ] Nález revize kola 2 „the round-1 fix was appended but the crashing expression was left in place" je vyřešen: pravidlo (3) čte jméno z `fields_meta`, katalog jen jako záloha, jedna složka `<kód>_neznamy_druh/` v pravidle, Error Handling i testu.
- [ ] Export potvrzeného dokladu, jehož druh byl mezitím smazán nebo nahrazen šablonou, projde: složka `<kód>_neznamy_druh/`, varování v `_navod.txt` (test v `backend/tests/export/test_manifest.py`).
