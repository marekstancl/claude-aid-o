...
depends_on_plans: []
---
<!-- Fixture: verbatim copy of ACTA .aid-o/plans/P025-katalog-druhu-rady-typ-dd-export.md at ACTA commit 9b68f91c98a0 (2026-09-17), id changed to P998. ACTA itself is not modified. Before review the author changed one Step 15 bullet from Create to Modify and dropped its Reuse check: the Docusaurus page had been created in the meantime and the deterministic check blocked the plan (B8). -->
...
**Reuse check:** searched: `find backend/db/versions -name "*sequence*"` → several matching `0016_client_sequences.py`, `0017_document_sequence_number.py`, `0019_client_sequence_mask.py`, `0024_sequence_direction.py`, `0027_unique_sequence_number.py` — jsou to historické migrace téže tabulky; nová migrace na ně navazuje (`down_revision = "0035"`) a přebírá z `0019` vzor „přidat sloupec, backfill, zpřísnit NOT NULL", ale žádná z nich sloupec `code` nezavádí.
