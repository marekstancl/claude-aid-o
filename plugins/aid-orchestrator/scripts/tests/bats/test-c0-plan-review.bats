...
# ═══════════════════════════════════════════════════════════════════════════
# Finding 3 tests: AID_C0_ATTEMPT evidence layering + loop-summary (Step 18 port)
# ═══════════════════════════════════════════════════════════════════════════

@test "Step 18 (Finding 3): AID_C0_ATTEMPT unset → legacy behavior, reports go to c0-plan-review.json root" {
  _build_high
  [ "$status" -eq 0 ]
  _seed_dispatch_env
  FAKE_C0_MODE=valid run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
...
@test "Step 18 (Finding 3): AID_C0_ATTEMPT=1 → attempt is layered under c0/attempt-01/, report copied to canonical root" {
  _build_high
  [ "$status" -eq 0 ]
  _seed_dispatch_env
  AID_C0_ATTEMPT=1 FAKE_C0_MODE=valid run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 0 ]
  # Canonical root must have the report
  [ -f "$REPORT" ]
  run jq -r '.review_status' "$REPORT"
  [ "$output" = "pass" ]
  # Attempt dir must have its own copy
  [ -f "$C0_EVIDENCE_DIR/c0/attempt-01/c0-plan-review.json" ]
  # Attempt must have its own manifest snapshot
  [ -f "$C0_EVIDENCE_DIR/c0/attempt-01/audit-input-manifest.json" ]
  # loop-summary must be created
  [ -f "$C0_EVIDENCE_DIR/c0/loop-summary.json" ]
  run jq -r '.attempts[0].n' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "1" ]
  run jq -r '.attempts[0].outcome' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "dispatched" ]
}

@test "Step 18: two sequential attempts with different findings triggers same-fingerprint-survived escalation" {
  _build_high
  [ "$status" -eq 0 ]
  _seed_dispatch_env

  # First attempt: one finding
  AID_C0_ATTEMPT=1 FAKE_C0_MODE=findings FAKE_C0_FINDINGS='[{"severity":"high","area":"test","finding":"bad thing","recommendation":"fix it"}]' \
    run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 0 ]
  run jq -r '.findings[0].fingerprint' "$REPORT"
  first_fp="$output"
  # Attempt 1 must record "fail" + recheck_count=0 (first attempt doesn't decrement)
  run jq -r '.attempts[0].outcome' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "dispatched" ]
  run jq -r '.recheck_count' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "0" ]
  run jq -r '.outcome' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "null" ]  # blocking, budget not exhausted yet

  # Second attempt: SAME finding (same fingerprint) — must escalate
  AID_C0_ATTEMPT=2 FAKE_C0_MODE=findings FAKE_C0_FINDINGS='[{"severity":"high","area":"test","finding":"bad thing","recommendation":"fix it"}]' \
    run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 0 ]
  run jq -r '.findings[0].fingerprint' "$REPORT"
  second_fp="$output"
  [ "$second_fp" = "$first_fp" ]  # fingerprints must match (same finding)
  # loop-summary must now show escalated + same_fingerprint_survived
  run jq -r '.outcome' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "escalated" ]
  run jq -r '.escalation_reason' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "same_fingerprint_survived" ]
  run jq -r '.recheck_count' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "1" ]
}

@test "Step 18: two sequential attempts with different findings does NOT escalate (fix worked)" {
  _build_high
  [ "$status" -eq 0 ]
  _seed_dispatch_env

  # First attempt: one finding
  AID_C0_ATTEMPT=1 FAKE_C0_MODE=findings FAKE_C0_FINDINGS='[{"severity":"high","area":"old","finding":"problem A","recommendation":"fix A"}]' \
    run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 0 ]

  # Second attempt: different finding (fix worked) — must NOT escalate
  AID_C0_ATTEMPT=2 FAKE_C0_MODE=findings FAKE_C0_FINDINGS='[{"severity":"high","area":"new","finding":"problem B","recommendation":"fix B"}]' \
    run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 0 ]
  # loop-summary must still be null (blocking but fix worked, no escalation yet)
  run jq -r '.outcome' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "null" ]
  run jq -r '.recheck_count' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "1" ]
...
@test "Step 18: terminal guard rejects attempt after 'clean' outcome without override" {
  _build_high
  [ "$status" -eq 0 ]
  _seed_dispatch_env

  # First attempt: clean (pass)
  AID_C0_ATTEMPT=1 FAKE_C0_MODE=valid run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 0 ]
  run jq -r '.outcome' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "clean" ]

  # Second attempt WITHOUT override — must be rejected
  AID_C0_ATTEMPT=2 FAKE_C0_MODE=valid run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 1 ]
  grep -q "PRECONDITION FAIL" <<<"$output" || grep -q "bounded-loop" <<<"$output"
}

@test "Step 18: terminal guard allows attempt with AID_C0_FORCE_BEYOND_ESCALATION override (>=20 chars)" {
  _build_high
  [ "$status" -eq 0 ]
  _seed_dispatch_env

  # First attempt: escalated
  AID_C0_ATTEMPT=1 FAKE_C0_MODE=findings FAKE_C0_FINDINGS='[{"severity":"high","area":"test","finding":"bad","recommendation":"fix"}]' \
    run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 0 ]

  # Move to attempt 2 with same-fingerprint to escalate
  AID_C0_ATTEMPT=2 FAKE_C0_MODE=findings FAKE_C0_FINDINGS='[{"severity":"high","area":"test","finding":"bad","recommendation":"fix"}]' \
    run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 0 ]
  run jq -r '.outcome' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "escalated" ]

  # Third attempt WITH override — must proceed
  AID_C0_ATTEMPT=3 AID_C0_FORCE_BEYOND_ESCALATION="PM-authorized recheck for investigation purposes" \
    FAKE_C0_MODE=valid run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 0 ]
  run jq -r '.attempts[2].n' "$C0_EVIDENCE_DIR/c0/loop-summary.json"
  [ "$output" = "3" ]
}

@test "Step 18: collision guard prevents reusing a 'dispatched' attempt number" {
  _build_high
  [ "$status" -eq 0 ]
  _seed_dispatch_env

  # First attempt with attempt-02
  AID_C0_ATTEMPT=2 FAKE_C0_MODE=valid run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 0 ]
  [ -f "$C0_EVIDENCE_DIR/c0/attempt-02/c0/c0-dispatch.json" ]
  run jq -r '.dispatch.outcome' "$C0_EVIDENCE_DIR/c0/attempt-02/c0/c0-dispatch.json"
  [ "$output" = "dispatched" ]

  # Try to reuse attempt-02 — must be rejected
  AID_C0_ATTEMPT=2 FAKE_C0_MODE=valid run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 1 ]
  grep -q "already recorded a completed dispatch" <<<"$output"
}

@test "Step 18: collision guard allows retrying a non-dispatched attempt (e.g. rate_limited)" {
  _build_high
  [ "$status" -eq 0 ]
  _seed_dispatch_env

  # First attempt with attempt-03: rate_limited (not dispatched)
  AID_C0_ATTEMPT=3 FAKE_C0_MODE=rate_limited run bash "$DISPATCH" dispatch "$C0_EVIDENCE_DIR"
  [ "$status" -eq 2 ]
  run jq -r '.dispatch.outcome' "$C0_EVIDENCE_DIR/c0/attempt-03/c0/c0-dispatch.json"
  [ "$output" = "rate_limited" ]

  # Retry attempt-03 (non-dispatched, so collision is OK) — must succeed
