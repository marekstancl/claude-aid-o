...
@test "start-only emits timeline + pending entries" {
  run bash "$SCRIPT" start --focus cp2-step-1-step-generalist --agent-id aid-orchestrator:verifier --evidence-dir "$EVID"
  [ "$status" -eq 0 ]
  ...
  [ -f "$PENDING" ]
  run jq -e 'select(.event == "start" and .focus == "cp2-step-1-step-generalist")' "$PENDING"
  [ "$status" -eq 0 ]
}

# 2. start+complete pair clears pending
@test "start+complete pair clears pending ledger" {
  bash "$SCRIPT" start --focus cp2-step-1-step-generalist --agent-id aid-orchestrator:verifier --evidence-dir "$EVID"
  touch "${TMPDIR_TEST}/out.md"

  run bash "$SCRIPT" complete --focus cp2-step-1-step-generalist --output-file "${TMPDIR_TEST}/out.md" --evidence-dir "$EVID"
  [ "$status" -eq 0 ]
...
@test "complete without matching start exits 2 (orphan)" {
  ...
  bash "$SCRIPT" start --focus cp2-step-9-step-generalist --agent-id aid-orchestrator:verifier --evidence-dir "$EVID"
...
@test "expected-duration-max clamped at 1800s ceiling" {
  run bash "$SCRIPT" start --focus cp2-step-1-step-generalist --agent-id aid-orchestrator:verifier --evidence-dir "$EVID" --expected-duration-max 9999
  [ "$status" -eq 0 ]
...
@test "concurrent starts serialize without corruption" {
  bash "$SCRIPT" start --focus cp2-step-1-step-generalist --agent-id aid-orchestrator:verifier --evidence-dir "$EVID" &
  bash "$SCRIPT" start --focus cp2-step-2-step-generalist --agent-id aid-orchestrator:verifier --evidence-dir "$EVID" &
  wait
...
@test "complete with missing output_file exits 2 and keeps ledger" {
  ...
  run bash "$SCRIPT" complete --focus cp2-step-1-step-generalist --output-file "/tmp/does-not-exist-$$.md" --evidence-dir "$EVID"
  [ "$status" -eq 2 ]
...
@test "malformed agent_id exits 1 and creates no pending file" {
  run bash "$SCRIPT" start --focus cp2-step-1-step-generalist --agent-id FOO_bar --evidence-dir "$EVID"
  [ "$status" -eq 1 ]
...
@test "concurrent start + complete survives inode rotation" {
  ...
  bash "$SCRIPT" start --focus cp2-step-1-step-generalist --agent-id aid-orchestrator:verifier --evidence-dir "$EVID"
  bash "$SCRIPT" start --focus cp2-step-2-step-generalist --agent-id aid-orchestrator:verifier --evidence-dir "$EVID" &
  bash "$SCRIPT" complete --focus cp2-step-1-step-generalist --output-file "${TMPDIR_TEST}/out.md" --evidence-dir "$EVID" &
  wait
...
@test "P045 existing --focus cp2-step-3-step-generalist still validates (status 0)" {
  run bash "$SCRIPT" start --focus cp2-step-3-step-generalist --agent-id aid-orchestrator:verifier --evidence-dir "$EVID"
  [ "$status" -eq 0 ]
}
