...
@test "AC5: after an ABORT, plan-state agrees with the authoritative state file" {
  ...
  # The authoritative file and the reporter must not disagree. `plan-state`
  # reads the runtime manifest's mirror, so a path that transitions without
  # mirroring leaves every reader believing the plan is still awaiting the PM.
  run plan_state_get "$PLAN_ID" "plan_state"
  [ "$output" = "ABORTED" ]
  run bash "$PLAN_FSM_CLI" plan-state "$PLAN_ID" --project-root "$TEST_PROJECT_ROOT"
  [ "$status" -eq 0 ]
