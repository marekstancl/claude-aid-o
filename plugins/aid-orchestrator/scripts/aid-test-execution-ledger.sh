...
#   duplication. So an append that cannot take its lock FAILS rather than being
#   skipped, and a dispatch point running outside a gate run (a developer
#   invoking run-all-tests.sh by hand) finds no ledger path and appends
#   nothing — which is not an error, because there is no run to account for.
#
...
cmd_append() {
  local path="${AID_EXECUTION_LEDGER:-}" unit="" gate="" fp="" point=""
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --path)        path="$2"; shift 2 ;;
  ...
  # No ledger means no gate run is accounting for this dispatch. A developer
  # running a suite by hand is not a defect.
  [[ -n "$path" ]] || return 0
  [[ -f "$path" ]] || return 0
  ...
  if jq -c --arg u "$unit" --arg g "$gate" --arg f "$fp" --arg p "$point" \
       --arg t "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
       '.entries += [{run_unit_id:$u, gate_id:$g, command_fingerprint:$f, dispatch_point:$p, at:$t}]' \
...
cmd_close() {
  ...
  jq -c --argjson contains "$contains" --arg t "$(date -u +%Y-%m-%dT%H:%M:%SZ)" '
    ...
    | ( [ $units[] as $u
          | ( [ $e[] | select(.run_unit_id == $u) | .gate_id ] | unique ) as $gates
          | select(($gates | length) > 1)
          | {run_unit_id: $u, gate_ids: $gates} ] ) as $dups
    | .closed_at = $t
    | .summary = {dispatched: ($e | length), distinct_units: ($units | length), duplicates: $dups}
  ...
  echo "aid-test-execution-ledger: dispatched=$(jq -r '.summary.dispatched' "$path") distinct=$(jq -r '.summary.distinct_units' "$path") duplicates=${n_dup}"
