...
**UI Change Contract:** `path: .aid-o/work/.../delta-contract.json | sha256: <hash> | schema_version: 1.0.0 | viewports: desktop, mobile` *(REQUIRED when UI Change Mode is `existing_ui` — omitting this blocks controller dispatch; see plan.schema.json. `viewports` names every viewport the baseline and the verification cover: desktop AND mobile when `project.yaml → ui.responsive` is true or absent — P087 — desktop alone when it is `false`; a step without a capture for each named viewport does not pass its visual check.)*
```
