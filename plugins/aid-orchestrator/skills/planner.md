...
> **Script-Assisted Implementation:** As of Plan P018, deterministic plan generation
> is handled by `plugins/aid-orchestrator/scripts/aid-epic-to-json.sh`. This script
> reads an EPIC file and produces the plan.json + plan_progress.json artifacts,
> implementing the algorithms defined in this skill. The LLM invokes the script via
> `/aid-plan-epic` (Step 4); it does not perform inline JSON generation. The algorithms
> in this skill document the authoritative specification that the script implements.

**Input:** EPIC file (role + objective + depends_on per step)
**Output:** Plan JSON conforming to `plan.schema.json` (steps with model + context_scope, dependencies, parallel_groups, analysis_groups, gates, budget); generated by `aid-epic-to-json.sh`
...
- `plugins/aid-orchestrator/scripts/aid-epic-to-json.sh` -- script that implements the plan.json generation algorithms defined in this skill
- `plugins/aid-orchestrator/scripts/aid-auto-pipeline.sh` -- orchestrator that chains all pipeline scripts (Plan → EPIC → plan.json → run.md → queue)
