...
4. **project.yaml** (`.aid-o/config/project.yaml`) — paths, conventions, tech stack (generated by `/aid-setup`)
...
If Qdrant MCP is enabled (`defaults/integrations.yaml` → `memory.enabled: true`):

After loading file-based context (steps 1-5 above), search vector memory for relevant past knowledge:

```
mem_config = read("defaults/integrations.yaml")
...
If knowledge acquisition is enabled (`defaults/integrations.yaml` → `knowledge.enabled: true`):
...
Docs MUST be in SAME commit as code. For role-specific documentation rules, see `skills/role-cards.md`.
...
AID Orchestrator provides 7 agents (loaded on demand via Read tool, not registered in plugin.json):
...
| `verifier` | Code review + security review — dispatched with focus card from `role-cards.md` |

**Parametric Agents (2):** `implementer` (dispatched with role card from `role-cards.md`), `project-scanner` (project analysis + memory scan).

**Specialist Agents (2):** `curator` (improvement proposals), `auditor` (post-EPIC audit + memory health).

**How to invoke:** Use Agent tool with `subagent_type` matching the agent name.
...
- **Run-End:** docs update + final quality gates + ask PM for PR/merge + archive
...
| Skill | When to Load |
|-------|--------------|
| `skills/pipeline.md` | EPIC execution (Controller state machine, dispatch, gates, DONE) |
| `skills/agent-protocol.md` | Agent I/O format (input/output contract) |
| `skills/role-cards.md` | Role identity + constraints for implementer/verifier dispatch |
| `skills/run-management.md` | Run start/end, handoffs, epics |
| `skills/quality-gates.md` | Pre-commit 6-gate protocol |
| `skills/planner.md` | Plan generation from EPIC |
| `skills/improvement-proposals.md` | Curator improvement notes format |
| `skills/memory-mcp.md` | Qdrant vector memory (store/find protocol) |
| `skills/memory.md` | File-based project context (project.yaml, active.md) |
| `skills/brainstorming.md` | Interactive 9-step brainstorm flow |
| `skills/plan-writing.md` | Plan document authoring |
| `skills/visual-companion.md` | Browser-based prototype viewer |
| `skills/token-estimator.md` | Token budget estimation |
| `skills/analytics.md` | Orchestration metrics |
...
Project configuration is stored in `.aid-o/config/project.yaml` (generated by `/aid-setup`).
...
.aid-o/
  ...
  config/                            # PM-customizable
    ...
    # (playbooks consolidated into skills/role-cards.md in v2)
...
- Epic ID: `E-{NNN}-{phase}_{total}` or `E-{NNN}` for ad-hoc (see `skills/pipeline.md` ID Generation)
