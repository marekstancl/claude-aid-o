...
| `code-reviewer` | After major steps — reviews against plan + coding standards; GATES step acceptance |
...
**Role Agents (9):** `architect`, `domain`, `backend`, `frontend`, `qa`, `security`, `observability`, `docs-writer`, `release` — dispatched by Controller during EPIC execution per `skills/pipeline.md`.
...
| `skills/pipeline.md` | EPIC execution (Controller state machine) |
