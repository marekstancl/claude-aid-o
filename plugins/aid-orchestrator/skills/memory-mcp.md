...
This skill defines how AID stores and retrieves knowledge across runs using a Qdrant
MCP server. Agents auto-index decisions, lessons, patterns, audit findings, and documentation
at run-end, EPIC completion, and research triggers. Before dispatching an agent, the
...
**Source:** Run log entries, `decisions.yaml`, architect agent outputs, ADRs
**Trigger:** Run-end, EPIC DONE
**Metadata schema:**
...
```json
{
  ...
  "alternatives_considered": ["run cookies", "OAuth2 only"]
}
```
...
**Source:** Architect agent outputs, PM-requested proposals, brainstorming runs
**Trigger:** Explicit proposal generation (not auto-indexed at run-end)
**Metadata schema:**
...
(agent-core, run-management, run-epic) use these — never call `qdrant-store`
...
### `memory_index_run(run_file_path)`

Index knowledge from a completed run. Called by `run-end.md`.

```
mem = check_memory_enabled()
IF NOT mem.enabled OR NOT mem.config.memory.auto_index.run_end:
  return

run = read(run_file_path)

# 1. Index decisions from run log
decisions = extract_decisions(run.ai_run_log)
...
new_lessons = read_new_entries(".aid-o/04-engine/lessons-learned.md", since=run.started)
...
new_commands = read_new_entries(".aid-o/04-engine/command-history.md", since=run.started)
...
log_memory_event("index_run", run.id, "success",
...
context = "## MEMORY CONTEXT (from past runs)\n\n"
context += "_The following knowledge was retrieved from past runs. "
...
FOR EACH r IN unique:
  ...
  IF r.metadata.run_id:
    context += "_Source: " + r.metadata.run_id + "_\n"
...
IF qdrant-store or qdrant-find tool call fails:
  ...
  → NEVER block any workflow (run-end, EPIC execution, agent dispatch)
  → NEVER retry memory operations (unlike Slack which retries 3x)
    Rationale: memory is supplementary, not critical-path
  → Log once per run: "Qdrant MCP unavailable — proceeding with file-based memory only"
```
...
{"ts": "2026-02-17T10:01:00Z", "action": "index_run", "run_id": "R-20260217-2eec", "status": "success", "counts": {"decisions": 3, "lessons": 2, "commands": 5}}
...
- `skills/run-management.md` — Run completion protocol (consumer: run-end indexing)
...
- `skills/run-management.md` — Run-end protocol (consumer: run-end indexing)
...
- `agents/lessons-extractor.md` — Produces lessons + commands (indexed by memory_index_run)
...
- All memory operations are **non-blocking**. A Qdrant failure must NEVER interrupt
  run completion, EPIC execution, or agent dispatch.
...
- Memory context injected into agent prompts is clearly labeled as
  `## MEMORY CONTEXT (from past runs)` so agents know it's historical reference,
