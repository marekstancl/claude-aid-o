...
### Added
- **Source-aware resource map (`aid-test-resource-map.sh`)** — replaces grep-shaped parallel-safety reasoning with a map that follows `source`/`.`/`load` directives into the files where the evidence actually lives and cites `file:line` for every resource it records. Because only private units may share a pool downstream, every rule resolves toward `unknown` rather than toward private: an explicit `git -C`/`GIT_DIR` selector beats the working directory, an absolute or `$VAR`-anchored path ignores it, an unresolvable or conditional `cd` makes it unknown, and a helper that exists but was never read caps the whole unit.
- **Parallel pilot (`aid-test-parallel-pilot.sh`)** — runs a candidate lane serially and then concurrently in a disposable clone bound to the audited tree, comparing exit codes, terminal job states and the per-case result sequence position by position. Each side of each repetition runs in its own fresh snapshot, and leak detection digests the whole tree including files git ignores. A failing repetition ends the pilot with the index that failed and no retry.
- **Catalog `parallel.provenance`** — a non-`unknown` `parallel.status` must now carry where it came from, when, by what method, and against which source content. Every consumer resolves it through `aid_test_catalog_provenance_effective_status`, which applies a two-tier reversion rule: a comment edit keeps the status and refreshes the hash, a change to the unit's resources reverts it to `unknown`.
- **Lanes in the decision artifact** — resource maps and pilot receipts become `proposed_parallel`, `keep_serial`, `blocked_pending_fix` or `context_required` entries, plus a `smallest_safe_pilot` naming the one bounded run that would settle the most. Lanes are proposals; nothing writes the catalog or a scheduler mode.

### Changed
- **Parallel-pool eligibility has one authority** — `aid-bats-parallel-lane.sh` now derives its pool from the catalog's effective `parallel.status` instead of a separate text allowlist, and the allowlist file is retired to a notice pointing at the catalog. Two authorities over one question is how a promotion outlives its evidence: a plain list cannot notice that a file it names has since acquired a lock, and the catalog can. P071 Step 3's evidence is migrated with `method: migrated_p071_step3` rather than discarded, but a file changed since P071 verified it is left `unknown` and named rather than re-blessed against content P071 never saw.
- **`aid-audit-tests-finalize.sh`** — also passes `--resource-maps-dir` and `--pilots-dir` through to the consolidator, defaulting to their conventional locations under the audit output directory.
