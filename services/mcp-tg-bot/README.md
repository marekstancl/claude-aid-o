...
- **HTTP transport** — for bash callers (FSM scripts) on `localhost:8817`.
  Endpoints:
...
This service **replaces** the legacy `svc-mcp-telegram` MCP that previously
held port 8817. Decommission the old service first:

```bash
# 0. Stop + remove legacy MCP (port 8817 takeover)
cd /opt/eco/services
sudo docker compose stop mcp-telegram
sudo docker compose rm -f mcp-telegram
$EDITOR /opt/eco/services/docker-compose.yml   # remove the mcp-telegram: block
...
# 5. Update ~/.claude/.mcp.json — remove legacy `shared-telegram` entry,
#    add the new `svc-mcp-tg-bot` entry per the snippet below. Restart CC.
```
...
`http://localhost:8817/send_message`. Best-effort: if the service is
...
docker run --rm -e MCP_HTTP_HOST=0.0.0.0 -p 127.0.0.1:8829:8817 svc-mcp-tg-bot:dev
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| Container restart loop, `unhealthy` | Port 8817 already in use | `docker ps --filter expose=8817` to find conflict; renumber via `MCP_HTTP_PORT` env or stop the other service. |
