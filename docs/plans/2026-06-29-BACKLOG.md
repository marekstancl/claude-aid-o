...
Project-internal backlog items. Ecosystem-shared items live in `/opt/eco/services/BACKLOG.md`.
...
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed live: no code anywhere compares `--common-dir` against the source repo before a dogfood run, so a dogfood in a linked worktree can still advance the real `main` (the plugin runs its own plans in linked worktrees since P074, e.g. `.aid-worktrees/plan-P080`). Sieve reason: not selected for P083 — no recurrence since the 2026-07-27 incident and no live pipeline path breaks today; the fix (explicit dogfood marker + common-dir refusal) stays open for its own plan.
**Priority:** high
...
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed live in WAN's `execution.yaml:302-305` (`docs_updated: command: "true < /dev/null"`, `required: false`); this repo's own `docs_updated` gate is unaffected. No mechanism anywhere inspects whether a gate command can fail, so any project can fabricate the same green result. Sieve reason: not selected for P083 — a consumer-project misconfiguration, not a defect on the pipeline AID runs today; needs its own gate-command-content design.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — `aid-release.sh:334-350` `_release_probe_first()` does the exit-code triage so a zero-match probe is not an error; all three call sites converted (CHANGELOG detection :378, optional pyproject :403, update_changelog's header probe :485); `test-release-detection.bats` covers all four listed cases plus two review-driven ones. Shipped by P073 (`1f7cea5`), NOT by P079's tag-seal guard — that is a different control.
**Original status:** ready — directly reproduced on `main` at v2.66.2
**Priority:** high
...
### NEW — IMP-469: standalone parallel test runner — parallelism for every caller, not only the gate runner

**Status:** REJECTED — cancelled by PM (2026-08-09) together with the entire
test-parallelism line. P077 brainstorm (2 Codex rounds, see
`.aid-o/work/interim-P077.md`) surfaced the economics: measured speedup 3–6 %
(5–9 min on a 2.5 h run), qualification cost ~15 h compute per HEAD, evidence
invalidated by every commit. PM decided to remove parallelism from the audit
and the code entirely (separate session, different agent). Authoritative
writeup remains `docs/plans/2026-08-02-IMP-AID-ENTRYPOINT-UX-HELP-INIT-SETUP-HANDOFFS.md`
§18, now marked cancelled.
**Priority:** ~~high~~ n/a (rejected)
**Class:** test execution / developer experience

**Summary:** The P069 parallel dispatch path lives inside `aid-run-gates.sh`,
so only an AID pipeline run can use the proven-safe lanes. The PM wants the
same speedup available to ANY caller: an agent running suites by hand, CI, a
developer terminal. The evidence already exists (approved catalog,
`parallel.status: safe` lanes) — only the entry point is missing.

**Proposed change:** a self-contained `aid-test-parallel.sh <profile|gate...>`
that reads the APPROVED catalog (never the proposed one), partitions the safe
lanes across N workers, runs unsafe/unknown units sequentially after, and
exits nonzero on any failure — same rollout discipline as the gate runner
(refuses to parallelize without an approved catalog; `--observe` mode compares
against a sequential rerun of failures). CI usage documented in
`docs/extending-aid.md`. Enforcement per AID-v3 §1: the runner refuses
unproven lanes at execution time, not by prose.

---

### NEW — IMP-470: the audit must notice a red CI streak

**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed: `aid-test-content-scan.sh` check 11 is a static workflow-file scan only; no reader of `gh run list`/CI conclusions exists anywhere in the plugin, and a live `gh run list` showed the CI workflow queued 50+ minutes on the last three pushes with nothing surfacing it. Sieve reason: not selected for P083 — belongs to the test-portfolio audit machinery whose fate is decided separately (see IMP-492); adding a collector to a component being sized for removal is the wrong sequencing.
**Priority:** high
**Class:** audit coverage / CI health

**Summary:** This repo's own CI on main was red for 3 days (hanging
plan-final/plan-boundary suites + one real AC failure) and neither a human nor
the test audit noticed. v2.78.0 added "does CI run any tests?" but nothing
checks whether CI has been PASSING — a permanently red CI is strictly worse
than a disabled one, because it trains everyone to ignore it.

**Proposed change:** content-scan collector reads recent CI run conclusions
(`gh run list` when available; degrade honestly to "not checkable" without gh)
and reports the current red streak length + first failing commit; report shows
it as a critical row when the streak exceeds N runs. Enforcement per AID-v3 §1:
surfaced as a critical report row + proposed action, and the audit's own
verdict cannot be "clean" while the streak stands.

---

<!-- Numbering note: P076 allocated IMP-484..480 (plus IMP-489, added at its CP2
     fix loop and deliberately NOT cross-referenced from §16 — it is a release-tooling
     defect found by the plan, not one of the plan's five deferrals), skipping 471-475. Those five are
     taken by concurrent P077/P078 work that was still uncommitted in the primary
     checkout when this branch was written, so they are absent from the branch's own
     copy of this file and the gap reads as an error to anyone reading the branch
     alone. It is not one — the numbers were deliberately allocated above them to
     avoid a silent collision between two sessions editing the same tracked file. -->

### NEW — IMP-484: fire-and-return ASYNC gates — a runner that returns before the job does

**Status:** **CLOSED — deliberate, not a defect** (verified 2026-08-11 against v2.83.1) — confirmed shipped exactly as designed: `plugins/aid-orchestrator/scripts/aid-run-gates.sh:286-323` accepts only `foreground`/`background` (rejects `async` by name); background gates are supervised, resumable and synchronous via the poll-to-completion loop, now at `:1201-1204` (entry's cited `:1163`/`:1166` have shifted). A registered, fixture-proven ASYNC collector remains a recorded future precondition (AID-v3 par1), not a defect.
**Priority:** medium
**Class:** gate execution / auto-mode ownership
**Area:** `aid-run-gates.sh` background gate path

**Shipped instead:** supervised-resumable-**synchronous** background gates. A gate
declaring `run_mode: background` is launched through `aid-job.sh` (owned process
group, durable terminal receipt, crash re-attach by command fingerprint), but the
runner then polls that job to completion inside the same invocation and returns the
gate row itself. The "background" win is survivability and resumability, not the
runner returning early.

**Why deferred:** returning before completion needs a second, independent
continuation owner — something must collect the terminal receipt and advance the
FSM when the runner is already gone. P076 ships exactly one honest continuation
path (`auto_resume_required.json` + `aid-fsm.sh resume`), and that path is
crash-recovery, not a scheduler. Building fire-and-return on top of it would have
meant inventing an unproven dispatcher inside a plan whose whole point was to stop
pretending about waits.

**Hook point:** `plugins/aid-orchestrator/scripts/aid-run-gates.sh:1163` — the
comment `# ── poll to completion, inside THIS invocation ──`, immediately above the
poll loop at `:1166` (`while true; do`). That loop is the mechanism an async mode
would replace with a return-plus-handoff.

**Proposed change:** an opt-in `run_mode: async` that returns a `dispatched` gate
row plus the job id, with the continuation contract (who collects, by when, what
happens on a lost collector) specified and fixture-proven BEFORE any caller may
declare it — per AID-v3 §1, the runner refuses `async` while no collector is
registered rather than documenting the requirement.

---

### NEW — IMP-485: services↔resource-map — teach the classifier about per-run service ports

**Status:** **CLOSED — moot** (verified 2026-08-11 against v2.83.1) — the resource-map classifier this entry asked to extend was deleted whole in P078/v2.82.0 (`plugins/aid-orchestrator/defaults/enforcement-registry.yaml:1523-1552`, `status: removed_scoped`; `scripts/aid-test-resource-map.sh` no longer exists), so the entry's subject no longer exists. The emitter it also asks about (`lib/aid-service.sh:967-976`) is intact and independently documented as useful evidence; keeping or dropping it is a one-line housekeeping call, not the deferral this entry described.
**Priority:** medium
**Class:** test-catalog / resource classification
**Area:** `lib/aid-service.sh`, resource-map classifier

**Shipped instead:** every service that reaches `healthy` appends one JSONL line to
`<evidence>/service-resources.jsonl` naming the resource it occupies
(`{kind: external_service, namespace: per-run, id: "<name>:<port>"}`). Nothing
consumes it for a decision.

**Why deferred:** the resource-map classifier feeds parallel-safety verdicts. Wiring
a brand-new emitter into it in the same plan that introduced the emitter would let an
unvalidated signal change a safety classification on its first run — and the
test-parallelism line was under PM review at the time, so the consumer's own future
is unsettled. (That review has since concluded: the PM cancelled the parallelism
line on 2026-08-09 in a separate session. At this commit that decision exists only
as uncommitted work in the primary checkout — a new §18 in the source document and
the IMP-469 rejection — so it is described here rather than cited, because a
citation to a section this branch does not carry would resolve to nothing.)

**Hook point:** `plugins/aid-orchestrator/scripts/lib/aid-service.sh:967` —
`_aid_svc_emit_resource() {`, the sole writer; the contract is stated at
`scripts/lib/aid-service.sh:104` (`# ── RESOURCE EVIDENCE (observe only) ──`, whose
body says the resource-map classifier is deliberately not touched).

**Proposed change:** once the parallelism decision settles, either (a) let the
classifier read the JSONL so a per-run port stops being scored as a shared resource,
or (b) delete the emitter as evidence nobody will ever consume. Do not leave an
observe-only signal in place indefinitely — an unconsumed detector is decoration.

---

### NEW — IMP-486: foreground gates get no `timeout -k` — a SIGTERM-ignoring suite still hangs

**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed and reproduced: `plugins/aid-orchestrator/scripts/aid-run-gates.sh:232`'s `timeout "$timeout_s" bash -c "$command"` has no `-k`, so a gate that traps/ignores SIGTERM runs to its own completion past its deadline (reproduced: a 2s timeout on a SIGTERM-trapping 6s sleep returned only after 6s, with the child's own output). Sieve reason: not selected for P083 — real and cheap (S), but changes kill semantics for every gate in this repo and every consumer project; deferred to its own change with a reaping bats fixture.
**Priority:** low
**Class:** gate execution / hang containment
**Area:** `aid-run-gates.sh` foreground gate path

**Shipped instead:** nothing on this path, deliberately. P076 added a background
path beside the foreground one and left the foreground execution byte-identical, so
that every gate not declaring `run_mode: background` behaves exactly as it did
before the plan — the plan's blast radius is the opt-in path only.

**Why deferred:** `timeout` without `-k` sends SIGTERM and gives up; a child that
traps or ignores SIGTERM (a node test server, a wedged docker exec) survives its own
deadline. Adding `-k` changes the kill semantics of EVERY existing gate in the repo
and every consumer project at once, which is a separate, separately-evidenced change.

**Hook point:** `plugins/aid-orchestrator/scripts/aid-run-gates.sh:196` —
`output=$(LC_ALL=C timeout "$timeout_s" bash -c "$command" </dev/null 2>&1) || exit_code=$?`
(no `-k`, no `--foreground`, no process group).

**Proposed change:** `timeout -k <grace> <deadline>` with the grace value read from
the same configuration the background path already uses
(`AID_GATE_DEADLINE_GRACE_SEC`), landed with a fixture that proves a SIGTERM-ignoring
child is actually reaped — and rolled out as its own change, not folded into an
unrelated plan.

---

### NEW — IMP-487: migrate the visual-companion server onto `lib/aid-service.sh`

**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed: `plugins/aid-orchestrator/lib/brainstorm-server/start-server.sh:100-104` still starts via unowned `nohup ... & disown`, the same shape `aid-service.sh`'s `_svc_backgrounding_form` would refuse for a declared service; no migration to `aid-service.sh` exists. Two corrections to the original rationale: the server already picks a random per-session port, and a `--foreground` mode already exists. The real remaining gap is ownership (a crash leaves an orphan node process nothing sweeps). Sieve reason: its own plan (P083 explicitly excludes it, plan lines 42/522).
**Priority:** medium
**Class:** service ownership / auto-mode cleanup
**Area:** `lib/brainstorm-server/start-server.sh`, `skills/visual-companion/SKILL.md`

**Shipped instead:** `scripts/lib/aid-service.sh` — declared services over
`aid-job.sh`, with health probes, per-run ports, acquire-once/release-once lifecycle
and a swept teardown on `resume`/`done-advance`. The visual-companion server was not
migrated onto it.

**Why deferred:** the companion server is a PM-interactive brainstorm tool with its
own start/stop contract and its own `--host`/`--url-host` reachability handling; it
is started by a human-facing skill, not by a run. Converting it needs a decision
about what "the run" even is for a brainstorm session, which the plan did not take.

**Hook point:** `plugins/aid-orchestrator/lib/brainstorm-server/start-server.sh:100`
— `nohup env BRAINSTORM_DIR=... node index.js > "$LOG_FILE" 2>&1 &` followed by
`disown` at `:102`: the exact unowned-detach shape that `aid-service.sh` exists to
replace (and that its own `_svc_backgrounding_form` check rejects in a declared
service's `start_cmd`, `scripts/aid-run-gates.sh:381`).

**Proposed change:** declare the companion as a service with a health probe and a
per-run port so a crashed brainstorm session cannot leave an orphan node process
holding a port; keep the `--url-host` reachability behaviour, which the generic
service layer has no notion of.

---

### NEW — IMP-488: host push-continuation adapter — make task-notification resume a verified mechanism

**Status:** **CLOSED — deliberate, not a defect** (verified 2026-08-11 against v2.83.1) — confirmed shipped: the artifact contract (`plugins/aid-orchestrator/commands/aid-run.md:108-115`) and its enforcement (`lib/aid-resume-artifact.sh:32`, writer at `aid-run-gates.sh:1173-1196`) work without a host push adapter; `awaiting_host_resume` is documented as derived, never stored. The missing piece is a host-specific push notification adapter, deferred with an explicit AID-v3 par1 rationale (no controlled delivery surface exists to test against) — a recorded intentional omission plus a feature request, not a defect.
**Priority:** high
**Class:** auto-mode continuation / host capability
**Area:** `commands/aid-run.md`, `skills/pipeline.md`, `lib/aid-resume-artifact.sh`

**Shipped instead:** the enforced fallback — a continuation pointer
(`<evidence_dir>/auto_resume_required.json`) written BEFORE any background job
spawns and deleted only on a clean terminal collect, a single-use-claimed
`aid-fsm.sh resume`, derived `awaiting_host_resume` (never stored, because a dying
controller cannot write), and an honest host card. What a capable host *should* do
with a task notification remains prose addressed to the agent.

**Why deferred:** a push adapter is host-specific and cannot be proven from inside
this repository: AID has no controlled surface that delivers a notification, so any
adapter shipped here would be a detector with no test — precisely the failure mode
AID-v3 §1 names. The artifact path was chosen because it is verifiable without a
host.

**Hook point:** `plugins/aid-orchestrator/commands/aid-run.md:108` — "*Do not wait
indefinitely for a missing notification. "Resume" is a named mechanical path, not a
judgement call*" — the instruction-only sentence an adapter would turn into a
mechanism; the mechanism side it would extend is the artifact writer at
`plugins/aid-orchestrator/scripts/lib/aid-resume-artifact.sh:32`
(`AID_RESUME_ARTIFACT_BASENAME="auto_resume_required.json"`).

**Proposed change:** a declared host-capability adapter with three states (absent /
present-untested / verified), a simulated-notification fixture that proves the
controller resumes from the recorded FSM boundary exactly once, and a status surface
that keeps saying `awaiting_host_resume` for as long as the adapter is not verified.
No adapter, no behaviour change — the fallback stays the enforced path.

---

### NEW — IMP-489: a non-numeric CHANGELOG heading makes `aid-release.sh` rename the LAST SHIPPED release

> **Duplicate notice (2026-08-10):** a concurrent session registered this same
> `aid-release.sh` defect as **IMP-482** in the primary checkout while this branch was
> open. IMP-482 is the canonical entry; this one was renumbered from 481 to 489 to
> resolve the collision, and is kept only so this branch's own cross-references resolve.
> Close it against IMP-482 rather than working it twice.

**Status:** **DONE (as its canonical twin IMP-482)** — the tag-seal guard shipped in P079/v2.81.0 (`aid-release.sh` `_release_version_sealed`, 4 occurrences: definition plus both retitle sites plus the fail-closed non-git path). Nothing remains under this id; kept only so this branch's cross-references resolve.
**Original status:** deferred by P076 (2026-08-10) — found live at CP2, warned in-file, not fixed
**Priority:** high
**Class:** release tooling / IMP-093 regression class
**Area:** `plugins/aid-orchestrator/scripts/aid-release.sh`, `CHANGELOG.md`, `plugins/aid-orchestrator/CHANGELOG.md`

**Shipped instead:** a warning comment in BOTH CHANGELOGs, immediately under the
pending `## [Unreleased]` heading, stating exactly what the script would do and that
the version must be set by hand first. The two files stay byte-identical and that
identity is asserted by `test-p076-docs-closure.bats` case 2. A warning is not an
enforcement — this entry exists because the mechanism side is still missing.

**Why deferred:** the fix belongs in `aid-release.sh`, which is outside P076's scope
and whose change needs its own review; a release script edited from a CP2 fix loop,
with no release-path test run, is exactly the sort of change that produces the next
incident. Recorded as work rather than folded in quietly.

**Hook point:** `plugins/aid-orchestrator/scripts/aid-release.sh:341` — the probe's
`grep -m1 -oP '## \[\K[0-9]+\.[0-9]+\.[0-9]+'`, which matches only NUMERIC headings
and therefore silently skips `## [Unreleased]`, returning the last released version
(`2.79.3`). That value equals `RELEASED_VERSION` read from `plugin.json`, so
`update_changelog` (`aid-release.sh:466`) takes its `header == CURRENT` branch and
`sed`-renames the SHIPPED `## [2.79.3]` heading into the new version: the released
entry's history is absorbed and the pending entry keeps its `[Unreleased]` heading.
The IMP-093 two-source guard cannot fire, because it compares against a heading the
probe never saw.

**Proposed change:** make the probe report the newest `## [` heading whatever its
form, and have the caller distinguish three cases — numeric-and-equal (rename),
numeric-and-different (prepend, today's IMP-093 path), and NON-NUMERIC (a pending
heading: rename THAT heading, and never a numeric one below it). A non-numeric
newest heading must never fall through to the rename branch of an older release.
Regression test: a fixture CHANGELOG with `## [Unreleased]` above the released
heading, asserting the released heading is byte-unchanged after a bump.

**DEMONSTRATED, not reasoned (CP3 fix loop, 2026-08-10).** `aid-release.sh patch`
was driven end to end against a throwaway git repository copied from this plan
worktree — same two CHANGELOGs, same `plugin.json` at `2.79.3`, and, like the
worktree, NO `.aid-o/config/project.yaml`. Result: `Bumping: 2.79.3 → 2.79.4`, then
`Updated: CHANGELOG.md (header 2.79.3 → 2.79.4)`. Afterwards `grep -c '^## \[2\.79\.3\]'`
returns **0** in both files and `## [Unreleased]` is still there, untouched, above
them. The shipped release's entry is not merely renamed — it is the entry the new
version now claims as its own.

**One consequence to correct, because it is the reverse of what a reviewer expected.**
Without `project.yaml` the `versioning.files[]` loop (`aid-release.sh:557`) never
runs, so it is tempting to conclude the plugin CHANGELOG is MISSED and byte-identity
breaks. It is not, and it does not: the no-config fallback (`aid-release.sh:573-581`)
`find`s every other `CHANGELOG.md` and applies the SAME `sed` rename, and the live run
above reports `Updated: plugins/aid-orchestrator/CHANGELOG.md (changelog)` with `cmp`
still clean afterwards. **That is worse, not better.** Both files are corrupted
identically, so `test-p076-docs-closure.bats` case 2 — which compares them only to each
other — is silent by construction. The only shipped assertion that would notice is case
4 (P076's entry still names `run_mode`, `auto_resume_required.json`, …), and it can only
notice after the release commit exists. There is no pre-release detector at all.

**Hook point (second):** `plugins/aid-orchestrator/scripts/aid-release.sh:573-581` —
the no-config fallback's blind `sed -i "s/## \[$CURRENT\].*/…/"`, which carries no
prepend awareness and no `[Unreleased]` awareness of its own. Fixing only
`_release_probe_first` leaves this path intact.
...
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed: Codex model/effort configuration for both C0 and C3 is environment-variable-only (`aid-c3-dispatch.sh:142`, `aid-c0-plan-review.sh:118-120`), with `model_reasoning_effort=high` hard-coded at the shared transport (`aid-c3-dispatch.sh:1112-1113`); no tracked project surface exists, and live P080 evidence shows C0 ran on the wrong model with the effort unrecorded. Sieve reason: its own plan (P083 explicitly excludes it, plan lines 42/522 — analysis-first scope, entangled with the not-yet-merged P080 entrypoint work).
**Priority:** high
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — `aid-job.sh` ships the full supervisor (process-group ownership, PID-reuse defeat via /proc starttime + cookie, atomic job/result, revision-bound results) and P076 wired AUTO liveness (pipeline.md watchdog step, JOB_LOST ladder route, run_mode background gates).
**Original status:** ready — implement manually outside AID after P064, before P068
**Priority:** critical
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-fsm.sh:5483-5501 binds increment on idempotency_token/step_index/step_id/plan_step_hash verified against the live plan; :5557-5598 idempotent short-circuit; :5801-5826 rejects partial binding.
**Original status:** ready — implement manually outside AID after P064, before P068
**Priority:** critical
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — schemas mark head_sha authoritative and the stored booleans DEPRECATED/NON-AUTHORITATIVE; aid-protocol-validate.sh:335-347 recomputes and rejects disagreement; aid-pm-brief.sh:137-182 computes at read time.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — lib/aid-plan-manifest.sh:1000 defaults lineage to `unproven` and :1008-1010 accepts only proven|unproven (exit 1, nothing written); aid-plan-fsm.sh:9363 healthy repair is a non-destructive no-op.
**Original status:** ready
**Priority:** high
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — PM ratified Option B on 2026-07-24 (docs/plans/2026-07-24-IMP-266-...-CEREMONY.md): merged_to_plan stays terminal with a documented ceremony; code matches (aid-plan-manifest.sh:1058-1075) and the forbidden generic rollback was never added.
**Original status:** idea — architecture decision required
**Priority:** medium
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-plan-fsm.sh:8804/:8868 re-derive ancestry from real git before attesting and persist the re-derived value; fail-closed per enforcement-registry.yaml:1320.
**Original status:** ready
**Priority:** high
...
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed and worse than described: `aid-plan-manifest.sh add-epic` (`:1639`, `:997-1008`) is an undocumented third writer of `lineage: proven` via a hidden 8th positional, and `update` (`:1642`) is an unconstrained fourth writer that can flip an existing entry — both reproduced live. The code's own "only two sources" comment (`aid-plan-fsm.sh:8297-8298`, `:2693`) is false in two places. Sieve reason: not selected for P083 — real integrity gap (fix: drop the 8th positional, gate `update` against `.lineage`), but no live incident traced to it; kept in the backlog with its own S-effort fix.
**Priority:** medium
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-c3-dispatch.sh:555-633 classifies ac_source plan|final_report_fallback|stub, downgrades a bundle byte-identical to final_report.md, and build-manifest exits 1 when an AC lens is required but ac_source != plan; the targeted-run receipt half shipped with it.
**Original status:** ready
**Priority:** high
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-gate-waiver.sh (issue/check/consume, payload bound to gate/epic/run/head/command sha) + aid-run-gates.sh:2140-2176 renders a waived gate as `waived`, NEVER `pass`; aid-fsm.sh:3004-3051 re-validates per named gate against the report's own head_sha.
**Original status:** ready — implement manually outside AID after P064, before P068
**Priority:** critical
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-plan-fsm.sh:2063-2072 requires an explicit --mode before any root resolution; the plan-final compensating-control probe (_pfsm_plan_final_installed) self-lifted once P068 shipped.
**Original status:** ready — required before P068 enables `plan_branch`
**Priority:** high
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-fsm.sh:8300-8335 _dep_merge_target_authorized with a labelled contract twin in lib/aid-queue-write.sh:749 and a twin-parity test.
**Original status:** ready — required before P068 enables `plan_branch`
**Priority:** high
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-fsm.sh:196-224 routes every consumer through _fsm_declared_plan_mode (the committed-manifest authority); cmd_init's old direct reader that failed OPEN is rerouted (enforcement-registry.yaml:1395-1401).
**Original status:** ready — required before P068 enables `plan_branch`
**Priority:** high
...
**Status:** **CLOSED for the one production fail-open instance -- P083 Step 6; wider scope deliberately unscheduled** (verified 2026-08-11 against v2.83.1) — confirmed the worst finding: `aid-review-signals.sh:22-29`'s `_aid_read_toggle` used `grep -qP` twice, and on any grep without PCRE both calls exit 2, the surrounding `if` reads false, and the function fell through to "enabled" — the exact fail-open the no-`grep -oP` guard exists to prevent, on production library code the guard never scanned. P083 Step 6 (commit `b9de41c`, hardened by `85357ab`) rewrote the function in bash's own `[[ =~ ]]`/POSIX bracket classes with a third fail-closed return state wired to every call site (`aid-fsm.sh` fails closed; `aid-release-policy.sh` reports and blocks on a distinct `toggle_unreadable`), registered as `review_signal_toggle_fail_closed`. The detector's own blind spots are explicitly NOT fixed, by the PM's sieve: only 4 of 13 live PCRE call sites are visible to `_imp274_scan`, `.bats` files are never scanned, and the `aid-release.sh` allowlist grants 3 slots to a file with 0 real hits — that widening stays unscheduled, recorded here per P083's Deferred section.
**Priority:** medium
...
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed narrowed: P073's supersede-epic path (`aid-fsm.sh:4586-4647`, `aid-plan-fsm.sh:8363-8519`) closed the sanctioned re-init route with an audited archive+record. Two symptoms survive: (a) a hand `rm fsm-state.yaml` bypasses the supersede check entirely and leaves no archive, so a second `fsm_init` event is appended undetected; (b) `plan.json`/`epic_input.md` are still overwritten in place at regeneration even on the sanctioned path — only the state file is archived. Sieve reason: not selected for P083 — narrower than originally filed, no live incident since P073 shipped; the fix (refuse re-init on an unverified prior `fsm_init`; archive `plan.json` before overwrite) stays open.
**Severity:** medium
...
**AID version:** v2.50.1
**Observed at:** 2026-07-02
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed narrowed: the flat-vs-subdir duality the entry named cannot recur (`aid-auto-pipeline.sh:1940-1941`/`:2107-2108` pin run.md to `runs/<run_id>/`), but the same duality moved one directory level down — `aid-json-to-run.sh:616-636` writes `<run_id>-<goal_slug>.md` with no archival of a prior file, so a re-scope that changes the goal slug leaves two candidate files under one run_id. Sieve reason: not selected for P083 — the original WAN symptom is closed; the residual is a smaller S-effort follow-up (archive stale `<run_id>-*.md` before writing), not on this plan's four hot-path defects.
**Severity:** low
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — P060 fsm_check_cp3_freshness (aid-fsm.sh:1419-1580) requires Reviewed-Head on every CP3 output and defaults BLOCKING; invoked at GATES:DONE and review→release.
...
**AID version:** v2.50.1
**Observed at:** 2026-07-02
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed for the main claim: `final_report.md`, the DONE REVIEW PM Summary template (`skills/pipeline.md:2173-2210`) and `aid-pm-brief.sh` all carry no delivery-gate (D0) line, so a run with `delivery_ready: false` and unverifiable findings reads as all-green to the PM; `aid-release-policy.sh:51,194` now blocks on the artifact's *presence* but not its verdict. The entry's "freshness fields contradict each other" sub-claim is WRONG ADDRESS — `aid-delivery-gate.sh:197-210` documents `revision.freshness` and `delivery_gate.freshness` as two deliberately distinct axes, not a bug. Sieve reason: not selected for P083 — real, S-effort (one template line), but not on the four hot-path defects this plan repairs.
**Severity:** medium
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-run-gates.sh:2407-2432 emits result:fail reason:undefined_gate and forces overall=fail; FSM refuses advance-to-gates on the same condition; regression at test-aid-run-gates.bats:169/:200.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — defaults/hooks/pre-commit:17,180-184 enforces path scope only in DONE/review (union of all steps' allowed_paths ∪ evidence dir); --no-verify residue caught out-of-band as commit_scope_violation.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-fsm.sh:7027-7036 requires both reports at review→release; blocking_findings must be a top-level machine field (:6958-6978); C3 profiles additionally require audit-report.json + a curator ref whose sha256 matches.
...
**Observed at:** 2026-07-02
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed narrowed: generation is now transactional (manifest + two-phase receipt, `aid-auto-pipeline.sh:2093-2099,2153-2155`) and P074's `supersede-generation` is an audited, non-destructive recovery — but it explicitly does not clean up already-created artifacts, and the one pointer it prints (`plan-rollback`) does not do that job either (`aid-plan-fsm.sh:10102-10120` is merge-revert bookkeeping, not cleanup). Sieve reason: its own plan (P083 explicitly excludes it, plan lines 42/522); the fix (a `--cleanup` mode reading `generated-epics.json`) stays open.
**Severity:** medium
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — scripts/gates/aid-contract-validate.sh is a blocking D5 gate in generation (aid-auto-pipeline.sh:1795-1840, exit 4/5); aid-generation-finalize.sh:67-69 refuses a package unless every phase validates.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-plan-diff.sh:164 accepts both '## Acceptance Criteria' and '## Success Criteria'; null plan_path now an explicit fast-mode skip, and aid-json-to-run.sh:686 derives the plan path from plan.json.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — both artifact families are indexed by the same 0-based step (aid-fsm.sh:5609,:5705); a copied file cannot pass the binding (:5838-5856); _fsm_human_step renders the human number. Also closes B-007.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-prefilter.sh:105-147 resolves the cp2 range from the step boundary, else base_commit..HEAD, else exit 22 range_undetermined (blocking); the HEAD~1 fallback survives only under an explicit observe policy with a loud event.
...
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed: `aid-fsm.sh:6090-6119`'s `cmd_set_field` still has no `log_event`/timeline write of any kind (`fsm_field_change` has zero hits in the file); every `set-field` mutation (`pm_decision`, `escalation_decision`, `c3_recheck_count`, etc.) is untraceable against `timeline.jsonl`. Sieve reason: not selected for P083 — real S-effort gap, but no incident tied to it since filing; kept in the backlog with its fix (capture old value, `log_event ... fsm_field_change`).
**Severity:** medium
...
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed narrowed to `legacy_epic_release_mode`: under `plan_branch`, the plan-final boundary review now covers the WAN topology because it reviews `plan_base_commit..candidate_sha` as one range. Still missing everywhere: nothing computes which commits were inside a verified run, and nothing flags an AID-look-alike `task/*` branch with no run evidence; `legacy_epic_release_mode` keeps the original gap in full. Sieve reason: not selected for P083 — M-effort, no live incident since P068 shipped `plan_branch` as the guarded default.
**Severity:** medium
...
**Status:** **CLOSED by P083 (Step 1)** (verified 2026-08-11 against v2.83.1) — confirmed: the writer default and seven independent readers all agreed on `<evidence>/gates/gates_report.json`, but `fsm_check_streamlined_integration_review` (`aid-fsm.sh:1825`) alone read the flat `<evidence>/gates_report.json`, hard-blocking every streamlined-mode EPIC at review to release and training operators toward a force waiver. P083 Step 1 (commit `d05356d`) made the check read the canonical path first, accept the legacy flat path as a logged fallback, and name both paths searched on failure; covered by `test-streamlined-integration-review.bats`.
**Severity:** low
**Class:** UX indexing / command surface
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — defaults/hooks/pre-commit:20 blocks main during EXECUTE/GATES via the FSM-managed active-runs map, worktree-safe; P074 plan worktrees stop an EPIC sharing the primary checkout.
...
**Observed at:** 2026-07-08
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed: `fsm_check_grandfather` (`aid-fsm.sh:1184-1196`) is the only era mechanism and its `DEPLOY_DATE` threshold has been frozen at v2.18.0 for ~65 releases (absent from `aid-release.sh`'s version files), so it is effectively always false; the newest, strictest precondition surface (`aid-release-policy.sh check_required_present`) never consults it at all. The specific 2026-07-08 symptom is mitigated (the relevant hooks now default `enforcement: observe`), but the general claim -- no migration/grandfather policy for retroactive preconditions -- still holds. Sieve reason: not selected for P083 -- needs a policy decision before the fix, not just a config change.
**Severity:** medium
...
**Observed at:** 2026-07-08
**Status:** **CLOSED — already fixed** (verified 2026-08-11 against v2.83.1) — confirmed: `aid-audit-mode.sh:52-57` now fails closed on an absent `review-profile.json` (echoes `c3`, exit 3) instead of resolving to "not required"; `c3-audit-policy.yaml:26-32` requires C3 for `high`/`unverifiable`; the producer is wired at both EPIC and plan-range scope (`aid-prefilter.sh:351,668-727`). The remaining per-EPIC `enforcement: observe` staging is a documented, deliberate decision (`aid-fsm.sh:6708-6712`, tied to E10 promotion), not a gap; plan-final already blocks on a missing profile.
**Severity:** high
...
**Status:** **CLOSED — already fixed** (verified 2026-08-11 against v2.83.1, supersedes the prior v2.82.0 annotation) — confirmed: `aid-fsm.sh:5997-6032`'s `cmd_increment_step` now syncs `steps[].status = "completed"` + `completed_at` on every increment, backfills `started_at` only when null, guards legacy state without `steps[]`, and emits `step_status_synced`; the fix (recurrence #4's `0be5e6f`) is on `main`. One honest residual: the sync fires only inside `increment-step`, so a final step whose increment never ran would not get stamped -- no separate archival/DONE sweep exists to catch that case.
...
**AID version:** v2.52.0
**Observed at:** 2026-07-08
**Status:** **CLOSED — moot** (verified 2026-08-11 against v2.83.1) — confirmed: `shell_pipeline_smoke` (P081/v2.83.0) has left every merge-path profile's `include[]` and now runs only in the nightly workflow directly (`.github/workflows/nightly-tests.yml:62-64`), so it can no longer fail every EPIC under `overall: pass`. One narrow residual: if the FSM's auto-resolved profile name is undefined, it applies no include-whitelist and would run this 5-hour gate -- an edge case, not the normal path, and not observed live.
**Severity:** low
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — init resumes or auto-creates the EPIC branch (aid-fsm.sh:4790-4817) and every preflight refusal is now evented (fsm_init_blocked, fsm_branch_mismatch_detected).
...
**Status:** **DEAD** (verified 2026-08-11 against v2.82.0) — the live-usage probe is a concluded activity, not a standing process: no OBS entry has been filed since 2026-07-11 and its own exit criterion (open a dedicated cleanup plan) was met and consumed by P060 and the Pre-E10 hygiene block. The runbook survives as a document.
...
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed: `aid-prefilter.sh classify --checkpoint cp3` is an accepted, documented CLI surface that still writes CP2's `verifier-output-step-N.md` file with a `pending` stub, destroying a real CP2 verdict, though it is unreachable from any shipped caller and a cp3-produced stub now hard-blocks the next `increment-step` (`wrong_checkpoint_stub`) rather than false-greening. Sieve reason: not selected for P083 -- the PM's sieve prefers removal, and the fix (drop `cp3` from accepted `--checkpoint` values) is a clean S-effort deletion recorded here, subsuming B-008.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — plan mode is now mechanically resolved, not interpreted: since P068 the default is plan_branch whenever the project declares gate_profiles, recorded in the committed lifecycle manifest with a named fallback reason (skills/pipeline.md:2421-2431). CAVEAT: the SHIPPED defaults/execution.yaml has no gate_profiles table, so a fresh consumer silently falls back to legacy — tracked separately as an open item.
...
**Status:** **CLOSED — already fixed** (verified 2026-08-11 against v2.83.1) — confirmed: `_fsm_human_step` (`aid-fsm.sh:1128-1141`) renders human-facing step numbers, and the evidence-integrity harm is now mechanically blocked -- both `step-N-verify.md` and `verifier-output-step-N.md` share one 0-based `step` convention, and per-step binding (`aid-fsm.sh:5834-5856`) rejects a verify file copied from another step. Residual cosmetic-only gap: a few error strings and the status JSON still print the raw 0-based number with no human gloss; no correctness consequence remains.
...
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed: `aid-prefilter.sh:158-165`'s cp3 range branch silently guesses (`merge-base HEAD origin/main`, or the literal `HEAD~5` with no `origin/main`) instead of failing closed like the CP2 and `profile` range resolvers do. It is reachable only through the same dead `cp3` command mode B-005 describes, so it has no live exposure on the shipped pipeline. Sieve reason: not selected for P083 -- subsumed by B-005's fix (deleting the `cp3` branch removes this code path at zero extra cost); recorded together, one removal closes both.
...
**Status:** **CLOSED — already fixed** (verified 2026-08-11 against v2.83.1) — confirmed by running the suite: `test-semantic-review.sh` -> 25/25 passed. Both sides were repaired: the suite now prints the canonical `Results: N/T passed` format, and `run-all-tests.sh`'s `parse_suite_result` treats a missing/ambiguous result line as `unparsed`, failing the aggregate instead of silently counting zero (commit `d1c2f5f`, CHANGELOG.md:576). The class of bug (0/0 read as success) is now structurally impossible, not just patched for this one suite.
...
**Status:** **CLOSED — already fixed** (verified 2026-08-11 against v2.83.1) — confirmed by running the suite: `test-plan-to-epic.sh` -> 24/24 passed, 0 failed, same 24-test denominator the entry names (no test deleted to reach green). Both named assertions (`:688` etc.) still exist in the suite and simply no longer fire; most recent behavioural fix is commit `930fcd7` ("validate source plan before epic generation"). The entry's premise of permanent red no longer holds.
...
**Status:** **CLOSED — already fixed, shipped more than requested** (verified 2026-08-11 against v2.83.1) — confirmed: P039's Design Validation Protocol (`skills/brainstorming.md:265-332`) ships the validate-then-verify cycle, a non-trivial-section floor that only escalates up, an anti-hallucination ground-truth rule (RULE 10), and a 6-block Section Verdict Format including the `command_run` anti-hallucination column. Both open questions are answered, not deferred: enforcement is an explicit PM confirmation gate (AID-v3 par1 mechanism #3, deliberately not an FSM brake), and the audit trail is the in-message claim table, not a timeline/evidence event -- a conscious rejection of that alternative, stated in the skill itself.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — see OBS-20260702-08 — aid-contract-validate.sh is a blocking D5 gate in generation.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-run-gates.sh:232 runs every gate command with </dev/null and :2392-2402 asserts defined==processed, emitting gate_integrity_fail and forcing overall=fail.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — enforcement moved to the commit boundary: defaults/hooks/pre-commit:100-113 blocks any staged file outside the state-appropriate scope; commit_scope_violation is the out-of-band backstop.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — presence gates are live at done-advance (aid-fsm.sh:7027-7030) and stricter dual-emit + sequencing-ref checks apply for C3-required profiles (:7163-7197).
...
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed: the archival hook people assume exists does not -- the ~6894-6906 check in `aid-fsm.sh` is a read-only precondition (refuses to advance unless a human already `mv`'d the task file), and `runs_completed`/`status:` have no writer anywhere after `aid-plan-to-epic.sh`'s initial stamp; live drift confirmed on the newest archived EPIC (`E-076-3_3`: still `status: active`, `runs_completed: 0`). Sieve reason: not selected for P083 -- M-effort (deciding whether the FSM may move files in `.aid-o/tasks/` is a real design question), no live incident forcing it now.
**Severity:** medium
...
**Observed at:** 2026-07-08
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed unchanged: `aid-prefilter.sh` runs `git -C "$ROOT"` against exactly one repository by construction, and no code anywhere detects or flags a diff outside that repo -- a cross-repo EPIC's out-of-repo half is silently unreviewed by CP3, with no skip event. Sieve reason: not selected for P083 -- affects the VULCAN/eco-services cross-repo pattern, not AID's own pipeline; the S-effort detect-and-downgrade fix (`cross_repo_diff_unreviewable` -> `risk_profile: unverifiable`) stays open for its own change.
**Severity:** medium
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — defaults/hooks/pre-commit:20 blocks main during EXECUTE/GATES and ships as a plugin default installed by /aid-init, no longer a hand-installed guard.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — aid-fsm.sh queue-revalidate ships with the 4-outcome contract and pipeline.md §12 requires revalidation before a blocked status is respected.
...
**Status:** **CLOSED by P083 (Step 5)** (verified 2026-08-11 against v2.83.1) — confirmed and worse than filed: the self-host `plan_diff` gate had lost its `command:` entirely (not merely `required: false`), so it produced a `skip/no_command` row that verified nothing across five merge-path profiles, while the "P038+" exemption note had gone unrevisited for 40+ plans. P083 Step 5 (commit `b9de41c`) restored the command from the shipped default, kept `required: false` for the duration of this plan (8 of this plan's own 11 suites did not exist yet), replaced the stale note with the dated decision, and made `aid-run-gates.sh` refuse -- by name -- any active profile whose `include[]` lists a command-less gate, registered as `gate_command_required`.
...
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed unchanged after 4 weeks: the CP3-Freshness-Exception trailer mechanism (`aid-fsm.sh:1430-1580`) has no counterpart in `aid-release-policy.sh`'s gates_report blocking path (`:797-806`), so a D4-exempted trailing commit still stamps `gates_report.json` one commit behind HEAD and counts as a hard blocker with no knowledge the trailer adjudicated it cosmetic. IMP-201 is still `pending`. Currently latent only: `release-decision-policy.yaml:40-41` is `enforcement: observe`; it turns live the moment E10 flips to blocking. Sieve reason: not selected for P083 -- M-effort, tied to the E10 promotion timeline rather than today's pipeline.
**Severity:** medium
...
**AID version:** v2.54.0
**Observed at:** 2026-07-11
**Status:** **PARTIALLY FIXED** (verified 2026-08-11 against v2.83.1) — the WAN instance is gone: the named stale test (`smlouva_unmatched`) no longer exists and both cited commits are unreachable in that repo's history. Still open: the systemic gap -- nothing mechanically forces the full e2e suite when a step changes shared routing/attach behaviour; CP2's test scope remains pure per-step judgment (`e2e` has zero hits in `agents/verifier.md`/`aid-prefilter.sh`). Not touched by P083. Open half: a `cp2_full_suite_paths[]`-style pre-filter rule (M-effort) remains unscheduled.
**Severity:** medium
...
**AID version:** v2.54.0
**Observed at:** 2026-07-11
**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed and broader than filed: `commit_scope_violation`'s matcher (`aid-fsm.sh:6032-6072`) is still literal-equality-or-prefix-only, while the enforcing gate (`gates/scope-check.sh:28-39`) is glob-aware -- the same `allowed_paths` entry is in-scope for the gate and false-positive for the telemetry companion. The entry's specific `{rev}` trigger is now lint-blocked upstream (closing that one reproduction), but any legitimately globbed entry still fires falsely. Sieve reason: not selected for P083 -- real, cheap (S), but not on this plan's four hot-path defects; the fix (reuse the gate's glob `case` form) stays open.
**Severity:** medium
...
**AID version:** v2.54.0
**Observed at:** 2026-07-11
**Status:** **CLOSED — already fixed** (verified 2026-08-11 against v2.83.1) — confirmed: the stray root-level file in WAN is gone and was never committed; the correct content now lives in the canonical evidence directory with the expected `Reviewed-Head`/verdict. A repeat can no longer be silent: `aid-fsm.sh:5705-5729`'s precondition treats a missing canonical `verifier-output-step-N.md` as a hard `increment-step` failure, not a quiet gap. One-off misplacement, since corrected, structurally guarded against recurrence.
**Severity:** low
...
**Observed at:** 2026-07-11
**Status:** **PARTIALLY FIXED** (verified 2026-08-11 against v2.83.1) — the destructive-rename half is sealed and predates this plan (`aid-release.sh:468-491`, `_release_version_sealed`, tested by `test-aid-release-seal.bats`): a tagged current version's CHANGELOG heading is preserved rather than renamed. P083 Steps 3-4 additionally fixed the concrete symptom this entry's own live evidence corroborated -- `README.md:3`'s tagline was frozen at v2.69.0 across 14 releases because `.aid-o/config/project.yaml`'s tagline regex used backslash-escaped parentheses that never matched under `sed`'s BRE; Step 4 removed the escapes, repaired the line once, and made a non-matching configured pattern report by name instead of printing `Updated`. Still open: `[Unreleased]`-heading recognition (`update_changelog` has no branch for it) and the README Roadmap updater, which still does a blind version-token substitution leaving the previous release's description attached to the new number; both remain unscheduled.
...
**Status:** **DONE** (verified 2026-08-11 against v2.82.0) — every profile interpolation now uses yq strenv() with the value passed via environment (aid-fsm.sh:5321, aid-run-gates.sh:467+), so a profile name can never be parsed as part of the expression.
...
**Status:** **WRONG ADDRESS — real defect fixed by P083 (Step 7); description below replaces the original** (verified 2026-08-11 against v2.83.1). The original claim was literally true (`defaults/execution.yaml` has no `gate_profiles` block) but named the wrong artifact: that file has no runtime reader at all (grep for `defaults/execution.yaml` outside CHANGELOG returns only test files), so seeding it would fix nothing. The real defect lived in the generator every consumer actually runs, `render_gate_profiles_block` (`lib/aid-init-execution-yaml.sh:206-265`), which emitted only 2 of the 5 canonical profiles (`targeted`, `full`) and never `release` — while the shipped `plan-boundary-policy.yaml` makes `release` the mandatory plan-final floor. A stack-detected `/aid-init` consumer therefore cleared the all-or-nothing `_pfsm_has_gate_profiles` guard on an incomplete table, flipped to `plan_branch`, and hit a hard `PRECONDITION FAIL: profile 'release' has an empty or missing include[]` at the very last stage of a plan, after all EPIC work was done. P083 Step 7 (commit `b9de41c`, hardened by `85357ab`) made `render_gate_profiles_block` emit the full canonical ladder (`quick < targeted < standard < full < release`) composed from the gates a project's detected stacks actually define, and additionally fixed a second consumer the original description did not name — the `/aid-init` existing-project upgrade path, which can no longer append a profile naming a gate the target file doesn't define. Covered by `test-init-gate-profiles.bats`; the zero-stacks branch (an audited, handled fallback to `legacy_epic_release_mode`) is untouched.

### EPIC generator truncates multi-line acceptance criteria to their first line

**Status:** **CLOSED by P083 (Step 2)** (verified 2026-08-11 against v2.83.1) — confirmed and reproduced exactly as filed, in TWO separate copy-pasted awk blocks (`aid-plan-to-epic.sh:909-925` feeding the flattened `## Acceptance Criteria` section, `:936-949` feeding the machine-read per-step `ac[]`), both filtering on a flush-left `- ` bullet test so indented continuation lines matched neither the emit test nor the `**` terminator and were silently dropped — reproduced against the entry's own P068 Step 2 example, truncating mid-sentence and dropping every continuation line in both outputs. The entry's own diagnostic pointer to `lib/aid-scoping.sh::_aid_extract_*` is wrong (those handle Files bullets only). P083 Step 2 (commit `d05356d`) replaced both blocks with one shared extractor (`lib/aid-ac-extract.sh`) that appends continuation lines until the next flush-left bullet or section terminator, and left a comment at the old pointer's spot correcting it; covered by `test-ac-extraction.bats`.
**Original status (idea — observed 2026-07-24 during P068 EPIC regeneration):**
...
**Status:** **CLOSED by P083 (Step 9)** (verified 2026-08-11 against v2.83.1) — premise had shifted before this plan: the zero-byte-seal complaint is already fixed (`lib/aid-c0-plan-review.sh:364-374,448-461` seals an absent graph as the explicit status `absent_pre_generation`, not an empty-string sha256, and validates a present graph's schema + plan-hash binding), and a real pre-generation graph producer now exists (`lib/aid-source-plan-graph.sh`, invoked from `aid-plan-to-epic.sh:136`). But it is wired into EPIC generation, which runs strictly *after* the C0 plan review — measured on P076/P079/P080, every real review sealed both graph paths `absent_pre_generation`, with the graph file's mtime 37 minutes after the review it was meant to feed, while the C0 prompt still asked a mandatory acyclicity/output-producer question with no artifact and no defined substitute. A first attempt to move the producer earlier was reversed on review (P083 plan text) because `provisional-graph.json` is also the producer-integrity seal EPIC generation depends on, and pre-minting it would silently re-bind that seal on every C0 re-run. P083 Step 9 instead removed the requirement from the prompt: `defaults/prompts/c0-plan-review-prompt-v1.md` no longer asks for an analysis of an artifact the review does not receive, naming the text-derived substitute for the `absent_pre_generation` case; covered by `test-c0-plan-graph-input.bats`. `provisional-graph.json` still has exactly one writer.
**Original status (idea — raised by the C0 review of P068, 2026-07-24. Owner: the C0
...
### NEW — IMP-471: standing whole-plan auto-GO as a first-class, mechanically consumed record

**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed with two corrections: the mode lives in the active-runs map (`aid-fsm.sh:344,472-492`), not `fsm-state.yaml`, and it is the env var `AID_AUTO_MODE=1` read at init time, not something passed `--auto` to the FSM — a fresh session or chained next-EPIC init without that env honestly stamps `manual`. `pm-auto-go.json` has zero mechanical consumers anywhere in the codebase (grep for `pm-auto-go` returns one prose comment). Sieve reason: its own plan (P083 explicitly excludes it, plan lines 42/522).
**Priority:** high
**Class:** auto mode / PM authority plumbing

**Summary:** `--auto` is a per-EPIC-run property (stamped into that run's
fsm-state.yaml at init). Between EPICs of a chained plan the mode is NOT
inherited — the next EPIC's init needs `--auto` passed again, so a controller
(or a fresh session unaware of the PM's chat-level GO) silently degrades to
manual. The "PHASE-END: wait for PM GO" rule is prose only, and a standing
whole-plan GO has no durable home at all. Live prototype from the P076 run:
`.aid-o/work/plan-state/P076/pm-auto-go.json` (+ an audited operations.jsonl
line) — a record + instruction, with no mechanical consumer.

**Proposed change:** (1) a plan-scoped standing-GO record with a schema
(grantor, granted_at, scope, optional expiry), produced by a public command in
the `pm-override grant` family — never a hand-written JSON; (2) mechanical
consumers: queue pickup of the next chained EPIC inherits auto mode from the
plan record instead of a re-passed flag, the PHASE-END check consults the
record (present → continue, absent → stop for GO), and a mode flip to manual
while a GO is active requires a named PM-authority exception with an audited
reason — never silent; (3) `/aid-status` renders the active GO; a PM revoke
command supersedes it (new record, not deletion). Extends P076's
`auto_controller` machinery; hook points: `upsert_active_run`/chain pickup in
aid-fsm.sh, the PHASE-END instruction in `skills/run-management.md:24`, and
the pm-override producer in aid-fsm.sh.

---

### NEW — IMP-472: case-insensitive single-token verdict parsing in FSM checks

**Plan written: P079** (2026-08-10; implemented outside AID on plan/P079, ceremony evidence in `.aid-o/work/evidence/P079/`).

**Status:** **DONE** — implemented by P079, released in v2.81.0 (whole-diff reviewed; all 8 plan ACs green). Original status: new (found live 2026-08-09, first P076 implementation run)
**Priority:** medium (each occurrence costs a stalled increment + one extra dispatch)
**Class:** parser tolerance / P073-family loosening

**Summary:** `fsm_check_verifier_output()` (aid-fsm.sh:958) accepts only
lowercase `pass|fail` on the `verdict:` line; a verifier writing `PASS` is
rejected as an unknown verdict and `increment-step` hard-fails. Observed live:
the E-076-1_3 Step 1 controller had to message the verifier to re-case one
word, costing ~2 minutes and an extra dispatch. Same defect class as the
adjudicator `- []` block-scalar rejection fixed in P074: a parser punishing a
trivially equivalent format.

**Proposed change:** normalize case (`tr A-Z a-z`) before the verdict match,
and sweep every other single-token field comparison in the FSM checks
(`classification:`, adjudicator `verdict:`, similar yaml_field consumers) for
the same rigidity — tolerance for equivalent forms, loud rejection for
genuinely unknown values. One bats case per normalized field.
**Timing note:** aid-fsm.sh is an active P076 target file — land this AFTER
P076 merges to avoid a pointless conflict with the running implementation.
**Scope addition (2026-08-09, second live occurrence):** the fix must also
sweep the DOCUMENTATION examples — instruction surfaces show `## Result:
PASS / FAIL` (uppercase) while the parser accepts only lowercase; whoever
follows the docs literally hits the reject. The primary offender is the
step-verify template itself (`defaults/templates/step-verify-template.md`) —
the file every verifier is told to follow. Normalize the parser AND make the
doc examples match the accepted form.

---

### NEW — IMP-473: review findings need a mechanical route to a step that may fix them

**Plan written: P079** (2026-08-10; implemented outside AID on plan/P079, ceremony evidence in `.aid-o/work/evidence/P079/`).

**Status:** **DONE** — implemented by P079, released in v2.81.0 (whole-diff reviewed; all 8 plan ACs green). Original status: new (found live 2026-08-09, P076 E-076-1_3 run — PM-forwarded
controller feedback)
**Priority:** high
**Class:** review loop / scoping seam

**Summary:** CP2 at Step 2 found two serious defects in `aid-run-gates.sh`,
but the step whose fix window was open (Step 5) is not allowed to touch that
file under per-step scoping — the controller had to hand-route the findings
into Step 4 (the last step with write rights on the file). Without that manual
alertness the findings would have silently expired. Same shape for
cross-cutting obligations: no EPIC-1 step had rights on the enforcement
registry or CHANGELOG, so those writes silently wait for a later EPIC with no
durable carrier. Recurrence count from the single P076 run (controller
retrospective 2026-08-09): step 5 → step 6, steps 8-9 → next step, and one
CROSS-EPIC hand-off (EPIC 1 finding → EPIC 3) — the mechanism must therefore
route across EPIC boundaries too, not only within one EPIC's step list. AID has no "carry finding → first/last step authorized to fix
it" mechanism; findings live only in review prose.

**Proposed change (analysis first):** a small carried-findings record per run
(finding, source checkpoint, target file, routed-to step or `unroutable`),
written by the controller at CP2/C3 verdict time; FSM refuses EPIC completion
while an unresolved routed finding targets a step already passed, and
`unroutable` findings must land in the backlog/registry before DONE (same
durable-registration convention as P076 Step 14). Design must stay a loosening:
routing is bookkeeping, not a new gate on content.

---

### NEW — IMP-474: verifier-output prefilter seeds inconsistently across steps
...
controller feedback)
**Priority:** low
**Class:** parser tolerance / predictability

**Summary:** at Step 0 the prefilter created `verifier-output-step-0.md` as a
seed file before dispatch; at later steps it did not, and each verifier had to
create its own file. Nothing broke, but the asymmetry is unpredictable for
controllers templating dispatches. Either always seed or never seed; one bats
case pinning the chosen behavior.

---

### NEW — IMP-475: advance-to-gates runs gates in the invoker's tree, not the plan worktree

**Plan written: P079** (2026-08-10; implemented outside AID on plan/P079, ceremony evidence in `.aid-o/work/evidence/P079/`).

**Status:** **DONE** — implemented by P079, released in v2.81.0 (whole-diff reviewed; all 8 plan ACs green). Original status: new (found live 2026-08-09, first P076 run — PM-reported false-green
gate run; verified in code same day)
**Priority:** critical (a gate verdict about the wrong tree)
**Class:** correctness / P074 worktree seam

**Summary:** `_fsm_require_plan_worktree` (the P074 Step 8 redirect) is called
by exactly two aid-fsm.sh commands — `cmd_init` (aid-fsm.sh:3036) and
`cmd_done_advance` (:5101) — plus plan-fsm's epic-merge-to-plan and
plan-finalize. `cmd_advance_to_gates` never calls it. P074 Step 1 fixed its
STATE paths (evidence dir, execution.yaml, gates report all resolve to the
state root — the in-function comment documents this), but the process working
directory stays wherever the controller invoked it, and the gate runner
executes gate commands in the caller's cwd. For any plan with an execution
worktree, gates therefore test the PRIMARY checkout's tree instead of the
candidate — a structurally false-green (or false-red) verdict. Observed live
on the first P076 EPIC run.

**Proposed change:** add the same first-statement
`_fsm_require_plan_worktree "$epic_id"` call to `cmd_advance_to_gates`
(before the pre-flight guards; the redirect absolutizes the relative
state_file argument like done-advance's call site already proves), plus one
bats case: worktree-recorded plan + advance-to-gates invoked from the primary
checkout ⇒ the gate runner's cwd is the worktree (assert via a fixture gate
command that records `pwd`). While there, audit the remaining non-redirected
subcommands for tree-dependence (transition/increment-step/promote-check are
state-root-only reads and look safe; verify rather than assume).

**Live evidence (P076 EPIC 1, 2026-08-09):** gates completed in 134 s AGAINST
MAIN, reported `overall: pass`; the risk resolver computed the diff in the
wrong tree, got an empty diff, and selected the cheapest profile;
`revision.head_sha` in the report was main's commit, not the candidate. The
false-green is therefore two-dimensional: wrong verdict subject AND corrupted
risk-profile selection.
**Secondary consequence (same run):** the `--state-file` circle — the runner
with `--state-file` refuses outside state GATES, while the transition to GATES
requires a finished report; `advance-to-gates` exists to resolve this
atomically, and it is exactly the command that does not redirect. Controller
workaround was running the runner without `--state-file`, losing the state
binding. The fix restores the atomic path; no separate change needed.
**Interim workaround for running plans:** invoke advance-to-gates with cwd
already inside `.aid-worktrees/plan-<id>` — the redirect being missing means
cwd is simply inherited, so a controller that cd's first gets correct gates.
**Timing note:** aid-fsm.sh is an active P076 target file — land the fix
right after P076 merges (same rule as IMP-472); the workaround covers the
running plan until then.

---

### NEW — IMP-476: release-blocking deferrals live only in a gitignored file

**Plan written: P079** (2026-08-10; implemented outside AID on plan/P079, ceremony evidence in `.aid-o/work/evidence/P079/`).

**Status:** **DONE** — implemented by P079, released in v2.81.0 (whole-diff reviewed; all 8 plan ACs green). Original status: new (found live 2026-08-09, P076 run — redundancy sweep at run end)
**Priority:** high
**Class:** evidence durability / plan-boundary correctness

**Summary:** `carried-obligations.md` — the file that records deferred items
the same document labels "release blocker" — is written under `.aid-o/`,
which is gitignored (`.gitignore: **/.aid-o/`), inside the plan worktree.
An untracked file in a worktree does not survive the merge to the plan
branch, so a release-blocking obligation silently evaporates at exactly the
boundary it was supposed to block. Found only by a redundancy sweep at the
very end of the run.

**Proposed change:** obligations that can block a release must live on a
durable, merge-surviving surface: either (a) force-track them like the
divergence evidence precedent (`git add -f` + explicit path allowlist), or
(b) move the carrier into the tracked plan evidence family that plan-close
already reads. Whichever carrier is chosen, plan-close/plan-finalize must
READ it mechanically (an obligation file that nothing consumes is decoration
per AID-v3 §1). One bats case: obligation written in the worktree survives
epic-merge-to-plan and is visible to plan-close in the state root.

---

### NEW — IMP-477: plugin self-development trips its own dogfood skew preflight

**Plan written: P079** (2026-08-10; implemented outside AID on plan/P079, ceremony evidence in `.aid-o/work/evidence/P079/`).

**Status:** **DONE** — implemented by P079, released in v2.81.0 (whole-diff reviewed; all 8 plan ACs green). Original status: new (found live 2026-08-09, P076 run)
**Priority:** medium (blocks every plugin-modifying EPIC once per run; manual workaround exists)
**Class:** dogfood seam / preflight false positive

**Summary:** `verify-state` hard-stops when invoked from a plan worktree
whose EPIC modifies `plugins/*/scripts/` — the dogfood cache-skew detection
correctly notices that the installed plugin differs from the tree's plugin
code, but that "skew" IS the work in progress. From the primary checkout the
same command passes. Every self-development EPIC (most of this repo's work)
hits this once it starts executing plugin changes in a worktree.

**Proposed change:** the skew preflight must distinguish "installed plugin ≠
this tree's plugin because the tree is mid-implementation" from real cache
staleness. Cheapest honest rule: when the invocation tree is a registered
plan execution worktree AND the diff base..HEAD touches `plugins/`, downgrade
the hard stop to a loud one-line warning naming the skew (the run's gates and
review already bind the candidate tree; the cache check protects the
CONTROLLER's tooling, which runs from the installed plugin either way). Keep
the hard stop for the primary checkout. One bats case per branch.

---

### NEW — IMP-478: chained EPIC task branches are cut from a stale plan-branch head

**Plan written: P079** (2026-08-10; implemented outside AID on plan/P079, ceremony evidence in `.aid-o/work/evidence/P079/`).

**Status:** **DONE** — implemented by P079, released in v2.81.0 (whole-diff reviewed; all 8 plan ACs green). Original status: new (found live 2026-08-09, P076 run — EPICs 2 and 3)
**Priority:** critical (an EPIC building on code that does not exist in its tree)
**Class:** chain integrity / branch topology

**Summary:** the task branch for a chained EPIC is created at PRE-FLIGHT from
the plan branch's head AS OF THAT MOMENT. In the P076 run, EPIC 2's and
EPIC 3's `task/E-076-*/main` branches pointed at d822957 (the pre-EPIC-1
head) and contained none of EPIC 1's merged work; the controller had to
manually re-point both branches to `plan/P076` and rewrite `base_commit`.
Without that, EPIC 2 would have built its services on top of code that does
not exist in its tree.

**Proposed change:** epic-start for a chain member must cut (or fast-forward)
the task branch from the CURRENT `plan/<id>` head at the moment the EPIC
actually starts, not reuse a ref captured at generation/preflight time; if
the branch already exists and is behind `plan/<id>`, fail loudly with the
exact repair (or auto-fast-forward when the branch has no unique commits) and
update `base_commit` in the same operation so the risk resolver diffs the
real base. Bats case: two-EPIC chain fixture where EPIC 1 merges a file and
EPIC 2's tree must contain it at epic-start.

---

### NEW — IMP-479: gate runner writes timeline to a cwd-relative path — evidence lost from a worktree

**Plan written: P079** (2026-08-10; implemented outside AID on plan/P079, ceremony evidence in `.aid-o/work/evidence/P079/`).

**Status:** **DONE** — implemented by P079, released in v2.81.0 (whole-diff reviewed; all 8 plan ACs green). Original status: new (found live 2026-08-09, P076 run)
**Priority:** high
**Class:** evidence integrity / P074 worktree seam

**Summary:** the gate runner's timeline logging writes to a path relative to
the process cwd. Invoked from the plan worktree (the CORRECT place per
IMP-475), every timeline append failed with `No such file or directory × N`
— gates executed but their audit trail did not. The two defects compound:
run from the primary checkout you get evidence about the wrong tree; run
from the worktree you get the right gates and no evidence.

**Proposed change:** resolve the timeline (and any other evidence write in
the runner) through the state root exactly as advance-to-gates already
resolves evidence_dir/report paths (aid_state_path precedent) — never
cwd-relative. Land TOGETHER with IMP-475's redirect fix and cover both in
one bats case: worktree-invoked gate run produces a written timeline in the
state root AND a report whose head_sha is the worktree's HEAD.

---

### NEW — IMP-480: allowed_paths derivation ignores files named only in a step's Test: bullet

**Plan written: P079** (2026-08-10; implemented outside AID on plan/P079, ceremony evidence in `.aid-o/work/evidence/P079/`).

**Status:** **DONE** — implemented by P079, released in v2.81.0 (whole-diff reviewed; all 8 plan ACs green). Original status: new (found live 2026-08-09, P076 run)
**Priority:** high
**Class:** scoping derivation / D2 seam

**Summary:** a file named explicitly in a plan step's `- Test:` Files bullet
(`test-instruction-closure.bats`) was absent from the step's derived
`allowed_paths`, so the implementer was forbidden to touch a file the plan
explicitly assigned to the step. Create/Modify entries flow into the
derivation; Test entries evidently do not.

**Proposed change (CORRECTED 2026-08-09 after code grounding):** the stated
mechanism was wrong — all four verbs (Create|Modify|Test|Rewrite) already
flow through the derivation (lib/aid-scoping.sh:210 strips all four;
aid-plan-to-epic.sh:988 passes all four; regression test
test-epic-to-json-regression.sh:317-344 pins Test/Rewrite since v2.57.2).
The real defect class is the SILENT DROP: a Files bullet that fails the verb
regex (indented bullet treated as prose per the column-0 rule, a verb+path
split across two lines, or a missing verb label) is `continue`d without a
word (aid-plan-to-epic.sh:988), so the path never reaches allowed_paths and
nobody is told. Fix: make the drop loud — collect unparseable bullets inside
a `**Files:**` block and fail (or at minimum warn with file:line) at
generation; reproduce the live P076 case (test-instruction-closure.bats)
against its plan text to confirm which drop shape it was. One bats case per
drop shape.

---

### NEW — IMP-481: the vacuous-green test pattern — tests that pass while checking nothing

**Plan written: P079** (2026-08-10; implemented outside AID on plan/P079, ceremony evidence in `.aid-o/work/evidence/P079/`).

**Status:** **DONE** — implemented by P079, released in v2.81.0 (whole-diff reviewed; all 8 plan ACs green). Original status: new (pattern named 2026-08-09, P076 run controller retrospective;
5 disguises + 2 fix-survivals observed in ONE run)
**Priority:** high
**Class:** test quality / verification honesty (analysis first)

**Summary:** one run surfaced five disguises of the same defect — a green
test that verifies nothing: (1) a test that SKIPS when its target file is
missing, and the skip reads as green (a clone reported 11/11 while checking
nothing); (2) an invariant that reads its own claim as evidence
(`status: wired` + all classes live, no loader involved — suite green);
(3) an enforcement citing documentation that does not exist
(`grep -ci service aid-run.md` → 0 hits, still green); (4) a check satisfied
by the FIRST file containing a token, so the anchor it exists for is never
opened; (5) `grep -c` that dies under `set -e` before printing its own error
(returns 0 matches with exit 1 → kills the test with no message). Twice the
defect SURVIVED its own fix: a containment check closed one hole while
opening the adjacent one (root computed from the data being judged), then
failed openly when `realpath -m` was missing — three rounds, same shape.

**Proposed change (analysis first, then mechanical):** (a) an authoring rule
in the test-writing guidance: a test must fail when its subject is ABSENT —
skip-on-missing is only legal with an explicit `skip` that the harness counts
and reports as skipped, never as passed; assertions must read a surface
DIFFERENT from the one that wrote the claim. (b) A cheap sweep harness that
flags the mechanical subset: `skip`-guards keyed on file existence, `grep -c`
under `set -e`, self-referential fixture reads. (c) Registry row once the
sweep exists. Scope consciously overlaps the (cancelled-parallelism) audit's
behavior-claims work — reuse its behavior_claims vocabulary if it survives
the removal; do not resurrect the parallelism machinery for this.

---

### NEW — IMP-482: aid-release.sh can rename an already-released version

**Plan written: P079** (2026-08-10; implemented outside AID on plan/P079, ceremony evidence in `.aid-o/work/evidence/P079/`).

**Status:** **DONE** — implemented by P079, released in v2.81.0 (whole-diff reviewed; all 8 plan ACs green). Original status: new (reported 2026-08-09, P076 run — plan-final boundary note)
**Priority:** medium (footgun at every plan-final; manual care is the current guard)
**Class:** release safety

**Summary:** at the plan-final boundary the CHANGELOG version must be set by
hand BEFORE `aid-release.sh` runs, otherwise the script bumps/renames the
CHANGELOG's top entry — which at that moment is the already-released and
already-tagged 2.79.3. A released, tagged version must be immutable; a tool
that can retitle it on a routine run is a correctness hazard guarded only by
operator memory.

**Proposed change:** `aid-release.sh` refuses to modify a CHANGELOG heading
whose version already has a git tag (`git tag -l vX.Y.Z` non-empty ⇒ the
entry is sealed; insert a NEW heading above instead, or abort with the exact
instruction). One bats case: fixture repo with tagged v1.0.0 at CHANGELOG
head — release run must create v1.0.1 heading, never retitle v1.0.0.
**Mechanism detail (P076 plan-final, 2026-08-09):** two hook points confirmed
live — (a) the version scan SKIPS the non-numeric `## [Unreleased]` heading
and lands on the already-released entry below it; (b) the plugin-CHANGELOG
fallback applies the same rename to the copy, PRESERVING byte-identity — so
the identity test that guards the two files is silent by construction. The
fix must close both: tag-sealing check per (a), and the identity test alone
must never be treated as evidence the release edit was correct.

---

### NEW — IMP-483: plan-final broad suite — take the real measurement, then delegate the heaviest process suites

**Plan written: P079** (2026-08-10; implemented outside AID on plan/P079, ceremony evidence in `.aid-o/work/evidence/P079/`).

**Status:** **DONE** — implemented by P079, released in v2.81.0 (whole-diff reviewed; all 8 plan ACs green). Original status: new (PM-approved 2026-08-09 during the first P076 plan-final run)
**Priority:** medium (tuning, not a defect — but every future plan-final pays it)
**Class:** gate runtime / test placement

**Summary:** the plan-final broad gate (`shell_pipeline_smoke` =
`run-all-tests.sh`, ~197 suites sequential) had p95 ≈ 31 min across 5
completed runs, then 8 consecutive timeouts at the old 1900 s cap; the cap is
now 7200 s. P076 added nine intentionally process-heavy suites (~25+ min:
test-aid-service ~10 min, test-service-lifecycle ~15 min,
test-p076-integration) that cannot be made faster without becoming vacuous
(orphan-freedom needs the real process table). The first completed run under
the new cap yields the first honest duration number in a month.

**Proposed change (two halves, in order):**
1. **Measure:** once the current run completes, record its duration into the
   runtime baseline; P076's own advice mechanism should then recommend
   `run_mode: background` for this gate in this repo's execution.yaml —
   plan-final stops blocking the session (owned job, controller re-attaches).
2. **Delegate:** move the heaviest P076 process suites into the existing
   `DELEGATED_SUITES` mechanism in `run-all-tests.sh` (the shipped precedent:
   boundary suites run as their own CI jobs) so the inline plan-final suite
   drops by ~25 min. Placement decision, not omission — the suites still run,
   with their owning job named in the map. One assertion: the delegated
   suites are named in exactly one owning job (no suite silently unowned).

**Explicit non-goal:** no parallel execution — the test-parallelism line was
cancelled by the PM 2026-08-09 (see IMP-469 REJECTED); this entry is about
placement and background ownership only.

**MEASUREMENT RESULT (2026-08-10, job shell-pipeline-measure):** 12 200 s
(3 h 23 min), 06:20:26→09:43:46 UTC, start HEAD 102a5f3; 3 673 tests in
207 suites; 3 651 pass / 21 fail / 1 skip ⇒ RC=1. All 21 failures sit in
6 suites of the CANCELLED test-audit/parallelism line (catalog `status:
proposed` since 5f24304 + cancelled machinery) — none in P076's 9 new
suites; they disappear with the removal plan. Suite-count cross-check:
47 sh + 163 bats − 3 delegated = 207 exactly ⇒ the delegated boundary
suites were NOT executed; the number IS the inline-pipeline baseline.
Caveats recorded: CPU contention from a GitHub Actions runner from
~minute 115 onward; HEAD moved during the run (102a5f3→6c6775f) — tests are
fixture-self-contained but strictly it is not tree-bound. Derived cap per
the shipped arithmetic: max(12 200×1.5, +60) = 18 300 s; re-measure after
the parallelism removal shrinks the portfolio.

---

### NEW — IMP-490: plan-final cannot be force-completed past a dead stage without fabricating artifacts

**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed: there is no PM-forceable path from a BLOCKED plan-final stage to the next one — every state/assertion refusal in `aid-plan-fsm.sh` (`:4426`, `:4744-4748`, `:5887`, `:6463`, `:6635`) is a plain `exit 1`, not routed through the force framework. A narrower escape hatch exists (`--substitute-receipt` + gate quarantine, `:4599-4726`) but still demands a real, passing, targeted run and explicitly excludes `plan_diff` from any substitute path. Sieve reason: not selected for P083 — M-L effort, and the honest design (a stage can be `waived` but never render as `passed`, with `plan_diff` permanently excluded) needs its own plan.
git merge was the only honest path)
**Priority:** high
**Class:** force framework / plan-boundary gap

**Summary:** the FSM path to plan-merge-to-main requires `release-decision.json`
produced by the C4 stage, which sits behind the review stage, which sits behind
the gates stage. When gates cannot complete (the broad suite's cap expired 9×),
there is NO legitimate forced path forward: the P073 force framework does not
cover skipping a plan-final stage, so the only options were fabricating the C4
artifact (refused — exactly the fraud class 40 review rounds eliminated) or a
direct `git merge` outside the FSM with a prose record that the ceremony ran
only through the inputs stage. The PM authorized the direct merge for v2.80.0;
the gap is that honesty required leaving the FSM entirely.

**Proposed change:** extend the plan-finalize force surface: a PM
`--force --reason` on a BLOCKED stage produces a stage-waiver receipt (same
fail-closed waiver family as P073) and lets the ceremony CONTINUE through the
remaining stages with the waived stage recorded as `waived`, never `passed` —
so C4 can still produce a real release-decision.json that says "gates: waived
by PM <receipt>" instead of the merge happening outside the record. The merge
then stays inside the FSM with an honest artifact chain. Design must reuse the
existing waiver receipt machinery, not invent a parallel one.

**Cross-ref:** IMP-489 (P076-line entry) is a DUPLICATE of IMP-482 (release
script renames a tagged version) — canonical is IMP-482; close 489 against it.

---

### NEW — IMP-491: aid-release.sh rollback leaves marketplace.json on the new version

**Status:** **CLOSED by P083 (Step 3)** (verified 2026-08-11 against v2.83.1) — confirmed and reproduced first-hand in a throwaway clone: a rollback printed 7 `Updated:` lines but "Updated 4 files total", then left `.claude-plugin/marketplace.json` at the bumped version while everything else in `UPDATED[]` returned to base. Root cause: `_release_update_files`'s no-config fallback path never added `marketplace.json` to `UPDATED[]` for its `.metadata.version`/`.plugins[0].version` branches (a "don't double-add" guard that dropped the file instead of de-duplicating it) — and this fallback is the LIVE path, since `.aid-o/config/project.yaml` is gitignored and every clone/worktree (including AID's own plan worktrees) takes it. P083 Step 3 (commit `d05356d`) made those branches record their file in `UPDATED[]`, de-duplicated the array before the count/staging/rollback, named pre-dirty skips and restore failures in the output instead of swallowing them, and fixed a related bookkeeping bug the same reproduction surfaced (`update_changelog`'s pre-written-entry no-op branch was inflating `UPDATED[]` for an edit it did not make). Covered by `test-aid-release-rollback.bats`.
**Original status (new — found live 2026-08-10 during the P079 release step — second of
two release-script defects; the first was fixed in P079 itself):**
**Priority:** medium
**Class:** release safety / rollback honesty

**Summary:** the release script's rollback path claims it restored every
version-carrying file, but `marketplace.json` is left on the NEW version —
the claim and the state diverge, which is the same lie-class P079 exists to
eliminate (a tool asserting something it did not do). Found and described by
the P079 implementer during the v2.81.0 release run; deliberately NOT fixed
in-flight (out of P079 scope, release path already exercised).

**Proposed change:** the rollback restores ALL files it touched (both
marketplace.json fields included) and its completion message enumerates the
restored files from the actual write-back list, never from a static claim;
one bats case: interrupted release → rollback → all 8 version locations
byte-equal the pre-release state.

---

### NEW — IMP-492: the catalog's root `status` field is vestigial after the parallelism removal

**Status:** **VERIFIED REAL — not scheduled — recorded decision: retire the test-portfolio audit outright** (verified 2026-08-11 against v2.83.1) — confirmed the field is load-bearing, not vestigial: `lib/aid-test-audit-command-allowlist.sh:117-119` refuses every catalog-derived command unless the document-root `status` is `approved` (gate-registered commands still pass via a separate path); its live value is `proposed` from commit `5f24304`, whose message claimed the opposite, so a real `measure`/`full` audit today silently measures only registered gates. PM decision (2026-08-11): delete the whole 16-script/5,260-line test-audit layer outright — the capability existed to decide test parallelism, that line was cancelled in P078, and the test-tier standard (P081) now answers the questions it was built to answer, more cheaply. `aid-select-tests.sh` is explicitly NOT part of the deletion (it is gate `targeted_tests`, reading the catalog's `source_pattern_mappings[]` as its path-to-suite map, which survives). Not a step in P083 — the blast radius made it its own P078-shaped removal plan; this entry records the decision per this plan's Success Criteria.
**Priority:** low
**Class:** schema truthfulness / audit-line cleanup

**CORRECTION (2026-08-10, later the same day):** the "no consumer reads it"
claim below is WRONG and is retracted. `lib/aid-test-audit-command-allowlist.sh:117-119`
reads the document-root status and refuses every command unless it is
`approved` — so the field gates the profiler's entire allowlist. It is the
direct cause of the 15 `test-aid-test-audit-profile.bats` failures tracked in
IMP-494. Priority raised low → medium; the field is load-bearing, not vestigial.

**Summary (as originally written, partly retracted above):** `.aid-o/config/test-catalog.yaml`'s document-root `status` field
is still REQUIRED by `defaults/schemas/test-catalog.schema.json` and still
written by `aid-test-catalog-approve.sh`, but after the removal **few consumers
read it** — `aid-select-tests.sh:261` gates on `mapping_approval.status`, not
the root field. Its live value is `proposed` (from commit 5f24304, whose
message claimed the opposite), so the file reads as "not approved" while being
the active catalog the selector consumes. Nothing breaks; it is a lie with no
reader, which is exactly the class of confusion that costs an hour the next
time someone debugs test selection.

**Proposed change:** decide the field's post-removal meaning during the test-tier
migration (which reworks the catalog anyway): either (a) it means "this is the
published/active catalog" ⇒ set it via the sanctioned `aid-test-catalog-approve.sh`
and document the meaning in the schema description, or (b) it has no meaning ⇒
drop it from schema, writer and file in one change. Do NOT hand-edit the field.

---

### NEW — IMP-493: gate runtime baseline still accepts parallel concurrency contexts

**Status:** **CLOSED by P083 (Step 8)** (verified 2026-08-11 against v2.83.1) — confirmed dead vocabulary: both producers hardcode `sequential` (`aid-run-gates.sh:2024`, `aid-fsm.sh:3762-3769` whose body is `printf 'sequential'` since P078 removed the scheduler), the receipt schema is already `enum: ["sequential"]`, and the live baseline's `*_by_context` maps were empty or absent on every entry — nothing to migrate. P083 Step 8 narrowed the acceptor in `lib/aid-gate-runtime-baseline.sh` to `sequential` only, deleted the non-sequential branches and the `*_by_context` assembly (fields included, per the CP1 review's finding that jq's `// {}` makes an absent key and an empty map indistinguishable to every reader), and registered the refusal as `gate_baseline_sequential_only`; a legacy file with populated `*_by_context` data still reads and yields correct percentiles.
**Priority:** low
**Class:** dead vocabulary / P063 machinery

**Summary:** `lib/aid-gate-runtime-baseline.sh:329-332` still validates
`concurrency_context` against `sequential|observe_parallel|parallel`, and the
lib branches on the value at `:401`, `:527`, `:535`, `:561` (per-context sample
grouping, fingerprint-reset rules). After P078 the only producer is
`aid-run-gates.sh:2024`, which hardcodes `sequential`, and the receipt schema
was narrowed to match. The acceptor is therefore dead vocabulary — but
historical `gate-runtime-baseline.yaml` files may still contain non-sequential
samples, so narrowing the reader without tracing the whole read path risks
breaking history reads. Correctly deferred by the implementer.

**Proposed change:** narrow to `sequential` only after either migrating the
historical baseline files (rewrite non-sequential samples) or letting them age
out of the rolling window; delete the per-context grouping branches in the same
change. One bats case pinning that a legacy file with non-sequential samples
still reads without error (or is migrated deterministically).

---

### NEW — IMP-494: `gate:bats_all` is red on main and nobody noticed

**Status:** **RESOLVED 2026-08-10** — both suites green; `gate:bats_all` no
longer carries a permanent red. Fix 1: `test-aid-test-content-scan.bats`
heredoc'd fixtures rewritten with `printf` (commit 704e94a). Fix 2 (PM
delegated the A/B choice, **B taken**): `test-aid-test-audit-profile.bats`
now builds an APPROVED catalog fixture in `setup()` instead of reading the
PM's workspace catalog — the allowlist
(`lib/aid-test-audit-command-allowlist.sh:117-119`) refuses every command
unless the catalog root status is `approved`, and `--approved-catalog`
defaults to `--catalog`, so all 15 failing cases were deciding on workspace
state rather than profiler behaviour. 21/21 green. Option **A** (confirm the
drifted `source_pattern_mappings` and re-approve the workspace catalog)
remains open and independent — it is the selector's staleness, tracked with
IMP-492, and the P081 tier pilot must decide whether T1 selection depends on
that mapping at all.
**Original status:** new (found 2026-08-10 by the P078 whole-diff review, independently
corroborated by the P078 implementer)
**Priority:** high
**Class:** gate truthfulness / the reason a deletion defect got this far

**Summary:** the `required: true` aggregate gate `bats_all` has been failing on
`main` since before the P078 branch: `test-aid-test-audit-profile.bats` fails
15 cases on BOTH main and branch, `test-aid-test-content-scan.bats` exits 1 on
both (`bats: unknown test name`). A required gate that is permanently red is a
gate nobody consults — and it is precisely why P078 could carry two
orphaned-subject suites (a deleted script's test suite, a fixture rejected by
the new schema) all the way to a merge request without an aggregate run
catching them. Correction to an earlier session claim: these failures do NOT
all disappear with the parallelism removal — profile still fails on the
removal branch.

**DIAGNOSIS (2026-08-10, both suites traced to root cause):**
- `test-aid-test-content-scan.bats` — **FIXED**. Three fixture bodies were
  written with heredocs containing `@test` at column 0; bats parses `@test`
  line-by-line and knows nothing about heredocs, so it registered three
  phantom tests in that file (`unknown test name`, "Executed 6 instead of
  expected 9", exit 1). Rewritten with `printf` so `@test` never sits at
  column 0 in the source. Now 6/6 green.
- `test-aid-test-audit-profile.bats` — **NOT a drive-by fix; needs a decision.**
  The suite hardcodes `CATALOG="$REPO/.aid-o/config/test-catalog.yaml"`
  (`:16`) — the PM's gitignored workspace artifact — and every case that
  actually runs the profiler dies at
  `lib/aid-test-audit-command-allowlist.sh:117-119`, which refuses all
  commands unless the catalog's root `status == approved`. The workspace
  catalog says `proposed` (commit 5f24304 flipped it while claiming the
  opposite) and cannot simply be re-approved: `aid-test-catalog-approve.sh`
  refuses with real mapping drift (`source_pattern_mappings[]` no longer
  matches a fresh selector snapshot after P076/P078/P079). Two honest exits:
  **(A)** the PM confirms the drifted mapping via
  `aid-test-catalog-confirm-mapping.sh` and the catalog is re-approved — fixes
  the selector's staleness too; **(B)** the suite stops depending on the
  workspace and builds its own catalog fixture — the correct test-design
  answer (its subject is the profiler, not the PM's workspace), and required
  anyway by the tier standard. Recommended: **B in the tier-migration pilot,
  A independently whenever the mapping is next reviewed.**

**Proposed change:** (1) fix or retire the two suites so `bats_all` is
genuinely green on main; (2) treat a red required gate as a first-class
condition — the nightly report of the test-tier standard must name it, and a
red aggregate must not be silently survivable across merges; (3) until then,
every removal-shaped plan carries an explicit orphan-detection step (grep each
surviving suite's subject path for existence), because the aggregate gate
cannot be relied on to catch it.

---

### NEW — IMP-495: the PM-facing decision card must be produced, not requested

**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed: nothing on `main` renders a PM decision card (`decision.card`/`decision_card` return zero hits); the single precedent (`lib/aid-test-audit-chat-summary.sh`) is real but the P080-specified renderers do not exist yet. P080 EPIC 3 already carries most of this entry's Proposed change (the four cards, the 7-block artifact skeleton, hard caps, verbatim-presentation tests) — the genuinely distinct residue is item (2), "a decision-required turn without an options list is refused," which P080 does not build. Sieve reason: not selected for P083 — explicitly scoped to land only after P080 ships the card contract (P083 plan line 43), to avoid duplicating P080 EPIC 3.
**Priority:** high
**Class:** human handoff / enforcement-vs-prose

**Summary:** the PM's standing requirement is that every non-trivial report
ends in a form he can act on at first reading: a plain-language summary of
what was done, plain-language findings, and **one question with concrete
options** — not a technical findings list whose question cannot be answered
without reading the list. His words: *"je přesně to, co potřebuju, aby si AID
adoptoval… a nejlíp vynuceně, pokud to jde."*

P080 EPIC 3 already specifies the four chat cards (Finished / Decision
required / Blocked / Progress, §14 D17) — but as an INSTRUCTION a model is
asked to follow. That is the weaker half of the same pattern the artifact
standard already rejects: *"stručnost vynucuje šablona, ne prosba."* The
observed failure mode is exactly what the standard predicts — a model that
follows the shape nine times and reverts on the tenth, precisely when the
report is long and the stakes are high.

**Proposed change:** make the decision card a rendered artifact rather than
requested prose. (1) A deterministic renderer assembles the card from
canonical data (what changed, what a check found, what the options are) with
the caps the artifact standard already defines; the model supplies only the
bounded prose blocks. (2) A "decision required" turn without options is
refused the same way an unfilled release entry is refused — the options list
is a required field, not a stylistic preference. (3) The options themselves
carry a recommendation and its reason, per the PM's clarification-format rule.
Land after P080 ships the card contract; this entry is the enforcement half.

---

### NEW — IMP-496: a closed plan is never archived, while its EPIC task files are

**Status:** **VERIFIED REAL — not scheduled** (verified 2026-08-11 against v2.83.1) — confirmed, with the entry's own mechanism claim corrected: no code anywhere moves a plan file into `plans/archive/` (`plans/archive` greps to one read site only, `aid-lifecycle.sh:1385`), and — contrary to the entry's premise — the EPIC task-file archival is ALSO not automatic: `aid-fsm.sh:6894-6907` is a blocking read-only PRECONDITION that refuses to advance until a human runs the `mv`, not an action that performs one. The real asymmetry is that the task file has an *enforcement* (a blocking check) and the plan file has *nothing* — no mover and no checker. Live count: 38 plan files still live (not 35), 55 already archived by hand. Sieve reason: not selected for P083 — S-effort but a fresh design decision (git mv at plan-close plus a one-shot backfill sweep needing a PM eyeball on which of the 38 are genuinely closed); not one of this plan's ten items.
**Priority:** medium
**Class:** lifecycle completeness / workspace hygiene

**Summary:** `.aid-o/plans/` holds 35 plan files, most of them long shipped
(P031..P069), because **nothing in AID archives a plan when it closes**. The
asymmetry is the tell: an EPIC's task file IS moved automatically —
`aid-fsm.sh` does `mv` into `tasks/archive/` before advancing — but the plan
that produced it stays in the live directory forever. `lib/aid-lifecycle.sh:1385`
already looks in BOTH `plans/` and `plans/archive/` when resolving a plan
file, so the archive location is a first-class, supported place; only the act
of moving is missing. Consequence: the live plans directory stops being a
list of what is being worked on, which is exactly why the PM had to ask which
of P031..P069 were still open.

**Proposed change:** at plan closure (`plan-close` / the plan-final terminal
transition), move the plan file to `.aid-o/plans/archive/` and prepend the
closure annotation the PM already writes by hand — released version, date,
and the merge SHA. Idempotent (already-archived is a no-op), and a plan whose
closure is administrative rather than released records that instead. One bats
case: closing a plan leaves `plans/` without its file and `plans/archive/`
with it, annotated; resolution through `aid-lifecycle.sh` still finds it.
**Backfill:** the same change ships a one-shot sweep for the 35 already-closed
plans, using each plan's recorded release version where one exists.

---

## Strukturální neduhy nalezené 2026-08-11 (souběžný běh P080 + P083 + WAN audit)

### NEW — IMP-497: nastavení bran se čte z hlavního stromu, kód běží z větve

**Status:** new (doloženo živě 2026-08-11, rozbilo běžící P080)

`aid_state_root()` se přes git common dir rozřeší na HLAVNÍ checkout i tehdy,
když příkaz běží v pracovní kopii plánu (`aid-fsm.sh:5257`,
`aid_state_path(".aid-o/config/execution.yaml")`). To je správně pro **stav**
(běhy, důkazy, počítadla — ten musí být společný), ale `execution.yaml` není
stav: **popisuje kód**. Brána přidaná do main proto okamžitě platí i pro větve,
kde její skript ještě neexistuje.

**Doloženo:** `lint_gate_names` přidaná do main 2026-08-11 odpoledne rozbila
běžící P080 (větev 15 commitů pozadu, skript tam není). Reprodukováno z jeho
worktree: `No such file or directory`. Dočasně obejito `required: false`
(commit `0d86468`), což je náplast, ne oprava.

**Druhá strana téže mince:** oprava `gate_profile_defaults` z téhož odpoledne
se ke stejnému běhu dostala taky — a ušetřila mu 13 minut na každý blok. Vazba
není jen škodlivá, je jen nerozlišená.

**Podstata:** nikdo nikdy nerozdělil „společný stav" od „nastavení, které patří
k větvi". Pracovní kopie přišly do AID až později (P064/P074); do té doby byl
jeden strom a otázka nevznikla.

**Náprava (návrh):** dvě cesty místo jedné — stav z hlavního stromu, popis
bran z větve; nebo verzování konfigurace tak, aby brána nesla, od kterého
commitu platí.

### NEW — IMP-498: padne jeden test a pouští se znovu celá sada

**Status:** new (2026-08-11, hlášeno controllerem P080)

Každý pokus o uzavření EPICu spustí brány jako celek. Když spadne jedna,
při dalším pokusu běží znovu i všechny, které prošly — a dvakrát to byl pád
z důvodu, který s plánem nesouvisel.

**Dvě části nápravy:**
1. **Pamatovat výsledek brány k danému HEADu.** Prošlá brána na nezměněném
   stromu se nemá pouštět znovu. Mechanismus částečně existuje — controller
   P080 si všiml, že druhý pokus vrátil identickou dobu 152 000 ms, tedy že se
   výsledek bere z uloženého záznamu; jen se nepoužívá k přeskočení.
2. **Opakovat nejdřív to, co spadlo.** Jedna sada z 28 se přeběhne za sekundy;
   zbytek až po ní. Runner to od v2.83.1 umí (`--only`), nikdo to nevolá.

**Odhad přínosu:** z 13 minut na každý další pokus se stane ~20 s + jeden běh
zbytku.

### NEW — IMP-499: riziko plánu se pozná z prózy, ne z toho, čeho se plán dotýká

**Status:** **DONE** — P084, vydáno v2.87.0 (2026-08-22). Klasifikace čte deklarované `Files:` proti mapě `defaults/policies/risk-paths.yaml`; referenční sada 23 plánů souhlasí s ručními štítky. Původně: new (změřeno 2026-08-11) — **PM to reklamoval jako „skoro vše je
vždy high risk", a měření mu dává za pravdu**

`aid-cp1-gate.sh:187` scanuje vzory přes **celý soubor plánu**
(`grep -qE "$pattern" "$plan"`), tedy včetně Contextu, Architektury a veškeré
prózy. Plán, který jen POPISUJE stavový automat, je klasifikován jako plán,
který ho MĚNÍ.

**Naměřeno na živých plánech** (počet zásahů vzorů): P074 = 33, P073 = 31,
P072 = 30, P080 = 13, P083 = 9, P078 = 5. P080 „srovnává texty nápovědy" a
trefil se třináctkrát — z toho první výskyt je **odkaz na zdrojový dokument
v hlavičce**, ne změna kódu.

**Náprava:** scanovat **deklarovaný seznam `Files:`**, ne celý dokument. Ten
seznam už existuje, je povinný a lint ho kontroluje — takže je to změna
jednoho `grep`u nad jiným vstupem, ne nový mechanismus. Teprve pak dává smysl
odstupňovat ceremonii (FSM/brány/vydávání → plná; testy a konfigurace →
střední; texty → lehká).

### NEW — IMP-500: generátor plánů vyžaduje totéž od každého plánu

**Status:** **DONE** — P084, vydáno v2.87.0 (2026-08-22). Povinnosti rozdělené na univerzální a pásmové, všech 28 kontrol Completeness Gate má zapsaný verdikt, testy se navrhují (`## Testing Strategy`) místo odpočítávání. Původně: new (PM 2026-08-11) — **potřebuje analýzu, ne rovnou opravu**

PM: *„generátor plánu je dobrý, ale potřebuje zásadní předělání, aby byl
cílenější."* Dnešní stav: každý krok musí mít Test položku, každý plán
strojově ověřitelná kritéria, human review summary, plnou sadu sekcí — bez
ohledu na to, jestli mění stavový automat, nebo přepisuje větu v nápovědě.

Souvisí s IMP-499, ale není totéž: 499 je o KLASIFIKACI (kdo je rizikový),
500 je o TOM, CO SE VYŽADUJE (co musí plán obsahovat a čím musí projít).
Analýza má zodpovědět: které povinnosti jsou univerzální (Files, kritéria),
které mají být odstupňované (Test u každého kroku, C0 kolo, počet posudků),
a co se dnes vyžaduje jen setrvačností.

**Nezačínat implementací.** Vstupem je rozbor skutečných běhů P076/P079/P080 —
kolik času sežrala která povinnost a kolik z toho chytilo reálný nález.

### NEW — IMP-501: přejmenovat brány podle nových pravidel (~575 výskytů)

**Status:** new (změřeno 2026-08-11) — **konvence i lint už PLATÍ, chybí jen
samotné přejmenování**

Pravidla jsou v ekosystémovém standardu (`test-standard`, „Jména bran", 6-10),
lint je zapojený jako brána `lint_gate_names` a nová brána musí projít čistě.
Existující jména jsou grandfatherovaná v
`scripts/tests/gate-name-allowlist.txt` — **včetně navržených náhrad**, takže
kdo to bude dělat, nemusí nic vymýšlet.

**Proč to není hotové:** naměřeno ~575 doslovných výskytů v ~60 souborech, plus
konzumentské konfigurace potřebují staré jméno jako alias po dobu jednoho
vydání. To je plán, ne úklid mimochodem — a mezitím lint zabraňuje tomu, aby
přibývala další lživá jména, což byla naléhavá část.

Až bude hotovo: seznam výjimek vyprázdnit a lint přepnout na `--strict`.

---

### NEW — IMP-504: 59 testovacích kontrol, které nemohou selhat, ve 21 sadách

**Status:** new (2026-08-12, P080 cross-model review)
**Priority:** high — jde o kontroly, jejichž jediná hodnota je záporná
**Class:** test integrity
**Area:** `plugins/aid-orchestrator/scripts/tests/` (21 sad mimo P080)

**Co je špatně.** V bashi je příkaz uvozený `!` vyňatý ze `set -e`, a ERR past
bats tu výjimku dědí. Takže řádek `! grep -q jehla soubor` v testu **hlásí `ok`
i tehdy, když ta jehla v souboru je**. Ověřeno přímou sondou, ne odvozeno.

Tenhle idiom nesl přesně ta tvrzení, jejichž celá cena je v tom záporu:
„neuniká žádné tajemství", „nikde není externí odkaz", „nikde nestojí, že
brána prošla". Tedy tam, kde test tiše mlčel, jsme si mysleli, že hlídá.

**Co už je hotové.** P080 přidal do `scripts/tests/bats/test-helpers.bash`
funkci `refute_grep` a převedl na ni **31 výskytů ve třech sadách**, kterých se
týkal jeho vlastní přezkum. Důkaz, že to není kosmetika: po záměrném rozbití
ochrany proti externím zdrojům hlásí případ `ok` pod `! grep` a `not ok` pod
`refute_grep`.

**Co zbývá.** **59 výskytů ve 21 dalších sadách.** Převést na `refute_grep`
a u každého ověřit, že po záměrném porušení opravdu spadne — samotný převod
nic nedokazuje. Pak přidat lint, který nový `! grep` v testech odmítne, jinak
se to vrátí; bez toho je tohle jednorázový úklid, ne oprava.

**Hotovo, až:** `grep -rn '^\s*! grep' plugins/aid-orchestrator/scripts/tests/`
vrátí prázdno a lint na ten tvar existuje a je v slévací cestě.

---

### NEW — IMP-503: publikovat AID stránku o šablonách artefaktů do ekosystémové dokumentace

**Status:** new (2026-08-12, P080 Step 16) — **stránka je NAPSANÁ a v tomhle repu
zaverzovaná; chybí jen commit v druhém repu, který smí udělat PM**
**Priority:** medium
**Class:** documentation handoff / cross-repo
**Area:** `plugins/aid-orchestrator/defaults/templates/artifact-templates-spec.md` (zdroj),
`/opt/eco/docs` (cíl — **jiný git repozitář**)

**Proč to není hotové.** `/opt/eco/docs` je samostatný git repozitář
(`git -C /opt/eco/docs rev-parse --show-toplevel` vrací sám sebe), takže obřad
plánové větve P080 tam nikdy nemohl commitnout. Zapisovatelný adresář dokazuje
oprávnění, ne pravomoc. Plán proto dodal **hotový balíček k aplikaci** a
publikace je úkon PM.

**Ověřená fakta, aby je PM nemusel objevovat znovu (CP1, 2026-08-12):**

- cílová cesta je `/opt/eco/docs/docs/aid/specs/artifact-templates.md`;
- adresář `docs/aid/specs/` v tom repu **zatím neexistuje**;
- sidebar té stránky je **100 % explicitní** (`grep -c autogenerated /opt/eco/docs/sidebars.ts` → `0`),
  takže `_category_.json` je tam **inertní** a nezaregistrovaná stránka se
  v navigaci nikdy neobjeví — jediné, co ji zviditelní, je záznam v `sidebars.ts`;
- `onBrokenLinks` i `onBrokenAnchors` jsou `throw`, takže **napůl zaregistrovaná
  stránka rozbije build dokumentace**. Buď obojí, nebo nic.

**Přesné příkazy, které PM spustí:**

```bash
mkdir -p /opt/eco/docs/docs/aid/specs
cp /opt/eco/projects/aid-orchestrator/plugins/aid-orchestrator/defaults/templates/artifact-templates-spec.md \
   /opt/eco/docs/docs/aid/specs/artifact-templates.md
# ... pak ruční editace sidebars.ts, viz snippet níže ...
git -C /opt/eco/docs add docs/aid/specs/artifact-templates.md sidebars.ts
git -C /opt/eco/docs commit -m "docs(aid): artifact template spec (AID v2.85.0, P080)"
```

**Přesný záznam do `sidebars.ts`** — nová kategorie uvnitř `label: 'AID'`,
vedle `Skills` / `Configuration` / `Contributing`:
...
```ts
            {
              ...
              label: 'Specs',
              items: [
                'aid/specs/artifact-templates',
              ],
            },
```

**Relativní odkaz ve stránce už počítá s cílovou hloubkou:** míří na
`../../ecosystem/specs/artifact-standard.md`, což z `docs/aid/specs/` sedí. Když
stránku PM umístí jinam, musí ten odkaz opravit, jinak `onBrokenLinks: throw`
shodí build.

**Hotovo, až:** `git -C /opt/eco/docs ls-files docs/aid/specs/artifact-templates.md`
vrátí soubor a `grep -c "aid/specs/artifact-templates" /opt/eco/docs/sidebars.ts`
vrátí `1`.
### NEW — IMP-502: AID po sobě neuklízí pracovní kopie plánů

**Status:** new (2026-08-11) — **PŮVODNÍ ZNĚNÍ BYLO ŠPATNĚ, opraveno tentýž
den při návrhu opravy**

Původně tu stálo, že AID po úspěšném uzavření plánu kopii neodstraní. **Není to
pravda.** `_pfsm_teardown_worktree` v `aid-plan-fsm.sh` (~:1600-1675) je
promyšlený úklid s per-plan zámkem, idempotencí a hlasitým hlášením — a při
konfliktu se zámkem schválně NEMAŽE nic, protože souběžná oprava může kopii
zrovna vytvářet. Ta část funguje.

**Skutečná díra je jinde: AID uklízí jen to, co samo vytvořilo na kanonické
cestě** `<root>/.aid-worktrees/plan-<id>` (`:1189`). Cokoli vzniklo jinde je pro
úklid neviditelné a nesmrtelné:

- `aid-orchestrator-p066-audit-design` a `aid-orchestrator-p078` ležely jako
  sourozenci v `/opt/eco/projects/`, tedy mimo kanonickou cestu — ruční nebo
  starší konvence. Teardown o nich nikdy nevěděl.
- **Zmrazené kopie pro CP3 nemá na starosti vůbec nikdo.** Grep přes celý
  plugin: `cp3-frozen` se nevyskytuje v žádném skriptu ani skillu. Vznikají
  ručně, protože CP3 vyžaduje, aby se oba posudky shodly na stejném commitu, a
  na živé větvi to nejde — controller P080 si musel strom zmrazit sám. Je to
  zavedená praxe bez mechanismu, tedy i bez úklidu.

**Doloženo:** na eco-devu leželo šest registrovaných worktree, z toho tři
odpad — `aid-orchestrator-p066-audit-design` (2. 8.) a `aid-orchestrator-p078`
(10. 8.), obě se slitou větví, nulou necommitnutých souborů a nulou commitů
mimo main; plus `.claude-worktrees/aid-v3-session-b` z 8. 5. Odstraněno
2026-08-11 (39 MB).

**Není to jen místo na disku.** Osiřelá kopie **shodila testovací sadu**:
`aid-test-adapter-shell-suite` prohledává skutečný repozitář, našla kopie sad
v `.aid-worktrees/` a odmítla je kvůli tečce v cestě — celé prohledávání padlo.
Opraveno vyloučením `.aid-worktrees/` a `.git/` (v2.84.0), ale to je náplast na
následek. Stačilo, aby měl někdo v druhém okně rozdělaný plán, a merge cesta
zčervenala na věci, která s jeho ani cizí prací nesouvisela.

**Náprava (revidovaná po zjištění výše):** NE další automatické mazání —
cizí kopie mazat automaticky je nebezpečné, zmrazené CP3 stromy jsou důkazní
materiál. Správná odpověď je **hlásit**: výpis registrovaných kopií s
klasifikací (živá / uklizená AIDem / cizí a osiřelá) a upozornění na ty, jejichž
větev je slitá a které leží déle než lhůta. Automaticky se maže dál jen to, co
AID vytvořilo a ví, že je hotové.

### NEW — IMP-503: generátor píše do plan.json bránu, kterou projekt smí smazat

**Status:** new (doloženo 2026-08-13 živě na WANu — od té chvíle neprojde
branami ŽÁDNÝ EPIC toho projektu)

`aid-plan-to-epic.sh:1354` má `dod_gates="- docs_updated"` — **natvrdo, bez
odvození z konfigurace projektu a bez fallbacku**. Každý vygenerovaný EPIC tedy
deklaruje bránu `docs_updated`, ať ji projekt má, nebo ne. Runner bere
deklarovanou-ale-nedefinovanou bránu jako tvrdý pád.

WAN ji z konfigurace odstranil, a **právem**: byla doslova `command: true`,
tedy vždy zelená a nic neověřující (IMP-427). Odměnou je, že od té doby neprojde
branami nic.

**Druhá vrstva je moje z 2026-08-11.** `aid-gate-name-lint.sh` zakazuje jméno
`docs_updated` (pravidla 6-10: musí začínat `tests_`/`lint_`/`check_`…) a jeho
seznam výjimek je **fail-closed: chybějící soubor = žádné výjimky**. V AID to
nevadí, protože allowlist jsem napsal. **Konzument, který lint převezme, nemá
allowlist žádný — takže mu každé zděděné jméno okamžitě nabíhá jako porušení.**
Ověřeno: WAN allowlist nemá. Přejmenovat bránu přitom nejde, protože `plan.json`
je otiskem svázaný s FSM stavem.

Tři cesty se tím zavřely najednou: nechat, smazat i přejmenovat.

**Náprava (dvě, obě malé):**
1. Generátor odvodí `dod_gates` z bran, které projekt **skutečně definuje**,
   místo aby jednu jmenoval natvrdo. To odblokuje všechny projekty naráz.
2. Fail-closed allowlist zůstává správný pro AID, ale **shipnutý lint potřebuje
   dodávaný výchozí allowlist** se zděděnými jmény — jinak je adopce lintu
   v cizím projektu okamžitý hard fail. Moje chyba v návrhu distribuce, ne
   v samotném pravidle.

### NEW — IMP-507: inventář testů neumí `--tier`, a proto hlásí dvojí běh, který se nekoná

**Status:** new (2026-08-14, nalezeno při simulaci nočního běhu)
**Priority:** medium — falešný nález v auditu je horší než žádný
**Class:** test audit fidelity
**Area:** `plugins/aid-orchestrator/scripts/aid-test-inventory.sh`

Do P081 (10. 8.) byla brána `bats_all` glob přes adresář s ručně udržovaným
záporným filtrem, který jmenoval dvě hraniční sady — inventář si tedy z
příkazu přečetl, že je vylučuje, a klasifikoval ji `bats_tree_runner/pool`.
P081 ten příkaz vyměnil za tierový runner
(`run-all-tests.sh --tier t0 && --tier t1`). Inventář ho čte jako
`aggregate_runner`, **patra neumí**, a proto do jeho členství započítá i obě
hraniční sady.

**Důsledek:** rekonciliace může ohlásit dvojí spuštění mezi `bats_all` a
`bats_boundary`. Je to nepravda — `bats_all` běží T0+T1 a obě hraniční sady
jsou T2 — a je to přesně ten druh falešného nálezu, kvůli kterému kdysi vznikl
P075 Step 3 (105 falešných hlášení o dvojím běhu).

**Náprava:** naučit inventář číst `--tier` z příkazu a filtrovat jednotky podle
značky patra (čtečka je v `lib/aid-test-tier.sh`). Klasifikace pak přestane být
odvozená z tvaru příkazu a začne odpovídat tomu, co runner opravdu vybere.

**Proč to nebylo hotové hned:** dva testy, které na tom visely, byly upraveny
tak, aby tvrdily dnešní pravdu a mezeru pojmenovaly (2026-08-14) — ale samotný
model zůstal. Oprava patří do nástroje, ne do testu.


### NEW — IMP-505: dvě nejdražší sady si staví výchozí stav znovu pro každý případ

**Status:** new (změřeno 2026-08-14)
**Priority:** high — je to 244 z 420 minut celého portfolia
**Class:** test cost
**Area:** `scripts/tests/bats/test-aid-plan-final-boundary.bats`,
`scripts/tests/bats/test-aid-plan-release-boundary.bats`

#### Co je naměřeno

| Sada | Případů | Čas | Na případ |
|---|---:|---:|---:|
| `test-aid-plan-final-boundary` | 261 | **199 min** (47 % portfolia) | **46 s** |
| `test-aid-plan-release-boundary` | 267 | 45 min (11 %) | 10 s |
| zbylých 200 sad dohromady | — | ≈ 175 min | — |

Kontrolní měření: 25 případů `AC3*` z té pomalé sady = **21 min 55 s**, tedy
**53 s na případ**. Sedí to na celkový čas.

#### Příčina — ne počet testů, ne jedno pomalé místo

198 z 261 případů volá `_seed_closable` → `_seed_merge_project`, což **odehraje
celý život plánu od nuly**: commit na větvi, dva EPICy se stavy, dvě reálné
operace uzavírání (`sync`, `freeze`), dva přechody automatu, dva posudky,
přepnutí větve, nasypání důkazů.

**Sourozenecká sada to nedělá** — její příprava plán jen založí jedním
příkazem — a běží 4,5× rychleji, **přestože spouští DVAKRÁT VÍC reálných
příkazů** (122 proti 59). Takže nerozhoduje, kolikrát se sáhne na produkční
kód. Rozhoduje, jestli se výchozí stav staví znovu, nebo se kopíruje.

Obecné pravidlo je ve standardu, sekce
[Cena přípravku](https://docs.aidlab.dev/ecosystem/specs/test-standard).

#### Navržený postup

Postavit stav **jednou za sadu** a pro každý případ ho **zkopírovat**
(`cp -a` adresáře projektu). U bats se to dělá přes `setup_file()` nebo
memoizací uvnitř `_seed_merge_project`.

#### Riziko, kvůli kterému to NENÍ hotové

**Ve stavu jsou absolutní cesty.** `setup_test_evidence_dir` dává každému
případu vlastní `mktemp -d`, takže zkopírovaný stav by odkazoval do cizího
adresáře. Tři cesty ven, každá s vlastní cenou:

1. **Přepsat cesty při kopírování** (`sed` přes stavové soubory) — funguje, ale
   je to křehké a tiše se to rozbije, až někdo přidá nové pole s cestou.
2. **Sjednotit adresář** pro celou sadu — mění izolaci mezi případy, což je
   přesně to, co dnes drží testy nezávislé.
3. **Kopírovat na STEJNOU cestu** — stav se obnoví do adresáře s týmž jménem,
   jaký měl při vzniku; vyžaduje deterministické jméno místo `mktemp`.

Rozhodnutí mezi nimi patří do té práce, ne sem.

#### Přejímací podmínky

- Všech **261 případů projde stejně jako dnes** — stejné výsledky, ne jen
  „zeleně".
- Sada běží **řádově rychleji**; číslo se změří, ne odhadne.
- Žádný případ nesdílí stav s jiným: náhodné pořadí (`--random`) dá tentýž
  výsledek. **Tohle je ta kontrola, která odhalí špatně provedenou izolaci.**
- Totéž pro druhou sadu.
- Nakonec **cross-model revize** — tenhle typ změny se dá udělat tiše špatně.

#### Proč to nebylo uděláno hned

Diagnóza vznikla na konci dlouhé relace, ve které tři opravy potřebovaly další
opravu. Tohle je změna s reálným způsobem, jak ji udělat nenápadně špatně
(sdílený stav mezi testy vypadá zeleně přesně do chvíle, kdy nevypadá), takže
patří do vlastního běhu s prostorem na ověření.

---

### NEW — IMP-506: `/aid-do` pozná velikost úkolu, ne jeho riziko

**Status:** new (2026-08-14, uzavření P061 — jediná nedodaná část plánu)
**Priority:** medium — obchází se tím jediná ochrana, kterou P061 stavěl jako
první, a to tichou cestou
**Class:** enforcement gap (detektor bez vynucení na jedné ze dvou vstupních cest)
**Area:** `commands/aid-do.md`, `scripts/lib/aid-gate-profile.sh`

**Co je špatně.** P061 D5 říká: Fast Mode musí používat **stejný** deterministický
klasifikátor rizika jako `/aid-run` — sdílenou funkci, ne druhou logiku. Dnes
`/aid-do` eskaluje podle **velikosti** úkolu (počet souborů, odhad času,
`aid_do_prefilter_fixloop`), ne podle toho, čeho se změna dotkla. Jednosouborová
úprava `aid-fsm.sh` je pro Fast Mode malý úkol; pro `/aid-run` je to
vysoké riziko, které povinně zvedne profil bran.

**Doloženo:** `gate_profile_resolve` má v produkci tři volající —
`aid-fsm.sh` (2×) a `aid-plan-fsm.sh` (1×). `/aid-do` mezi nimi není.
Hlavička `lib/aid-gate-profile.sh:137-144` přitom **popisuje přesně ten tvar
volání, který by Fast Mode potřeboval** („the exact shape a future EPIC 5
/aid-do call has: no fsm-state.yaml exists at all in Fast Mode") — kontrakt je
připravený, konzument nikdy nevznikl.

**Proč to není jen kosmetika.** Obě vstupní cesty vedou do stejného repozitáře.
Dokud jedna z nich riziko neměří, je vynucení na té druhé obejitelné volbou
příkazu — a nikdo si toho nevšimne, protože Fast Mode se chová přesně tak, jak
slibuje. To je princip #1 z `AID-v3-principles.md` na půl cesty.

**Náprava.** Zavolat `gate_profile_resolve` s vlastním souborem změněných cest
a bez `fsm_state_file` (dokumentovaný fallback je definovaný), a na výsledku
`full`/`release` buď eskalovat na `/aid-run`, nebo vyžádat výslovný PM override
s evidencí. Registry řádek k tomu patří, jinak vzniká druhý detektor bez
vynucení.

**Původ.** P061 EPIC 5, rozhodnutí D5. Druhá půlka E5 (D6 — reálné místo
spuštění profilu `release`) **dodaná je**, jen jinou z obou cest, které plán
připouštěl: ne v `aid-release.sh`, ale jako pojmenovaná fáze plan-final
(`aid-plan-fsm.sh:4530`, `max(plan_final_required_profile, release)`).

---

### NEW — IMP-508: jméno brány se ověří až u runneru, ne při převodu EPIC → plan.json

**Status:** new (doloženo 2026-08-16, adversariální review šablon po IMP-503)

`aid-epic-to-json.sh` krok 11 (`:778-800`) **schválně** neporovnává jména bran
ze sekce `## DoD Gates` proti `.aid-o/config/execution.yaml`. Komentář na místě
to zdůvodňuje a to zdůvodnění platí: dřívější natvrdo zadaný čtyřprvkový seznam
tiše zahazoval vlastní brány projektů (P061 E1 Krok 1), takže se filtrování
zrušilo a kontrola nechala na `aid-run-gates.sh`, kde je dynamická a hlásí
`undefined_gate`.

Runner tedy chybu **najde** — princip #1 je formálně naplněn. Cena je ale
vzdálenost: mezi „autor napsal jméno, které projekt nemá" a „autor se to
dozvěděl" leží celý převod, zapečetění `plan.json` a spuštění bran. A protože
`plan.json` je otiskem svázaný s FSM stavem, oprava jména není edit jednoho
řádku, ale přegenerování.

**Kde to jméno vzniká.** Po IMP-503 už ne v generátoru — ten si `dod_gates`
odvozuje z konfigurace. Zbývá **ruční cesta**: člověk nebo agent píšící EPIC
opisuje jména odjinud. Šablony byly 2026-08-16 upraveny tak, aby žádné jméno
k opsání nenabízely (`defaults/templates/epic.md`), respektive aby u vyplněné
ukázky stálo, že jde o jména výchozí konfigurace a projekt, který si je
přejmenoval, je opisovat nesmí (`epic-example.md`). **To je prosa, ne
vynucení** — přesně to, co principy zakazují vydávat za nápravu.

**Náprava.** Do kroku 11 (nebo do `aid-plan-lint.sh`) přidat kontrolu, která
každé přečtené jméno porovná proti `.gates | keys` konfigurace na **state rootu**
projektu (ne proti nejbližšímu `.aid-o` od CWD — viz IMP-497) a při neshodě
selže s výpisem bran, které projekt skutečně má. **Nesmí filtrovat** — to je
regrese, kterou P061 odstranil; smí jen odmítnout a poradit. Rozlišit „nelze
přečíst" od „projekt žádnou nemá" stejně, jako to už dělá generátor
(`aid-plan-to-epic.sh:1356-1371`).

**Registry.** Nová detekce = nový řádek v `enforcement-registry.yaml`
(`type: structural`, `surface: generation`, severita blokující — je to
konfigurační chyba, ne názor).

---

## Harness hooky — jedna vrstva místo deseti (2026-08-16)

Kontext: AID dnes používá VÝHRADNĚ git hooky (`defaults/hooks/pre-commit`
scope commitu podle kroku, `pre-push` bump verze). Harness hooky Claude Code
(31 událostí) nepoužívá vůbec. Ověřeno v dokumentaci 2026-08-16:
`SessionStart`/`UserPromptSubmit` vkládají stdout do kontextu modelu; `Stop`,
`SubagentStop`, `PreToolUse`, `PreCompact`, `UserPromptSubmit` umí blokovat
(exit 2 + důvod, resp. `permissionDecision: deny`); hooky dostávají na stdin
JSON (`session_id`, `cwd`, `agent_id`, `agent_type`, `tool_name`,
`tool_input`); plugin je deklaruje v `hooks/hooks.json` s `${CLAUDE_PLUGIN_ROOT}`
a **hooky pluginu běží i uvnitř subagentů**.

### IMP-509 — jednotná hooková vrstva AID (předpoklad pro všechny níže)

**Priorita:** high — bez tohoto vznikne deset různých skriptů
**Class:** architektura / vynucení

Jeden vstupní bod `scripts/aid-hook.sh <event>`, deklarovaný v
`hooks/hooks.json` pro každou použitou událost. Skript: přečte JSON ze stdin →
určí kontext (state root z `cwd`, controller vs. role agent z `agent_type`,
plán/worktree) → z registru `defaults/hook-registry.yaml` vybere pravidla
platná pro tuto událost a kontext → spustí je → sjednotí výstup (kontext na
stdout, nebo exit 2 s důvodem).

Povinné vlastnosti, jinak to bude past:
- **fail-open default**, fail-closed jen u výslovně označených pravidel
  (rozbitý hook nesmí zastavit práci);
- **časový rozpočet** per pravidlo i celkem (hook je v cestě každé zprávy);
- **vlastnictví**: pravidlo deklaruje, zda běží v controlleru, v subagentovi,
  nebo obojí — plugin hooky běží i v subagentech, takže výchozí „obojí" je chyba;
- **escape hatch** `AID_HOOKS_OFF=1` s auditním záznamem (fail-closed `Stop`
  hook umí zamknout session);
- **zápis do enforcement registry** (V9): každé pravidlo má typ/zdroj/
  závažnost/povrch;
- **testovatelnost**: pravidlo = funkce (JSON vstup → verdikt), bats fixtures
  bez živé session.

### IMP-510 — obnova kontraktu po kompakci (`PreCompact` + `SessionStart:compact`)

**Priorita:** high. `PreCompact` uloží „kapsli" (komunikační kontrakt, FSM stav,
další povolený krok, cesty ke stavovým artefaktům); `SessionStart` s matcherem
`compact|resume|fork` ji vloží zpět. Řeší ztrátu kontraktu a stavu po compactu —
dnes se po kompakci controller orientuje znovu a nákladně. Riziko: zastaralá
kapsle nasměruje na špatný krok → stav číst relativně k worktree, ne z mainu.

### IMP-511 — vynucení PM decision card na `Stop`

**Priorita:** high. Turn, který žádá rozhodnutí a nemá možnosti/doporučení/důvod,
se nepustí ven: exit 2 + konkrétní důvod, model kartu doplní. Vynucovací
polovina IMP-495 (V7). **Podmínka správného provedení:** validovat strojově
čitelný artefakt karty, ne volný text transcriptu — jinak vzniknou falešná
zamítnutí a smyčky. Fail-closed pravidlo → escape hatch povinný.

### IMP-512 — fail-closed ochrana neukončeného FSM (`Stop`, `SubagentStop`)

**Priorita:** high. Je-li běh v rozpracovaném stavu (krok bez výsledku, čekající
brána), `Stop` vrátí exit 2 s přesným dalším přechodem místo tichého konce.
Vlastnictví podle `session_id`/`agent_id`, jinak zablokuje cizího agenta.

### IMP-513 — scope editace už při zápisu (`PreToolUse`)

**Priorita:** medium-high. Zápis mimo `allowed_paths` aktuálního kroku se
odmítne (`permissionDecision: deny`) v okamžiku editace, ne až při commitu.
Git hook ZŮSTÁVÁ jako poslední nezávislá hranice — hook v harnessu se dá
obejít shellem, který parser nerozpozná.

### IMP-514 — konfigurace bran z větve + přeskočení už zelených (`PreToolUse`/`PostToolUse`)

**Priorita:** medium. Před během gate runneru vybrat konfiguraci z plánového
worktree (řeší IMP-497) a přeskočit brány, které jsou zelené na nezměněném
stromu (řeší IMP-498 část 1); po běhu atomicky zapsat výsledek + fingerprint.
Zapisovat smí jen controller.

### IMP-515 — živý protokol pro subagenty (`SubagentStart`) — PODMÍNĚNÉ

**Priorita:** medium, **nejdřív ověřit**. Cíl je IMP-179 (subagent čte protokol
z nainstalované cache, ne z repa). Otevřená otázka: `SubagentStart` stdout
podle dokumentace vkládá kontext, ale NENÍ ověřeno, že nahradí systémový prompt
subagenta. Ověřit experimentem dřív, než se na to spolehne; jinak to je most,
ne oprava, a správné řešení je v dispatchi.

### IMP-516 — registrace pracovních kopií (`WorktreeCreate`/`WorktreeRemove`)

**Priorita:** low-medium. Automaticky registrovat/odregistrovat plánové
worktree do stavu (V10), aby nevznikaly osiřelé adresáře, které dnes musí
PM uklízet ručně (`git worktree prune` scénář v `aid-plan.md`).

### Vědomě NEdělat hookem (oponentura Codexu, přijato)

- **Přepisovat systémové prompty subagentů** — hook to spolehlivě neumí; patří
  do dispatche.
- **Nahradit git hooky** pro scope commitu — git hook je nezávislá hranice.
- **FSM rozhodování v `UserPromptSubmit`** — blokování maže prompt uživatele,
  příliš invazivní.
- **`SessionEnd` jako pojistka dokončení** — neumí blokovat, tedy negarantuje nic.

### Analýza: kde jinde v AID mají hooky přidanou hodnotu (2026-08-16)

**Východisko z registru vynucení** (`defaults/enforcement-registry.yaml`,
205 řádků): 92 aktivních vnitřních strážců (kód), **44 aktivních `llm-facing`
pravidel, z toho 40 označených `blocking`** — tedy pravidel, která jsou
formálně závazná, ale vynucuje je jen to, že si je model přečte a dodrží.
Nejhustší zdroje: `skills/pipeline.md` (16), `agent-protocol.md`,
`role-cards.md`, `plan-writing.md`, `commands/aid-run.md`.
**Tohle je ta hromada „strojových kontrol místo vynucení":** kontroly běží AŽ
POTOM (gate, CP, audit) a nález pak stojí kolo oprav. Hook posouvá vynucení
na okamžik činu.

Sedm tříd použití, seřazeno podle přínosu:

| # | Třída | Událost | Co to řeší |
|---|---|---|---|
| 1 | **Doručení protokolu subagentovi** | `SubagentStart` | IMP-179: agent čte protokol z nainstalované cache pluginu, ne z repa; dnes se vkládá ručně do dispatche. Hook pluginu běží I v subagentovi a zná `agent_type` → doručí roli přesně její protokol z živého stromu. **Nejdřív ověřit** (IMP-515), zda stdout stačí vedle systémového promptu |
| 2 | **Kontrola výstupu agenta** | `SubagentStop` | 40 blokujících `llm-facing` pravidel typu „report musí obsahovat důkaz / vazbu na krok / zákaz fabrikace" se dnes ověřuje až u controlleru. Hook odmítne dokončení agenta, jehož report nemá povinná pole → chyba nevznikne, místo aby se pak hledala |
| 3 | **Zákaz akce v daném stavu** | `PreToolUse` | Scope editace (IMP-513) + třída „design defeat" z P035: `yq -i` nad `fsm-state.yaml`, `git commit --no-verify`, ruční zápis do evidence. Dnes to hlídají detektory po činu; hook to nepustí |
| 4 | **Kontinuita kontextu** | `PreCompact` + `SessionStart` | IMP-510: kontrakt a stav přežijí kompakci |
| 5 | **Konec turnu / konec agenta** | `Stop`, `SubagentStop` | IMP-511 (karta rozhodnutí), IMP-512 (nedokončený FSM) |
| 6 | **Životní cyklus prostředí** | `WorktreeCreate`, `WorktreeRemove`, `CwdChanged`, `ConfigChange` | IMP-516 (registrace worktree) + **detekce IMP-497 v okamžiku vzniku**: běžím v plánové kopii, ale konfigurace bran se čte z hlavního stromu → hook to pozná z `cwd` a upozorní hned, ne až pádem brány |
| 7 | **Telemetrie zadarmo** | `TaskCreated`, `TaskCompleted`, `PostToolUse` | **Změřeno 2026-08-16: `.aid-o/work/timeline.jsonl` má 0 řádků** — telemetrie, kterou nikdo nepíše, protože se spoléhá na to, že si ji agent zapíše sám. Hook ji zapíše vždy a bez proseb. Zároveň je to zdroj chybějících čísel z roadmapy §2 (přijaté nálezy, escape rate, cena řetězce) |

**Hlavní myšlenka celé vrstvy:** dnešní AID staví na detektorech, které běží po
činu, a proto potřebuje kola oprav. Hook mění „udělal jsi to špatně" na
„nedovolím ti to udělat špatně". Tím se část dnešních strojových kontrol dá
NEJEN doplnit, ale i **zrušit** — což je přesně V11 (méně kontrol, lépe
umístěných). Vyhodnotit při implementaci IMP-509: které z 92 vnitřních
strážců jsou po zavedení hooku redundantní.

**Hranice, na které je hook špatný nástroj:** neuvidí, co se děje uvnitř úvah
agenta; nenahradí git hooky (ty jsou nezávislá poslední hranice); a fail-closed
hook je jediné místo v AID, které umí zablokovat práci úplně — proto escape
hatch a fail-open default nejsou volba, ale podmínka.

### NEW — IMP-517: nová testová sada jmenovaná jen v Testing Strategy uteče povinnosti tieru

**Status:** new (nalezeno mezimodelovým posudkem zadrátování 2026-08-23)
**Priority:** medium
**Class:** enforcement gap / test integrity
**Area:** `scripts/aid-plan-to-epic.sh` (tier refusal), `scripts/aid-plan-lint.sh`

P084 zrušil povinnost `Test:` položky u každého kroku a nahradil ji sekcí
`## Testing Strategy`. Povinnost deklarovat patro u NOVÉ sady ale zůstala
navázaná **jen na `Test:` bullet** (`aid-plan-to-epic.sh`, refusal kolem
ř. 995-1010). Plán, který novou sadu jmenuje pouze v próze Testing Strategy
a žádný `Test:` bullet pro ni nemá, tedy patro deklarovat nemusí — a přesně
tomu měla ta povinnost zabránit („portfolio, které smí růst netierované,
roste zpátky na merge cestu").

**Náprava (návrh):** lint prohledá `## Testing Strategy` na jména sad
(`test-*.bats`, `test-*.sh`), která nejsou v žádném `Test:` bulletu, a u těch,
které v repozitáři neexistují, vyžádá buď bullet s tierem, nebo větu, proč
sada nevzniká. Poradní tier, ne blokující — jméno v próze je slabší signál
než deklarovaná cesta.

**Proč ne hned:** parsování jmen z prózy je heuristika a P084 i P085 právě
prošly dvěma koly oprav; přidávat další heuristiku do čerstvého kontraktu
bez měření by bylo přesně to, co V11 zakazuje.

### NEW — IMP-518: nedeklarovaný sdílený výstup projde souběhem bez povšimnutí

**Status:** new (nezávislý DONE posudek P087, 2026-08-25)
**Priority:** medium
**Class:** souběh / hranice izolace
**Area:** `scripts/aid-plan-parallel-check.sh`, `scripts/lib/aid-dispatch-contract.sh`

**Scénář, doslova podle posudku:** dva kroky jsou vyhodnoceny jako disjunktní
podle deklarovaných cest, oba běží ve vlastní pracovní kopii a oba **nepřímo**
změní tentýž centrální soubor — lockfile, generovaný registr, index dokumentace —
který v povolených cestách ani ve `Shared interfaces` není. Git konflikt vzniknout
nemusí (soubor se generuje shodně nebo ho mění jen jeden z nich), kontrola vln to
neuvidí, protože posuzuje deklaraci, a `turn_write_scope` je stupeň 3 a nezastaví.
Kontrakt to zachytí **až při návratu**, tedy po hodině práce — a zopakování
druhého kroku tutéž vedlejší změnu vyrobí znovu.

**Co už proti tomu stojí:** izolace v pracovní kopii (škoda se projeví jako
konflikt, ne jako poškozený strom), deklarace `Shared interfaces` (P087 Krok 3)
pro rozhraní, o kterých autor plánu ví, a validace návratu proti disku.
**Co chybí:** rozpoznání *implicitních* společných výstupů PŘED dispatchem.

**Návrh (nezačínat implementací):** po prvním souběžném běhu změřit, jak často
k tomu doopravdy dojde. Teprve podle čísla se rozhodnout mezi (a) seznamem
generovaných cest v konfiguraci projektu, (b) odvozením z historie gitu — soubory,
které se mění spolu s jinými, aniž je někdo deklaruje, a (c) ponecháním na
konfliktu, protože izolace to ustojí. PM v P087 seznam centrálních souborů
vědomě odmítl jako overengineering; tahle položka ho neotevírá znovu, jen drží
scénář zapsaný, dokud pro něj nebudou čísla.

### NEW — IMP-519: krok nemůže sáhnout do jiného repozitáře, i když to plán deklaruje

**Status:** new (CP1-deep P089, čočka L2, 2026-08-26)
**Priority:** medium
**Area:** `scripts/lib/aid-dispatch-contract.sh`, `scripts/aid-plan-lint.sh`

Krok, jehož `Files:` ukazují mimo repozitář plánu (typicky do `/opt/eco/docs`),
je při návratu odmítnut, i když agent práci udělal správně. Kontrakt ověřuje
očekávané artefakty jako `${root}/${cesta}` (`aid-dispatch-contract.sh:264`)
a změny sbírá `git -C "$root" status` (:273) - obojí nad repozitářem plánu.
**Reprodukováno** nad existujícím souborem: `verdict: reject`,
`reasons: ["expected artifacts are missing on disk: /opt/eco/docs/…"]`,
`out_of_scope: []` - refuzuje kontrola disku, ne kontrola rozsahu. Podpora pro
druhý kořen nikde není (hledáno `docs_root|external_root|documentation_root`).
`aid-plan-lint.sh` takový plán propustí, protože posuzuje TVAR cesty, ne to,
jestli leží v repozitáři, který kontrakt kontroluje.

**Nejlevnější první krok:** ať lint takovou cestu nahlásí. Plná podpora druhého
kořene (deklarace v `project.yaml`, per-root `git status`, commit v cizím repu)
je větší práce a chce vlastní plán. P089 se tomu vyhnul tím, že znění standardu
vzniká v repu AID a publikace je ruční krok.

### NEW — IMP-520: kontrola vln nevidí, že krok běží souběžně se svou závislostí

**Status:** new (CP1-deep P089, čočka L2, 2026-08-26)
**Priority:** medium
**Area:** `scripts/aid-plan-parallel-check.sh`

`aid-plan-parallel-check.sh` dokazuje jednu vlastnost: dva kroky v jedné vlně
nesdílejí soubor ani deklarované rozhraní (`:170`). Deklaraci `Depends on:`
nečte - jediný výskyt té fráze v souboru je komentář na `:114`. Vlna, která
obsahuje krok **i jeho závislost**, tedy projde jako PASS. V P089 to tak bylo:
vlna 3 nesla kroky 3, 4, 5, 6 a 13, přičemž Krok 6 závisí na 3-5 a Krok 13 na
1-12. Chytila to až CP1-deep čočka, tedy člověk/model, ne stroj.

**Návrh:** doplnit do kontroly druhou vlastnost - pro každou dvojici ve vlně
ověřit, že mezi nimi není hrana v grafu závislostí (`aid-source-plan-graph.sh`
ten graf už umí). Je to blokující třída: rozvrh, který porušuje vlastní
závislosti, pošle agenty na práci, jejíž podklad ještě neexistuje.

---

## Hlášení z konzumentských projektů, 2026-08-27 (plugin 2.94.0)

Dva projekty narazily týž den na 27 problémů. Zdroje zůstávají u nich a jsou
podrobnější než tenhle výpis:
`/opt/eco/projects/wan/.aid-o/work/aid-plugin-issues.md` (12 bodů, generování
EPICů a běh P099) a `/opt/eco/projects/acta/.aid-o/work/plugin-issues-P016.md`
(15 bodů, obnovení plánu pozastaveného měsíc; *soubor už na disku není - ověřeno
20. 9. 2026*). **Není to 27 nezávislých vad** —
sedají do šesti tříd a dvě z nich blokují práci právě teď.

### NEW — IMP-521: cesta mimo repozitář (WAN #12) — BLOKUJÍCÍ, a je to IMP-519 v ostrém provozu

**Status:** new | **Priority:** high | **Area:** `scripts/lib/aid-dispatch-contract.sh`

Totéž, co CP1-deep čočka P089 předpověděla jako [[IMP-519]] — teď doložené jako
zastavená práce. `aid_dispatch_contract_validate` skládá cestu jako
`[[ -e "${root}/${a}" ]]` (`:264`, ověřeno v 2.94.0), takže absolutní cesta se
slepí za kořen stromu:
`/opt/eco/projects/wan/.aid-worktrees/plan-P099//opt/eco/docs/docs/wan/mcp.md`.
Krok 11 plánu P099 má **dva** očekávané artefakty, jeden absolutní (repozitář
`docs`) a jeden relativní (repozitář `wan`); **žádná hodnota `root` je nesplní
obě naráz**. Kontrakt skončí `reject`, `increment-step` odmítne postoupit — ačkoliv
oba soubory na disku jsou a CP2 nad nimi dopadl `pass`. Obejít to nejde ani během
odjinud, protože `increment-step` si kořen odvozuje sám
(`_c_tree="$(git rev-parse --show-toplevel || pwd)"`). Jediná cesta dál je
`increment-step --force`, což je podle `commands/aid-run.md` vyhrazeno PM.

**Oprava, kterou navrhl hlásící projekt a která sedí:**
`[[ "$a" = /* ]] && p="$a" || p="${root}/${a}"`. Jeden řádek pro odblokování;
plná podpora druhého kořene (deklarace v `project.yaml`, per-root `git status`,
commit v cizím repu) zůstává [[IMP-519]] a chce vlastní plán.

### NEW — IMP-522: číselník rolí je za bránou, ne v lintu (WAN #2)

**Status:** new | **Priority:** high | **Area:** `scripts/aid-plan-lint.sh`, `scripts/aid-epic-to-json.sh`

Plán P099 prošel lintem, CP1-light, devíti čočkami CP1-deep, rozhodčím i
cross-provider revizí — a teprve `aid-auto-pipeline.sh` spadl na
`Invalid role 'fullstack' in step 2`. `VALID_ROLES` žije v
`aid-epic-to-json.sh:63` (ověřeno), tedy až **za** bránou. Stejně dopadly `docs`
(mělo být `docs-writer`) a `devops` (mělo být `release`).

Uzavřená množina patří do lintu plánu, kde autor stojí. Druhá půlka nálezu je
věcná: pro krok, který je backend i frontend, v číselníku **nic není**, takže
`fullstack` je přirozená volba — buď ji přidat, nebo lint musí poradit, jak krok
rozdělit.

### NEW — IMP-523: co se spotřebuje při běhu, který nic nevytvoří (WAN #4, WAN #6, ACTA #3)

**Status:** new | **Priority:** medium | **Area:** `scripts/aid-cp1-gate.sh`, `scripts/lib/aid-cp1-ledger.sh`

Tři hlášení, jedna třída: **nárok se potvrzuje pokusem, ne úspěchem.**
- PM override `cp1-pm-escalation-override.json` se přejmenuje na `.consumed-<epoch>`
  ve chvíli, kdy si ho brána vezme — i když běh vzápětí spadne na něčem jiném a
  žádný EPIC nevznikne. WAN takhle za den spotřeboval **čtyři** artefakty na jedno
  skutečné generování.
- Ledger C0 počítá **každý nový otisk plánu**, takže rozpočet 5/5 dojde přepisováním
  plánu, ne neúspěchy. WAN i tahle session na to narazily nezávisle (P090: 6 pokusů,
  z toho polovina byla přepis znění, ne nový posudek).

Nárok by se měl potvrzovat až úspěchem transakce. Poznámka pro implementaci: samo
`claim-pm-override` je „single-use claim primitive" sdílené víc volajícími, takže
změna se týká toho, KDY se claim volá, ne jeho atomicity.

### NEW — IMP-524: obřady kolem přepisu plánu (WAN #3, WAN #5)

**Status:** new | **Priority:** medium | **Area:** `scripts/aid-auto-pipeline.sh`, hook `plan_artifact_rendered`

Změna jediného řádku plánu změní otisk → identitu transakce → další běh skončí na
`generation transaction identity mismatch` a chce ruční `supersede-generation`
s dvacetiznakovým důvodem. Když předchozí transakce **nic nevytvořila** (všechny
fáze `not generated`), je to obřad — dala by se archivovat sama a jen to zalogovat.
Táž třída: `EPIC generation refused: P099 has no current PM page` po každém commitu
plánu; stránka by se mohla renderovat sama, když je starší než plán.

### NEW — IMP-525: `plan-start` nechá plán jen na větvi plánu (WAN #1)

**Status:** new | **Priority:** high | **Area:** `scripts/aid-plan-fsm.sh`

`plan-start` commitne plán i `.aid-lifecycle/manifests/<plan>.yaml` na větev
`plan/<id>`, ale generování je čte z `main` a odmítne dvěma nezávislými kontrolami.
Druhá hláška je **zavádějící**: radí „run from its plan worktree" ve chvíli, kdy
se právě z něj běží. Skutečná příčina je `aid_lifecycle_ensure_manifest` → rc=3
(manifest není na cílové větvi), a diagnostika to přeloží na problém s větví.
Obchází se ručním commitem na `main` — to má dělat `plan-start`.

### NEW — IMP-526: obnovení pozastaveného plánu (ACTA #1, #2, #7, #10, #12, #13, #14)

**Status:** new | **Priority:** high | **Area:** `scripts/aid-generation-readiness.sh`, `scripts/aid-auto-pipeline.sh`

ACTA obnovila plán P016 psaný 24. 7. a pozastavený po Stepu 10. Sedm hlášení
z toho jediného dne popisuje **jednu chybějící schopnost: plugin nemá pojem
„plán, který už z části běžel".** Projevy: readiness nezná „už hotový krok"
a nemá PM override (obojí označeno jako blokující); generation receipt je
celoplánový, takže mid-plan resume regeneruje hotové EPICy; stará `fsm-state.yaml`
přežije regeneraci EPICu s jiným počtem kroků; stará task branch bez lineage
zablokuje `epic-start`; a gramatika, kterou readiness vyžaduje, byla v době psaní
plánu volnější — plán tedy zestárl do nesouladu, aniž se změnil.

Tohle je největší položka ze všech dvaceti sedmi a **není to oprava, je to
schopnost**. Chce vlastní plán, ne řádek v backlogu.

### NEW — IMP-527: drobnosti, které stojí čas a nic neshodí (WAN #9, #10, #11, ACTA #4, #5, #8, #9, #15)

**Status:** new | **Priority:** low

- kroky se číslují od nuly v evidenci (`step-0-verify.md`, `classify 0`) a od
  jedničky všude, kde to čte člověk (WAN #9);
- `aid-run-gates.sh` značku `nightly: true` v `execution.yaml` **nečte** — jedinou
  obranou proti 28minutové noční bráně je `--profile standard`. Značka je dnes
  dokumentace, ne mechanismus (WAN #10);
- hook varuje **verifiéra** za zápis do jeho vlastního evidence adresáře: rozsah
  kroku je rozsah implementátora a neměl by se vztahovat na revizní agenty (WAN #11);
- „model at capacity" se maskuje jako `rate_limited` (ACTA #4);
- Codex občas nezkopíruje `input_manifest_hash` a celé kolo propadne (ACTA #5) —
  tahle session to viděla taky, jako `ledger_increment_failed`;
- `Parallel group` je v lintu ADVISORY, ale v readiness FATAL (ACTA #8);
- dopředné závislosti jsou zakázané, ale nic to neřekne při psaní (ACTA #9);
- `--output-dir` u `aid-epic-to-json.sh` se snadno splete (ACTA #15).

### Co z toho plyne

Šest tříd, ne 27 vad. Dvě blokují teď: **IMP-521** (jeden řádek) a **IMP-526**
(chybějící schopnost, vlastní plán). Zbytek je práce na jedno odpoledne, kromě
IMP-523, kde je potřeba rozmyslet, kdy se nárok potvrzuje.

Poznámka k původu: šest z 27 bodů předpověděly ceremonie tohohle repozitáře dřív,
než na ně někdo narazil ([[IMP-519]] → WAN #12, vyčerpatelný ledger → WAN #6).
Předpověď bez opravy ale nikoho neodblokuje.

### NEW — IMP-528: hooky hlásí cizí běhy všem oknům v projektu

**Status:** new (PM 2026-08-28) | **Priority:** high
**Area:** `scripts/aid-hook.sh`, `scripts/aid-fsm.sh` (active-runs), `defaults/hook-registry.yaml`

Hook v KAŽDÉ session přečte `.aid-o/work/active-runs.json` a hlásí VŠECHNY běhy
v projektu — včetně těch, které patří jiné session. Ta pak dostane výzvu, aby
dokončila práci, do které nevidí a která možná právě běží jinde.

**Doloženo v kódu:**
- `aid-hook.sh:304` čte `session_id` ze vstupu do `HOOK_SESSION_ID`, ale ta
  proměnná se používá **jen v auditním řádku** (`:98`). Podle ní se nikde
  nefiltruje.
- `active-runs.json` **nenese vlastníka**: pole jsou `auto_controller`, `branch`,
  `governs_main`, `plan_id`, `resume_artifact`, `run_id`, `state`, `state_file`,
  `updated_at`. Žádné `session_id`, žádný `owner`.

**Doloženo v provozu (tahle session, 28. 8.):** hook opakovaně vyzval k dokončení
běhu `E-001-2_2`, který jede v jiném okně a mezitím se posunul z kroku 2 na 3 —
tedy živá cizí práce hlášená jako zaseknutá povinnost. Jediná „oprava", která se
nabízela, bylo přepnout cizímu běhu `auto_controller` na `blocked_for_pm`, což by
tomu druhému oknu **zabralo řízení**.

**Co se tím kazí:** PM to spamuje ve všech oknech; agent je tlačen dokončovat
cizí práci; a pravidlo, které mělo hlídat vlastní rozdělanou věc, se stane šumem,
který se ignoruje — tedy přesně to, co vynucení nemá dělat.

**Návrh:** běh si při `active-runs init` zapíše session, která ho vlastní. Hook
pak hlásí jen běhy své session; cizí nanejvýš JEDNOU zmíní jako kontext ("v tomhle
projektu běží ještě X"), nikdy jako výzvu k dokončení. Chybějící vlastník
(záznamy z doby před opravou) se čte jako „není můj" — hlásit cizí práci jako
svou je horší chyba než mlčet.

### NEW — IMP-529: rozpoznat z transkriptu, co session zapsala, jde jen přibližně

**Status:** new (2026-08-28, po pěti kolech nezávislého posudku) | **Priority:** low
**Area:** `scripts/lib/aid-artifact-obligation.sh`

Milníkové pravidlo musí vědět, které plány tahle session **zapsala**. Jediný
per-session záznam, který hook dostane, je přepis konverzace, takže se to
odvozuje z něj. Pět kol posudku vyrobilo pět stále užších verzí a ke každé
protipříklad:

| verze | co pouštěla navíc / co míjela |
|---|---|
| podle času (`find -newer`) | plány, které měnilo jiné okno |
| podle zmínky (`P###`) | plány, o kterých se jen mluvilo (299 zmínek P062) |
| řádkový grep zápisu | `{"file_path":"notes.txt","content":"…P062…"}`; míjela `{"name": "Write"}` s mezerou |
| whitelist zápisu | `cat plán.md \| tee jinam` |
| whitelist vázaný na segment | `echo "tee .aid-o/plans/P900.md"` |

**Poslední zůstává otevřený a nezavře ho žádná další heuristika**: statickou
inspekcí řetězce se nedá rozhodnout, co příkaz zapíše. Zavřel by ho jen
skutečný záznam zápisu — například kdyby hook `PostToolUse` zapisoval dotčené
cesty do úložiště session, a milníkové pravidlo pak četlo fakt místo odhadu.
To je vlastní práce, ne úprava tohohle pravidla.

**Proč to dnes nevadí tolik, kolik by mohlo:** obranou proti šumu není přesnost
detekce, ale to, že se týž nález ohlásí **jednou za session**. Chybný zásah tedy
stojí jeden řádek za celou session, ne jeden na každém tahu — a to byla ta
skutečná stížnost.
