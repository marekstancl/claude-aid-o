...
<!-- Numbering note: P076 allocated IMP-476..480 (plus IMP-481, added at its CP2
...
### NEW — IMP-476: fire-and-return ASYNC gates — a runner that returns before the job does
...
### NEW — IMP-477: services↔resource-map — teach the classifier about per-run service ports
...
### NEW — IMP-478: foreground gates get no `timeout -k` — a SIGTERM-ignoring suite still hangs
...
### NEW — IMP-479: migrate the visual-companion server onto `lib/aid-service.sh`
...
### NEW — IMP-480: host push-continuation adapter — make task-notification resume a verified mechanism
...
### NEW — IMP-481: a non-numeric CHANGELOG heading makes `aid-release.sh` rename the LAST SHIPPED release
