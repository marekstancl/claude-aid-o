...
- `plugins/aid-orchestrator/defaults/schemas/acceptance-evidence.schema.json`
...
- `plugins/aid-orchestrator/scripts/lib/aid-c0-plan-review.sh (lines ~300-360)`
...
- `plugins/aid-orchestrator/scripts/tests/bats/test-c0-plan-review.bats (line ~383)`
...
- Modify: `plugins/aid-orchestrator/scripts/lib/aid-c0-plan-review.sh` (lines ~300-360) — treat `plan-graph.json` as an optional manifest input at plan-review time, closing CF3, and resolve plugin-relative contract paths.
- Modify: `plugins/aid-orchestrator/scripts/tests/bats/test-c0-plan-review.bats` (line ~383) — regenerate the golden manifest fixture and expected `input_hash` for the new `plan_graph` shape, plus an absent-graph case.
...
- Modify: `plugins/aid-orchestrator/defaults/schemas/acceptance-evidence.schema.json` — same string-or-null widening of `identity.epic_id` for the plan-level aggregate.
...
`plugins/aid-orchestrator/defaults/enforcement-registry.yaml` (1114 lines,
git-tracked). The copy under `docs/plans/archive/AID-audit-2026-06/` is a
...
- [ ] [backend] CF3 closed, asserted on the bridge rather than on a review verdict:
...
- [ ] [qa] The plan-final `gates_report.json` carries `profile: "release"` and
- [ ] [qa] No gate that is `required: true` or plan-declared appears in
- [ ] [qa] `_command_log` is non-empty and every entry has a non-null duration.
...
- [ ] [qa] A `result: skip` on any gate that is `required: true` fails the stage
...
<!-- Auto-generated by aid-plan-to-epic.sh from P068-plan-final-release-boundary.md on 2026-07-21 -->
