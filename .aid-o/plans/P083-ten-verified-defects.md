...
**Error Handling:** A configured pattern that matches nothing is reported per file and per row, by name, and is not printed as `Updated`. **Two no-match cases must be told apart, because `sed` cannot tell them apart** (the C0 review's point): a row whose target is ALREADY at the new version is an expected no-match and is reported as `already current`; a row that matches neither the old nor the new version is a MISS and is reported as one. The distinction is made by probing for the new version before substituting, not by guessing from the exit code. Both `already current` and `miss` are, in addition to not printing `Updated`, excluded from the loop's collector at `:589` — the round-5 C0 finding that a suppressed message still fed the shared `UPDATED[]`/count/rollback/staging pipeline is what this step now closes directly, rather than assuming a later step's array correction reaches a loop it never lists. Today `aid-release.sh:576` prints `Updated: $FILE_PATH (regex)` whether or not the `sed` changed a byte, and `:589` collects it regardless, which is how this survived fourteen releases.
...
**Ordering obligation — re-checked 2026-08-13 after the round-5 C0 finding that this was prose with no mechanical backing.** The config repair merges to main BEFORE the runner refusal ships, and any live worktree refreshes from main before its next gate run. `.aid-o/config/execution.yaml` is tracked but each worktree carries its own snapshot copy. No plan-P080 worktree is live at the time of this revision (`git worktree list` shows only this checkout's own worktree), so there is no currently-named casualty — but the obligation is general, not tied to that one worktree, and the round-5 finding is right that "Depends on: none" understated it. The mechanical check this obligation now requires is named below and its output is evidence, not a claim.
...
**Dependencies:**
- Depends on: none (no other STEP in this plan produces an input this step consumes — this is a statement about plan-internal step ordering only).
- Blocks: none — but "Depends on: none" must not be read as "no prerequisite exists"; the config repair and the runner refusal are staged as two commits within this SAME step precisely because a real external prerequisite exists (every live worktree's `execution.yaml` must already define a command before the refusal can ship), and AC2 below is the finite, machine-verifiable condition that gates the second commit, not prose.
...
- [ ] AC2 is a two-part, ordered check, and both parts are captured as evidence in this step's commit: (a) `aid-worktree-report.sh` enumerates every live worktree; (b) for each one, `yq` resolves every gate named in every profile's `include[]` against that worktree's own `execution.yaml` and confirms a non-null `command` for each — not merely that the worktree is listed. If (b) fails for any worktree, that worktree's `execution.yaml` is repaired (or the worktree is closed) BEFORE the commit carrying the runner refusal is created; a refusal that lands on a tree which cannot satisfy it is the same destruction this plan already caused once (IMP-497). At the time of this revision no worktree other than the main checkout is live, so (b) is expected to pass trivially — the check runs anyway, because "currently no other worktrees" is a fact about today, not a substitute for the mechanism.
...
- [ ] A fixture copy of the live baseline, captured and committed at implementation time (not read live from the mutable runtime path), yields numerically identical percentiles before and after the deletion.
...
verification_pattern:
  ...
  expected_exit: 0
```
- [ ] AC4: A configured version pattern that matches nothing is reported by name instead of counted as an update — counted meaning absent from `UPDATED[]`, the printed total, and the rollback/staging sets, not merely absent from the printed `Updated:` line. (NOT "the tagline pattern is fixed" — that repair is Deferred, and an AC claiming it would pass while the real pattern stayed broken.)
```yaml
verification_pattern:
  type: cmd
