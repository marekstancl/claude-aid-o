...
Tier assignment for this plan's suites, chosen so the enforcement claims in Success Criteria are literally true:
- **T0/T1 (merge-blocking)** — `test-help-index-coverage.bats` (Success Criterion 1 says a new command "fails CI until intentionally indexed"; that is FALSE unless this suite is on the merge path), `test-enforcement-registry-cites.sh`, `test-communication-wiring.sh`, `test-fsm-step-render.bats` (already tagged — its existing tier is preserved, not lowered), `test-aid-config-summary.bats`, `test-aid-artifact-render.bats`, `test-aid-gate-outcome-summary.bats`, `test-aid-plan-close-summary.bats`.
- **T2 (nightly)** — `test-init-idempotency.sh` and `test-integration-handoff-rendering.sh`, both of which build real fixture repos and are correspondingly slow; their claims are regression-shaped, not merge-gate-shaped.

Every step that creates a suite states its tier tag in the file's header, and the step's ACs are worded against the tier it actually gets. Step 16's registry rows record the tier in the row's surface/severity wording, so a row can never imply merge-blocking enforcement for a nightly-only suite.
