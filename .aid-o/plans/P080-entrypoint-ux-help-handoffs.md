...
- AID artifact template family per the ecosystem artifact standard: a deterministic artifact renderer library with template-enforced caps and computed tiles, templates shipped in the plugin, and an AID artifact-template spec page **authored and tracked in this repo, then handed to the PM for publication** into the `/opt/eco/docs` Docusaurus site. The publication itself is a PM-owned handoff in a separate repository, deliberately NOT claimed as this plan's outcome — see Step 16.
...
**Implementation Detail:** **The bijection is over PUBLIC rows only** (C0 cross-provider review found the two requirements mutually exclusive as first written: Step 1 mandates an `intentionally_internal` row for every future `user_invocable: false` command, while the enumerator by definition never sees those, so any such row would break a whole-file equality check). Rows are partitioned by disposition before any comparison: `current|update|index_only` are PUBLIC and take part in the bijection; `intentionally_internal` and `remove_or_deprecate` are asserted separately.

**`index_only` is a TRANSITIONAL state, not a resting place, and the test enforces that** (C0 round 7: as first written, `index_only` was exempt from routing assertions, so the four surfaces this plan exists to route could sit indexed-but-unrouted with the suite green — and any future public command could be parked there forever, contradicting Success Criteria 1 and 2). The rule: **a row whose `command` corresponds to a `user_invocable: true` surface may NOT be `index_only` once Step 3 has landed.** Step 1 uses `index_only` only for the four rows Step 3 immediately flips to `current`; from Step 3 onward the test asserts that every enumerated public surface's row is `current` or `update` and therefore carries a topic route. `index_only` remains legal solely for a public surface that is not a `user_invocable: true` command — and if no such surface exists, the value is simply never used. Semantics of the two non-public dispositions, stated so neither is a silent hole: an `intentionally_internal` row's `file` must exist, it must NOT appear in aid-help.md (case 5), and it takes no topic route; a `remove_or_deprecate` row's `file` must exist and it takes no topic route, but it MAY appear in help with a deprecation note — its purpose is to mark a surface scheduled for removal, and when the file is finally deleted the row goes with it.

Bats cases: (1) enumerated set == the PUBLIC subset of index `command` values (diff both directions, name missing/extra entries); (2) uniqueness of slash names in the index; (3) for every row with disposition `current|update`: its `topic` value appears as a literal `### Topic: <topic>` heading in `commands/aid-help.md` AND the command's slash name appears somewhere in that topic's section body (extract section with awk from `### Topic: <t>` to next `### ` or EOF); (4) for every advertised topic in help's router table: a matching `### Topic:` section exists (kills the advertised-but-missing class); (5) `intentionally_internal` rows do NOT appear anywhere in aid-help.md outside fenced code blocks (reuse the `fenced_stripped()` awk pattern from `aid-lint-skill.sh:39-44`); (6) every `file` column path exists, for public and non-public rows alike; (7) every `intentionally_internal` and `remove_or_deprecate` row has an existing `file` and no topic route, asserted separately from the bijection. The test DERIVES and exports the plugin root in its own `setup()` — `AID_PLUGIN_PATH="$(cd "$BATS_TEST_DIRNAME/../../.." && pwd)"; export AID_PLUGIN_PATH` — the pattern every sibling suite uses (live precedent: `test-aid-fsm.bats:13-15`). The variable is never assumed ambient. The previously cited `test-aid-test-scheduler.bats:26` precedent is dead: P078 deleted that file.
...
- [ ] Adding a dummy `commands/aid-x.md` with `user_invocable: true` in a fixture makes the suite fail with the surface named — and **fails again when that dummy is given an `index_only` row**, proving a public command cannot be parked as indexed-but-unrouted (the negative control is worthless if the cheapest way to silence it is a disposition value).
...
- [ ] The command table in the repo-root `README.md` lists all 13 public surfaces.
...
- [ ] `bash scripts/aid-lint-skill.sh commands/aid-init.md` and the same for `commands/aid-setup.md` still report zero findings after this step's edits — Step 5 delisted both from GRANDFATHERED, so a finding introduced here is blocking and no longer downgraded to advisory.
...
**Objective:** Fixture-based proof of **the scripted substrate of** init's contract: for the operations init delegates to shipped libraries, fresh creation matches the declared item set, a re-run changes zero bytes outside the declared exception, and a declined upgrade leaves the tree untouched. **This step deliberately does NOT claim to prove `/aid-init` as a whole is idempotent** (C0 round 4) — init is LLM-executed prose, and any operation it performs inline rather than through a library (hook installation is the concrete case at this HEAD) is outside what any harness here can observe. Extracting hook installation into a shared library would close that gap and is the right eventual fix; it is explicitly OUT of this plan's scope and recorded as a named follow-up, rather than being papered over by a harness that replays its own reimplementation and calls the result product coverage.
...
**Architecture Context:** Closes the verified absence "no workspace-level init idempotency test". The test cannot execute LLM prose; it pins the scripted substrate and the file manifest, which is where every past idempotency regression lived.

**What this harness does and does NOT prove — stated in its own header, because a test that overstates its reach is worse than none** (C0 round 2). It pins the two operations that genuinely HAVE a shipped library — `lib/aid-init-execution-yaml.sh` (compose) and `lib/aid-gitignore-backfill.sh` (backfill) — plus the declared file manifest. **Hook installation has NO library at this HEAD** (`defaults/hooks/pre-commit` and `pre-push` are payload files that aid-init.md's prose installs), so a harness-side reimplementation of hook install would test the harness, not the product; the marker-idempotency case is therefore written against `defaults/hooks/*` as fixture INPUT with the marker-replace semantics asserted on the harness's own replay, and the header states plainly that it cannot detect drift in aid-init.md's prose instructions. The doc-vs-test cross-check on the manifest count is what couples the prose to the test; anything beyond that is out of reach until hook install gets a library (recorded as a follow-up, not silently implied).
...
**Publication boundary — stated explicitly rather than implied** (C0 cross-provider review: the plan promised a publication outcome nothing in it could verify). This plan delivers TWO things and claims only those two: (1) deterministic artifact-body generation, fully tested at script level, and (2) instruction wiring that tells the controller to publish before presenting, verified by this grep. **Live publication through the session's Artifact tool is an external controller responsibility, outside every test boundary in this plan** — exactly the boundary the shipped audit renderer already draws (`scripts/lib/aid-test-audit-chat-summary.sh` treats actual session presentation as outside its script-level tests). No step may state or imply that a page was published; the registry row is worded as a wiring-PRESENCE guard, and the plan's Success Criteria say "renders and is wired to publish", never "is published"; (b) superseded fragments are absent: the metrics-first DONE-review block header pattern in aid-run.md (grounded ~lines 352-360), the hardcoded "in **Czech**" requirement in BOTH `aid-verify-implementation.md` (:59, :149) and `aid-verify-plan.md` (:33, :122) — verified at CP1 as four identical lines across two files — and duplicate card-shape definitions outside communication.md (grep for the distinctive card line `Potřebuji tvoje rozhodnutí:` — exactly one file may define it).
...
**Which blocks are mandatory and which are conditional — matched to the cited standard rather than over-tightened** (C0 round 4: blocks 5 and 7 are conditional in the standard, so requiring all seven unconditionally would make the generic library contradict the contract it implements). Blocks 1-4 and 6 are ALWAYS rendered — block 6 in particular is never omitted, and renders its "nothing needed" wording when the input asks for nothing, because a silently absent ask block reads as "nothing is required of me". Block 5 (related links) renders only when links exist; block 7 (detail link) renders only when a detail target exists, which is an EXPLICIT input to the call rather than an assumption. Both renderers in this plan happen to always supply a detail target, and each states and tests that for itself — it is not imposed on the generic library. Caps enforced in code: ≤5 result items, ≤3 next steps, ~220 chars/sentence soft-wrap, overflow truncated with an explicit "and N more in the technical detail" line. Tiles come ONLY from `facts_json` (computed by callers from canonical JSON); `prose_json` carries the bounded model-written blocks; a missing prose block renders the standard "summary missing — the numbers below are computed and valid" warning instead of failing.

**Secret policy at the renderer boundary — not just HTML escaping** (C0 round 3: the ecosystem artifact standard forbids passwords, tokens, keys or fragments of them in any artifact, and gate `output` fields carry arbitrary command output straight from the run). The lib applies a deterministic secret scan to EVERY input it renders — `facts_json`, `prose_json`, and any embedded command output — before writing a byte. **Matches are redacted, not failed on** (`<redacted:<detector-name>>`), because failing closed at the gate-outcome boundary would suppress precisely the message telling the PM a run broke; but the redaction is COUNTED and the count is rendered in the artifact's provenance footer, so a redaction can never be silent. Detector set is declared in the lib header (the shipped patterns: `ghp_`/`github_pat_`, `sk-`, AWS `AKIA`, PEM `BEGIN * PRIVATE KEY`, `Bearer <token>`, `password=`/`token=`/`api_key=` assignments, and long high-entropy base64/hex runs). Escaping is applied AFTER redaction, never instead of it.
- Create: `plugins/aid-orchestrator/defaults/templates/artifact-outcome.html` — the generic outcome template (used by gate/waiver and plan-close renderers). **The compatibility contract is stated as testable structure, not as the unverifiable adjective "visually identical"** (C0 round 4: greps over a few CSS selectors pass for a materially different layout). The binding requirements are: (1) the vendored CSS block is byte-identical to the source's, asserted by a checked-in `sha256` of the extracted style block plus the source path and revision recorded in the template header — so an upstream restyle shows up as a failing test rather than silent drift; (2) the block ORDER and the tile slot names match the source's; (3) the theme guards are present in all three states (bare `:root`, `prefers-color-scheme: dark` guarded against an explicit light choice, and `[data-theme="dark"]`). AID changes only content slots, never layout. The HTML/CSS is vendored from `/opt/eco/services/agent-runtime/agent-report.py` (the standard's named template source — CSS custom properties with the three-state light/dark theme guards, masthead with eyebrow + serif title + timestamp, tile grid with `state-ok|state-warn|state-critical` left-border tiles, uppercase section labels, the split good/bad columns, the accent `ask` block for "Co se čeká ode mě", the `golink` detail button, provenance footer). Block order, tile slots (result / duration / scope / unresolved) and placeholder grammar `{{fact:key}}` / `{{prose:key}}` documented in a header comment.
- Create: `plugins/aid-orchestrator/scripts/tests/bats/test-aid-artifact-render.bats` — golden fixtures: full facts+prose renders all seven blocks in order; empty prose renders the warning block; overflow list of 9 items renders 5 + "and 4 more"; block 6 present even when input says nothing is needed; **no-links input omits block 5 and no-detail input omits block 7** (the conditional halves of the contract, otherwise untested); the body contains no `<html>`/`<head>`/`<body>` tag and no external URL in any `src`/`href`/`@import` (CSP compliance, asserted mechanically).

**Architecture Context:** Implements the ecosystem artifact standard (`/opt/eco/docs/docs/ecosystem/specs/artifact-standard.md`) inside the plugin so consumer projects work standalone: the 7-block skeleton with its two conditional blocks as the standard defines them, "brevity enforced by template not request", computed-not-claimed numbers, one-A4 target with detail linked, the no-secrets rule enforced by the redactor above, and internal-audience rules. The publication channel (Claude Artifacts via the controller's Artifact tool) is deliberately outside this lib — it produces the BODY; the controller instruction (Steps 11-12) owns publication, mirroring the shipped audit renderer's Artifact-first banner precedent.
...
- [ ] The rendered golden artifact contains blocks 1-4 and 6 unconditionally and blocks 5/7 exactly when their inputs are present, in the standard's order; the no-links and no-detail fixtures prove the conditional halves.
- [ ] The vendored style block matches its source byte-for-byte: the template header records the source path + revision and a `sha256`, and the test recomputes it (an upstream restyle must fail this test, not drift silently past a selector grep).
- [ ] The rendered body carries no `<html>`/`<head>`/`<body>` tag and no external URL in any `src`/`href`/`@import` (Artifact tool + CSP contract).
- [ ] Block 6 renders for the nothing-needed case, asserted deterministically: the lib defines ONE fixed literal for that state (declared in its header, e.g. `Nic — ozvu se, až bude hotovo`), the nothing-needed fixture is rendered to a named path inside the bats temp dir, and the test greps that exact literal in that file. No `<golden output>` placeholder and no "or its English equivalent" — one literal, one expected value.
...
**Architecture Context:** First consumer of the artifact layer. Input is the ordinary gate run's canonical artifact produced by `scripts/aid-run-gates.sh` (:1629). **Its path is not a constant and the renderer must not hard-code one** (C0 round 3): the runner accepts an arbitrary `--report-file`, and the repo contains BOTH a nested layout (`<run_dir>/gates/gates_report.json`) and a flat one (used by plan-final flows). Path-resolution contract, in precedence order: (1) the exact runner-reported `--report-file` path when the caller has it — this is the preferred wiring and the controller instruction passes it explicitly; (2) nested `<run_dir>/gates/gates_report.json`; (3) flat `<run_dir>/gates_report.json`. Resolution stops at the first hit and the chosen path is echoed in the artifact's provenance footer, so which report was rendered is never ambiguous. Corrected at CP1: `lib/aid-run-gates-report.sh` exposes only `merge_escalation_report()` and produces a report ONLY on a targeted→full escalation, so it is not the canonical input — the renderer must additionally ACCEPT that escalation-shaped variant, but the ordinary report is the primary. The renderer re-derives nothing. A recommendation is never represented as fact and a waiver never as a passing gate — this is the regression rule §7/4-5 of the source doc, enforced by fixture assertions rather than prose.

**Implementation Detail:** Facts extraction with `jq 'to_entries'` over the report's `.gates` **object** (keyed by gate name — verified at CP1: it is a map, not a row array), whose `result` enum is `pass|fail|skip|profile_excluded|waived`. **`waived` is a first-class row result and the report is the PRIMARY waiver source, not the receipts directory** (C0 cross-provider review corrected this): `aid-run-gates.sh` rewrites a waived row to `result: "waived"` with a `waiver_ref` (:2174-2176) and surfaces the canonical top-level `waived_gates[]` array (:1692-1694, :2512-2537) precisely so nothing downstream has to reconstruct waivers. The renderer reads `waived_gates[]` + the rows' `waiver_ref`; the optional receipts dir only ENRICHES detail (expiry, verdict word) and its absence must never hide a waiver. **Card selection reads the envelope's `.overall`, never a per-gate verdict.** Verified at CP1: gate rows carry `{gate, result, reason, exit_code, duration_ms, output, attempts}` and have NO `required` key — required-ness lives in `execution.yaml` and survives only in `.overall`, which `aid-run-gates.sh` already computes correctly (a failing non-required gate leaves `overall=pass` at :2001/:2259, a waived required failure is treated as a pass at :2145, and `skip`/`profile_excluded` never affect it). So: `.overall == "fail"` → Blocked card; otherwise Finished card. Deriving the card from individual `result` values would tell the PM a run is blocked while the FSM advances — the exact mismatch this rule exists to prevent. Individual `skip` and `profile_excluded` rows are reported in the core table and never counted as failures. Waiver facts from the waiver receipts dir when passed (existing waiver verdict vocabulary per `aid-gate-waiver.sh`: valid/consumed/expired…). On the Blocked branch, the `Doporučené řešení` line is derived from what the gate row actually carries — corrected at CP1: gate definitions have only `description`/`required`/`command`/`timeout_seconds`/`pass_criteria`, there is **no** registered fix-loop/remediation field anywhere, so the line renders the gate's own `command` as the reproduction step plus the exact public force command from the P073 surface as the risk-acceptance line. Duration tile = sum of rows' duration_ms rendered human (m/s).

**Error Handling:** Missing/invalid report → exit 1 with a one-line error; the instruction wiring states the controller then falls back to a Blocked card and must say the renderer failed (never silently skip the boundary message). **The fallback path is redacted on the same terms as the happy path** (C0 round 4: as first written it told the controller to hand-write a card from raw gate output, which is exactly the arbitrary text the renderer redacts — a bypass that would defeat the whole secret policy). The fallback card is built from BOUNDED COMPUTED FACTS only (gate names, results, exit codes, counts); if any raw-derived text is included at all, it must first pass through the same deterministic redactor the lib exposes as a callable entry point for precisely this case. No path from gate output to a human surface skips redaction.
...
- [ ] A fixture carrying ONLY the current report shape (a `result: "waived"` row plus `waived_gates[]`, with **no** waiver directory passed at all) still renders the waiver visibly and never as a pass.
...
- [ ] Fixtures cover all three report locations — explicit `--report-file` path, nested, and flat — plus the escalation-shaped variant; each renders and names the resolved path in the provenance footer.
...
**Implementation Detail:** jq extraction from the two declared inputs; the Decision card's `Proč teď` line = the reason implied by `release_ready`/`merge_mode`; `Riziko` = the brief's unresolved `blockers` list (cap 5 via the artifact lib). Tag status tile uses `release_tag_status` from `plan_summary` verbatim; the shipped vocabulary is `not_tagged` / `none` / `v<version>` (CP1 verified there is no `pending` and no `tagged` value, and no `options` field anywhere in the brief). Fixtures must be built from a real PLAN-mode `release-decision.json` shape — note that every one of the 13 such artifacts currently in the repo is EPIC-mode and therefore carries `plan_summary: null`, so the fixture is hand-authored from the producer's field set and the step says so. The option set is DERIVED from `merge_mode` + `release_ready`, not read from the brief. **It is reduced to the options the shipped API actually supports** (C0 cross-provider review: there is no plan-level defer and no abandon command at this HEAD). The three real commands, verified in `scripts/aid-plan-fsm.sh`, with every dynamic argument the card must fill:
- merge — `aid-plan-fsm.sh plan-merge-to-main <plan_id> --decision <path-to-release-decision.json>` (:7006)
- close — `aid-plan-fsm.sh plan-close <plan_id>` (:7884)
- rollback — `aid-plan-fsm.sh plan-rollback <plan_id> --revert-commit <sha> --reason <text>` (:10149). This option is offered ONLY when `final_merge_sha` is non-null, because the revert commit has no other source in the renderer's inputs; while it is null the card says rollback is not applicable rather than printing an uninvocable command.

"Defer" is expressed as taking no action (the plan simply stays open), never as a fabricated command. This satisfies §14's "the exact public command/decision" rule for the risk path.
...
- [ ] The pending-decision fixture's card contains `Doporučení:` and asserts the EXACT rendered invocation of every offered option (byte-compare against the expected string), not merely "at least one command".
- [ ] A fixture with `final_merge_sha: null` renders no rollback command at all; a fixture with a merge SHA renders `plan-rollback <plan_id> --revert-commit <that sha>` with the SHA interpolated from the input.
...
- [ ] The full definition survives in exactly one place and the other five are references — asserted with a UNIQUE full-definition literal, not an occurrence count (a bare `grep -rc` prints per-file counts and can prove neither half of the claim): `grep -rl 'executing_step = min(current_step + 1, total_steps)' plugins/aid-orchestrator/commands plugins/aid-orchestrator/skills` returns exactly `skills/pipeline.md`, AND each of the other five sites contains the reference phrase without that literal.
...
**Declared tiers are EXPECTATIONS the step then verifies, not decrees** — every tier above is what the suite is designed to measure, and the creating step confirms it with `--timing` + `aid-test-tier-lint.sh`. A suite that measures into a slower tier is retagged to the truth and any Success Criterion resting on its merge-path enforcement is reworded in the same step (see the Test tiering section).

**Aggregate invocation.** The runner's flag set changed under this plan and this step uses the CURRENT one (C0 round 5): P081 (v2.83.0) added `--tier t0|t1|t2`, `--timing` and `--include-delegated`, and CI now invokes it as `--tier t0` then `--tier t1`. This step runs the merge-path tiers exactly as CI does, with `--timing` added so the duration journal is refreshed and every tier tag stays VERIFIED rather than decaying to UNVERIFIED: `run-all-tests.sh --tier t0 --verbose --timing` and `run-all-tests.sh --tier t1 --verbose --timing`, plus the four T2 suites above invoked per-suite — so nothing this plan touches is proven only by a nightly run that happens after the release. The exact commands are recorded in the verify output.
...
- [ ] **Runtime leakage is proven impossible, not just absent from fixtures:** a deliberately MALICIOUS fixture (a gate `output` field carrying `ghp_`-shaped and `AKIA`-shaped strings, and a prose block carrying `password=hunter2`) is rendered through BOTH renderers, and the harness asserts none of those strings appears in the produced HTML or the chat card, while the provenance footer reports a non-zero redaction count.
- [ ] **The failure path is covered too:** a forced-renderer-failure fixture carrying every supported secret pattern proves the fallback card leaks none of them either — the bypass is closed, not merely discouraged in prose.
...
**No file in `/opt/eco/docs` is written by this plan** (C0 round 5 caught the contradiction: the step declared publication a PM-owned handoff while still directing an implementer to create a page and edit `sidebars.ts` in a repository this plan has no authority over). The implementer's deliverable is a **ready-to-apply handoff package, authored entirely inside this repo**: the spec page content (the file below, tracked here), the exact `sidebars.ts` entry to add as a quoted snippet, and the exact commands the PM runs. Facts the handoff must carry, verified at CP1 so the PM does not have to rediscover them: the target path is `/opt/eco/docs/docs/aid/specs/artifact-templates.md`, the `docs/aid/specs/` directory does not exist yet, that site's sidebar is 100% explicit (`grep -c autogenerated sidebars.ts` → 0) so a `_category_.json` is inert and an unregistered page never appears, and `onBrokenLinks`/`onBrokenAnchors` are both `throw` so a half-registered page breaks the docs build.
...
**Why the boundary is drawn there:** `/opt/eco/docs` is a SEPARATE git repository (`git -C /opt/eco/docs rev-parse --show-toplevel` → itself), so this plan's plan_branch ceremony could never commit there anyway, and a writable directory proves permission, not authority. The handoff package is therefore the deliverable, and the PM's commit is the publication.

**Completion gate — and an honest narrowing of what this plan can promise.** C0 round 1 showed the step could end as an open "PM-pending" with the page nowhere; C0 round 2 then showed that letting the in-repo fallback satisfy the AC would mean **all of Step 16's criteria pass while the declared Docusaurus deliverable is absent**. Both are true, and they cannot both be fixed by an acceptance criterion, because this plan has no authority to commit in another repository. So the promise is narrowed to what the plan can actually discharge:

- **This plan delivers the AUTHORED spec page**, tracked in this repo, reviewable in this repo's diff. That is Step 16's completion condition: `git ls-files plugins/aid-orchestrator/defaults/templates/artifact-templates-spec.md` returns the file.
- **Publication to Docusaurus is a PM-owned handoff, not a plan outcome, and the implementer writes nothing into that repository.** The step produces the handoff package described above and records it as a NAMED follow-up backlog item so it cannot evaporate. If the PM applies it during the run, the step records `git -C /opt/eco/docs ls-files docs/aid/specs/artifact-templates.md` and the resulting SHA as evidence of early completion — as an observation of the PM's action, never as its own.
- **The Scope entry and Success Criterion 6 are reworded to match** ("an AID artifact-template spec page is authored and handed to the PM for publication", not "published to Docusaurus"), so no criterion in this plan claims an outcome the plan cannot reach. A plan that promises another repository's state is a plan that lies at its own boundary.
...
**Error Handling:** No cross-repo write is attempted at all, so there is no access check and no write-failure branch — the handoff package is authored in this repo unconditionally. Version-registry drift (another plan released between generation and this step) → re-read the current version and bump from it; never hardcode the number in the plan. **The release sub-step is idempotent, and the idempotence is CHECKED rather than declared** (C0 round 2: "all 8 locations agree" is true after a double-bump too, so it cannot detect one). The step reads the current version first and treats an already-at-target state as a no-op; the detection is three assertions a second dispatch would fail, all using the declared `$NEW_VERSION` variable: (1) the heading `## [$NEW_VERSION]` appears **exactly once** in each of the two CHANGELOGs (`grep -c` == 1 — a re-run prepends a second one); (2) `git tag -l "v$NEW_VERSION"` shows the tag at most once and pointing at the release commit; (3) `git log --oneline --grep "release: v$NEW_VERSION" | wc -l` == 1. The shipped `release_changelog_paths` guard (registry ~:257, `test-aid-release.bats`) applies whenever `scripts/aid-release.sh` is used; state explicitly which of the 8 locations the script handles and which stay manual.

**Edge Cases:**
- `docs/aid/specs/` directory absent in the docs repo (verified absent at CP1) — the handoff package's commands include creating it, and note that `_category_.json` is inert on that fully explicit sidebar so the `sidebars.ts` entry is the only thing that makes the page appear. The implementer writes none of this; it is text handed to the PM.
...
**Acceptance Criteria:**
- [ ] All 8 version locations agree, proven by RUNNING `bash plugins/aid-orchestrator/scripts/tests/verify-version-files.sh "$NEW_VERSION" --baseline "$OLD_VERSION"` (exit 0) — not by a manual grep. **`<new_version>` is a MANDATORY argument** (C0 round 3: the script exits with usage without it, so the criterion as first written was unrunnable). The step resolves `OLD_VERSION` from the then-current CHANGELOG head and `NEW_VERSION` from the chosen minor bump, declares both as named variables at the top of the release sub-step, and every other assertion in this step (duplicate-heading, tag, release-commit count) uses that same `$NEW_VERSION` rather than a literal `X.Y.Z` placeholder. That script already exists (P079) but CP1 confirmed it has no mechanical caller anywhere: it falls outside the `test-*.sh` glob used by `scripts/tests/run-all-tests.sh`, `aid-release.sh` never calls it, and `.github/workflows/version-sync.yml` has the plugin check commented out at ~:21 with the note "Plugin has independent versioning". This step makes its invocation an explicit release-ceremony command and registers it as `version_registry_sync` in the enforcement registry with severity and surface stated honestly (invocation-time, release-boundary — NOT a CI gate). Deliberate decision recorded here rather than left implicit: the check stays release-boundary rather than being renamed into the suite glob, because the 8 locations legitimately diverge mid-development and a suite-wide run would fail every non-release commit.
- [ ] Both CHANGELOGs byte-identical for the new entry, and the release is provably single: `grep -c '^## \[X.Y.Z\]'` == 1 in each CHANGELOG, one tag, one `release: vX.Y.Z` commit.
- [ ] `git ls-files plugins/aid-orchestrator/defaults/templates/artifact-templates-spec.md` returns the authored spec page (the plan's own deliverable, tracked in this repo).
- [ ] The PM handoff is recorded as a named follow-up backlog entry carrying the exact `git -C /opt/eco/docs` commit command; if the PM committed during the run, the docs-repo SHA and a `sidebars.ts` reference check are recorded as evidence of early completion.
...
## Cross-EPIC ordering — what actually guarantees it

**Declared cross-EPIC dependency edges do NOT survive generation** (C0 round 5, verified: `strip_cross_phase_deps` at `scripts/aid-plan-to-epic.sh:727`, applied at :1199, removes every dependency pointing outside the current EPIC). Two orderings this plan needs are cross-EPIC: Step 3 (EPIC 1) before Step 6 (EPIC 2), both of which modify `commands/aid-help.md`; and Steps 1-2 (EPIC 1) before Step 14 (EPIC 3), which modifies `defaults/help-index.yaml` and the coverage suite.

What guarantees them is the **queue, not the graph**: this plan is generated with the default `--queue-mode chain`, so EPIC 2 is claimable only after EPIC 1 is merged into the plan branch, and EPIC 3 only after EPIC 2. That is a stronger guarantee than the edges would have been, and it is the reason the stripped edges are not a defect — but it is a REQUIREMENT, not an accident: **if this plan is ever regenerated with `--queue-mode separate`, the two shared-file orderings are lost and the EPICs can race.** The declared in-plan edges are kept anyway, because they survive WITHIN an EPIC and they document intent for a reader.

## Test tiering (mandatory for every new suite — P081, v2.83.0)

**This landed in main WHILE this plan was in CP1** (P081 merged as v2.83.0 during the C0 review round), and it changes what "the test blocks CI" means, so it is stated once here and binds all nine new suites this plan adds.

A suite declares its tier with a header comment `# aid-tier: t0|t1|t2` in its LEADING comment block (`scripts/lib/aid-test-tier.sh`). `ci.yml` now runs **T0 + T1 on the merge path; T2 runs nightly only**. An untagged suite matches no tier filter and therefore **never runs on the merge path** — the exact "looks green but never ran" failure the pilot exists to end. A duplicated or unknown tag is a hard error, not a guess.

**A tier is a MEASURED PROPERTY, not a policy choice.** `scripts/aid-test-tier-lint.sh` compares the tag against the suite's recorded duration and fails on a mismatch: `t0` under 2 s, `t1` under 30 s, everything slower is `t2` (`scripts/lib/aid-test-tier.sh:60-61`). So this plan cannot "assign" a tier by wishing — each step **measures its new suite, tags it with the measured tier, and runs `aid-test-tier-lint.sh` as part of its own ACs**.

**The measurement must EXIST, not merely be intended** (C0 round 7: `aid-test-tier-lint.sh` treats an absent duration record as `UNVERIFIED` and still exits 0, so an arbitrary tag passes the lint and silently moves a check between merge-path and nightly). Every step that creates a suite therefore: (1) runs that suite under `--timing` so a duration lands in the journal (`.aid-o/work/test-durations.jsonl`), (2) tags the file with the tier that duration implies, and (3) has an AC asserting the lint reports the tag **VERIFIED against a recorded duration** — an `UNVERIFIED` result fails the step. Step 15's aggregate invocations likewise pass `--timing`, so the release-time run refreshes the journal rather than leaving the tags resting on a stale or missing measurement.

What the plan CAN control is whether a suite is cheap enough to land on the merge path. Two of this plan's claims depend on merge-path enforcement, so they carry a design constraint rather than a tag assertion:
- `test-help-index-coverage.bats` — Success Criterion 1 ("a new command fails CI until intentionally indexed") is only true if this suite measures into t0/t1. It is therefore written as pure file/YAML inspection with **no git fixture and no subprocess per row**, which is what keeps it under the t1 threshold. If it nonetheless measures t2, Success Criterion 1 is reworded to "fails the nightly run" in the same step — the criterion follows the measurement, never the other way round.
- `test-communication-wiring.sh` and `test-enforcement-registry-cites.sh` — same shape (grep/yq over files), same expectation, same fallback rule.

`test-init-idempotency.sh` and `test-integration-handoff-rendering.sh` build real fixture repos and will measure t2; that is expected and accepted — their claims are regression-shaped, not merge-gate-shaped.

**`test-fsm-step-render.bats` is already tagged `# aid-tier: t2` and this plan does NOT retier it** — P081 made that call deliberately and on a measurement, and Step 13 changing the suite's content is not a reason to relitigate its cost class. The consequence is stated plainly instead of hidden: the Step 13 regression runs nightly, not on the merge path, and **Step 15 therefore invokes it explicitly** alongside the other T2 suites so this plan still proves it green before release.

Step 16's registry rows record each check's tier in the row's surface/severity wording, so a row can never imply merge-blocking enforcement for a nightly-only suite.
...
3. Every init/setup-written config file has exactly one declared owner, stated in both command surfaces; the **scripted substrate** of init (the operations delegated to shipped libraries) is proven idempotent by the harness. Init operations performed inline by prose — hook installation at this HEAD — are explicitly outside that proof, and extracting them into a library is a named follow-up, not a claim of this plan.
4. Every public surface's final turn is inventoried (`final_turn` column) and the three deterministic renderers ship with verbatim-presentation wiring and passing goldens; a waiver is never rendered as a pass. Artifact BODIES are generated and the publish-before-present wiring is asserted at all four renderer sites; live publication through the session's Artifact tool stays an external controller responsibility and is not claimed as a tested outcome.
5. Humans read `Plan Step N` at the public seams; machine surfaces byte-unchanged.
6. Release shipped with all 8 version locations synchronized (proven by RUNNING `verify-version-files.sh`, not by a manual grep) and the AID artifact spec page authored, tracked in this repo, and handed to the PM as a named follow-up for Docusaurus publication.
