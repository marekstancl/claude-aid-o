...
**Completion gate — the deliverable cannot end as an open "PM-pending" (C0 cross-provider review: as first written, the terminal step could finish with the required page published nowhere).** The step completes on exactly one of two recorded outcomes, never on a pending one:
- **(a) docs-repo commit recorded** — `git -C /opt/eco/docs ls-files docs/aid/specs/artifact-templates.md` returns the file and the commit SHA is written into the verify output; or
- **(b) the tracked in-repo fallback exists** — the fallback file below is committed in THIS repo and `git ls-files` returns it.

The fallback activates on **any** unavailable PM completion — not only on a refused write, but equally when the PM has not committed by the time this step finishes. Release completion is conditional on (a) or (b); "PM-pending" is a transient state during the step, never its terminal report.
...
- [ ] **Exactly one of the two completion outcomes is recorded, and the step fails if neither is** — (a) `git -C /opt/eco/docs ls-files docs/aid/specs/artifact-templates.md` returns the file AND `sidebars.ts` references its id (mere existence is NOT acceptance — an untracked or unregistered page reaches nobody), or (b) `git ls-files plugins/aid-orchestrator/defaults/templates/artifact-templates-spec.md` returns the tracked fallback.
