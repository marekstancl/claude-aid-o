...
- **Publication to Docusaurus is a PM-owned handoff, not a plan outcome.** The step writes the page and `sidebars.ts` entry into `/opt/eco/docs`, prints the exact commit command, and — whether or not the PM commits during the run — records the handoff as a NAMED follow-up backlog item so it cannot evaporate. If the PM does commit during the run, the step additionally records `git -C /opt/eco/docs ls-files docs/aid/specs/artifact-templates.md` and the resulting SHA as evidence of early completion.
