...
- Test: `plugins/aid-orchestrator/scripts/tests/bats/test-help-index-coverage.bats` + `test-aid-config-summary.bats` + `test-init-idempotency.sh` + `test-communication-wiring.sh` + `test-enforcement-registry-cites.sh` + `test-fsm-step-render.bats` + `test-aid-artifact-render.bats` + `test-aid-gate-outcome-summary.bats` + `test-aid-plan-close-summary.bats` — full pass documented in the harness header. **The runner's flag set changed under this plan and the step uses the CURRENT one** (C0 round 5): P081 (v2.83.0) added `--tier t0|t1|t2`, `--timing` and `--include-delegated` to the `--verbose/-v/--list/--help` this plan was written against, and CI now invokes it as `--tier t0` then `--tier t1`. This step runs the merge-path tiers exactly as CI does, with `--timing` added so the duration journal is refreshed and every tier tag stays VERIFIED rather than decaying to UNVERIFIED — `run-all-tests.sh --tier t0 --verbose --timing` and `--tier t1 --verbose --timing` — plus explicit per-suite invocations for every T2 suite this plan creates OR modifies — `test-init-idempotency.sh`, `test-integration-handoff-rendering.sh`, **`test-fsm-step-render.bats`** (modified by Step 13 and staying t2) and **`test-skill-lint.sh`** (modified by Steps 3/5/6's delisting) — so nothing this plan touches is proven only by a nightly run that happens after the release. The exact commands are recorded in the verify output.
