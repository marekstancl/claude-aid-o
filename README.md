...
- **v2.58.4** (current) — CI fix: a SIGPIPE flake in `test-regression.sh` (a 36 KB `echo … | grep -q` under `set -o pipefail`) that had failed the `bash-tests` CI job for several releases, resolved by switching those greps to here-strings
