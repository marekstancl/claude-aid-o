...
- **v2.60.0** (current) — C3 cross-provider dispatch bridge: the independent audit E8 only detected feasibility of now runs for real against a different vendor (Codex) via `aid-c3-dispatch.sh` (`build-manifest`/`dispatch`/`verify`), with a versioned prompt template + deterministic renderer, a Codex-response schema, and a `c3_executor` policy (`c3_on_unavailable: unverifiable`). The FSM `done-advance` C3 hook now enforces the full provenance + faithful-transform chain via a `verify` shell-out (real merge-blocking capability; shipped enforcement stays `observe`, blocking promotion at E10), and `pipeline.md`'s `c3` audit dispatch uses the bridge instead of an in-process Claude call
