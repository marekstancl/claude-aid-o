...
- **v2.56.0** (current) — P063 "Gate Runtime Baselines" (4 steps): `scripts/lib/aid-gate-runtime-baseline.sh` percentile-driven runtime-baseline library, `aid-run-gates.sh` sample-recording integration + lazy `.aid-o/metrics/` gitignore backfill, a repeated-timeout `gate_timeout_policy_block` GATES:EXECUTE precondition (stops retrying a gate whose last 3 samples all timed out at/above its configured timeout), and the `aid-gate-runtime-report.sh` CLI (+ `gate_runtime_baseline_advisory` registry row) exposing per-gate p95/timeout/run-mode recommendations to a human without hand-crafted yq/jq
- **v2.55.0** — P061 EPIC 1/6 (gate profile substrát + plan-gate floor): `aid-run-gates.sh --profile` flag + `profile_excluded` reporting, `aid-fsm.sh` GATES:DONE plan-gate floor (`plan_gate_profile_excluded`/`plan_json_malformed`), gates-enum unlock (`plan.schema.json`/`aid-epic-to-json.sh`), generic `gate_profiles` substrate for new/existing projects (D3/D9), release-policy surface-rule bootstrap check, plus a mid-flight test-cost hotfix for `test-release-policy.bats` (18s/test → 4.6s/test)
- **v2.54.0** — P060 false-green / stale-evidence hardening (E-060-2_2): 8 OBS-ledger fixes — gate-count integrity + undefined-gate reconciliation in `aid-run-gates.sh`, CP2 step-range prefilter, CP3 head-freshness check, runtime cache preflight (`scripts/lib/aid-cache-preflight.sh`, IMP-179 partial), commit-path guard hook, queue dependency revalidation, C4 `head_match` policy hook — plus the E11 enablement map with the mandatory K4×K8 binding and 8 new enforcement-registry rows (271→279)
