...
- **v2.28.4** (current) — added compliance recovery alert: AID now emits a `✅ compliance cleared, release unblocked` Telegram alert when a previously-blocked EPIC clears all blocking compliance failures, paired with an `fsm_done_advance_recovered` timeline event for dedup and auditability
