...
- **v2.62.0** (current) — Phase-1 control-plane hardening: controller-owned job supervisor, gate-scoped single-use waivers, idempotent step advancement, one fail-closed mode authority, fail-closed lineage and re-derived attestation, explicit C3 AC/test-evidence binding, and read-time evidence freshness.
...
- **v2.60.1** — AUTO liveness instruction hotfix: the controller retains ownership of FSM, commits, gates, evidence, and background jobs through a terminal outcome; recoverable technical decisions route to bounded Codex adjudication instead of repeatedly stopping at the PM; test claims are bound to immutable revisions and long-running work has an explicit ownership/deadline contract.
- **v2.60.0** — C3 Cross-Provider Dispatch Bridge plan complete (P065, 7 EPICs): real merge-gate enforcement (`done-advance` shells out to `aid-c3-dispatch.sh verify`), an advisory same-provider Claude fallback for when Codex is unavailable, a bounded C3 fix→reverify loop, and the plan-time C0 counterpart (`aid-c0-plan-review.sh`) with its own revision-limit ledger (`aid-cp1-ledger.sh`) — AID's "independent cross-provider audit" claim is now backed end to end by an actually-independent, actually-verified second opinion. Fixes a CRITICAL verify-binding bypass and a fail-open exit-code bug found via 12 rounds of the plan's own live-Codex dogfooding. Enforcement stays staged at `observe`; full blocking promotion is a separate, deferred decision (P062/E10)
