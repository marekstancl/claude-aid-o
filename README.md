...
- **v2.78.0** (current) — the audit asks "what exists", not "what do gates run": CI test-execution check, wrapper-aware marker-exact reachability, per-test cost
- **v2.77.4** — runner-aware test-case counting: pytest and js/ts cases counted alongside bats, per-runner breakdown in the report
- **v2.77.3** — every finding of an independent review landed: runner-aware scanning, mechanical fabrication check, honest not-applicable, truthful sources, today's gates. Previously v2.77.2 — readable in both themes, counts test cases not just suites, names the parallelism blocker, measures naming conventions. Previously v2.77.1 — v2.77.0 plus the publish-the-page mandate from v2.76.1. Previously v2.77.0 — `epic-start` finally has a caller. It registers an EPIC's task branch with lineage back to its plan, and had been built, tested and documented without ever being wired — invisible while everything ran in one checkout, fatal the moment a plan implemented in its own tree. Generation now registers each EPIC before init needs it, hands your branch back when an init refuses instead of parking you on a task branch, and returns the plan worktree to its plan branch after every phase, so a multi-phase generation completes instead of stopping after the first.
