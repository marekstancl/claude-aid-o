...
**Multi-agent orchestration plugin for [Claude Code](https://claude.com/claude-code).** v2.107.0
...
**8 agents** — Implementer (and its low-effort twin), Reviewer-light, Verifier, Gate-fixer, Auditor, Simplifier, Project-scanner. The Implementer is dispatched per step from your plan's dependency graph; every delivery is read by reviewer rounds (step, EPIC, whole plan).
...
- **v2.107.0** (current) — mantinely místo slepých uliček: každé odmítnutí jmenuje příkaz, který pokračuje; revizní kola bez slepých uliček a spor k PM; autor plánu pouští kontrolu generování; rozsah, kandidát a uzavření plánu pokrývají, co kroky legitimně dělají
...
- `CONTRIBUTING.md` — what a change must update, how a release is cut, test tiers, the testbed
