...
- **v2.58.3** (current) — Plan-time Files-shape lint: malformed `**Files:**` entries are caught when the plan is written (and as a deterministic hard pre-flight in `aid-plan-to-epic.sh` that can't be skipped), instead of blowing up phase-by-phase during EPIC generation. The lint, the generator, the JSON deriver and the D5 gate now share ONE Files extractor + path cleaner + shape predicate in `lib/aid-scoping.sh` (an integration test proves lint-clean ⇒ gate-clean), and `plan-writing.md` documents the canonical Files grammar. Two-tier severity: gate-breaking entries always block; non-canonical-but-cleaner-OK entries block `lifecycle_strict` plans and are advisory for legacy plans
- **v2.58.2** — Hotfix hardening the IMP-232 isolated-index write path: the untracked-collision guard now covers the manifest too (built to a temp first; a differing untracked manifest is refused, never overwritten — parity with the receipt), a closure-receipt commit failure in `record-delivery`/`plan-reconcile --apply` propagates a non-zero return instead of a false success, and a plan can declare `depends_on_plans:` in its frontmatter so the D1 init gate hard-blocks on a real unclosed dependency via the normal path
- **v2.58.1** — Wires the IMP-232 plan-level closure model into the runtime lifecycle (v2.58.0 shipped it unwired): the pipeline creates a git-tracked manifest at plan scaffold, a named post-merge hook `aid-fsm.sh plan-record-delivery` records the real merge SHA + review provenance and writes the closure receipt when the last required EPIC is delivered + review-accepted, `aid-fsm.sh plan-reconcile`/`plan-state` become real subcommands, all lifecycle commits use an isolated git index (user index never touched), and a merged-but-unverifiable EPIC is honestly `delivered`/`review: unverifiable` (plan stays `active`)
