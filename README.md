...
| `/aid-init` | Create or upgrade the `.aid-o/` workspace — 15-item base manifest, stack auto-detection, idempotent |
...
- **v2.83.0** (current) — the full test portfolio leaves the merge path: every suite declares a measured tier, merges run T0+T1 in 13 min instead of 6 h 49 min, and the whole portfolio runs nightly with a budget on the way in and a reaper on the way out
