...
- **v2.29.2** (current) — fixed `verifier_provenance` false-positive blocking: `dispatch_mode` defaulted to `subagent` (requires timeline events the CC Agent tool never writes), permanently blocking every EPIC in standard self-hosted operation; default is now `agent_tool`
